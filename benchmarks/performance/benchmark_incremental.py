...
def benchmark_incremental_parsing():
    ...
    try:
        ...
        print(f"   Incremental: {incremental_time:.3f}s ({len(incremental_chunks)} chunks)")
        ...
        start = time.perf_counter()
        regular_chunks = regular_chunker.chunk_file(file_path, "python", force_reparse=True)
        regular_time = time.perf_counter() - start
        print(f"   Regular reparse: {regular_time:.3f}s")
        
        start = time.perf_counter()
        incremental_chunks = incremental_chunker.chunk_file_incremental(file_path, "python")
        incremental_time = time.perf_counter() - start
        ...
        if 'incremental.success' in metrics:
            print(f"   Successful incremental parses: {metrics['incremental.success']['count']}")
        if 'incremental.no_change' in metrics:
            print(f"   No-change detections: {metrics['incremental.no_change']['count']}")
