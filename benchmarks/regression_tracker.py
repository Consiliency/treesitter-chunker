...
from benchmark import run_benchmarks, BenchmarkSuite
...
class PerformanceRegressionTracker:
    ...
    def record_baseline(self, name: str, measurements: List[float], 
                       commit_hash: Optional[str] = None,
                       metadata: Dict[str, Any] = None) -> PerformanceBaseline:
    ...
    def check_regression(self, name: str, current_measurements: List[float],
                        threshold: float = 0.1,
                        confidence_level: float = 0.95) -> Optional[RegressionResult]:
    ...
    def run_regression_tests(self, test_directory: Path, language: str = "python",
                           iterations: int = 5) -> List[RegressionResult]:
    ...
    def update_baseline(self, name: str, measurements: List[float], force: bool = False) -> bool:
...
class PerformanceHistory:
    ...
    def get_trend(self, metric: str, window: int = 10) -> Optional[str]:
        ...
        if slope < -0.01:  # Performance improving (times decreasing)
            return 'improving'
        elif slope > 0.01:  # Performance degrading
            return 'degrading'
        else:
            return 'stable'
    
    def plot_history(self, metric: str, output_file: Optional[Path] = None):
...
if __name__ == "__main__":
    ...
    parser.add_argument("command", choices=["check", "update", "report", "history"],
                       help="Command to run")
