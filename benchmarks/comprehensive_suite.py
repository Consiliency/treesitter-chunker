...
class ComprehensiveBenchmarkSuite:
    ...
    def _setup_scenarios(self):
        ...
        self.scenarios.append(BenchmarkScenario(
            ...
            metadata={"category": "scaling", "sizes": ["small", "medium", "large", "xlarge"]}
        ))
        ...
        self.scenarios.append(BenchmarkScenario(
            ...
            metadata={"category": "strategy", "strategies": ["semantic", "hierarchical", "adaptive", "composite"]}
        ))
        ...
        self.scenarios.append(BenchmarkScenario(
            ...
            metadata={"category": "cache", "scenarios": ["cold", "warm", "partial"]}
        ))
        ...
        self.scenarios.append(BenchmarkScenario(
            ...
            metadata={"category": "fallback", "file_types": ["code", "text", "mixed"]}
        ))
        ...
        self.scenarios.append(BenchmarkScenario(
            ...
            metadata={"category": "export", "formats": ["json", "jsonl", "parquet", "csv"]}
        ))
    ...
    def _setup_multi_language_files(self) -> Dict[str, List[Path]]:
        ...
        for lang in ["python", "javascript", "rust", "c", "cpp"]:
            try:
                ...
                files[lang] = [self._create_test_file(lang, size) for size in ["small", "medium", "large"]]
            except:
...
"""
    ...
    def _benchmark_strategies(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for name, strategy in strategies.items():
            ...
            for _ in range(5):
                start = time.perf_counter()
                chunks = strategy.chunk(tree.root_node, source, str(test_file), "python")
                elapsed = time.perf_counter() - start
                times.append(elapsed)
            ...
            results[name] = {
                "mean_time": statistics.mean(times),
                "chunks": len(chunks),
                "avg_chunk_size": statistics.mean([c.end_line - c.start_line for c in chunks]) if chunks else 0
            }
    ...
    def _benchmark_concurrency(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for workers in [1, 2, 4, 8, multiprocessing.cpu_count()]:
            ...
            results[f"{workers}_workers"] = {
                ...
                "speedup": results.get("1_workers", {}).get("mean_time", times[0]) / statistics.mean(times) if workers > 1 else 1.0
            }
    ...
    def _benchmark_memory(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for size_name, file_path in context["files"].items():
            ...
            results[size_name] = {
                ...
                "chunks": len(chunks),
                "memory_per_chunk": (final_memory - baseline_memory) / len(chunks) if chunks else 0
            }
        ...
        for scenario_name, scenario_data in results.items():
            ...
            if scenario_name == "language_comparison":
                for lang, stats in scenario_data["results"].items():
                    lines.append(f"  {lang}: {stats['mean_time']*1000:.2f}ms ({stats['total_chunks']} chunks)")
            
            elif scenario_name == "file_size_scaling":
                for size, stats in scenario_data["results"].items():
                    lines.append(f"  {size}: {stats['mean_time']*1000:.2f}ms ({stats['file_size_kb']:.1f}KB, {stats['time_per_kb']*1000:.2f}ms/KB)")
            
            elif scenario_name == "strategy_comparison":
                for strategy, stats in scenario_data["results"].items():
                    lines.append(f"  {strategy}: {stats['mean_time']*1000:.2f}ms ({stats['chunks']} chunks, avg size: {stats['avg_chunk_size']:.1f} lines)")
            
            elif scenario_name == "concurrency_scaling":
                for workers, stats in scenario_data["results"].items():
                    lines.append(f"  {workers}: {stats['mean_time']*1000:.2f}ms (speedup: {stats.get('speedup', 1.0):.2f}x)")
...
if __name__ == "__main__":
    ...
    # Save report
    report_file = suite.output_dir / f"benchmark_report_{time.strftime('%Y%m%d_%H%M%S')}.txt"
