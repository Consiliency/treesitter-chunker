...
class PackageAutomation:
    ...
    def __init__(self, project_dir: Path, version: Optional[str] = None):
    ...
    def _build_sdist(self):
        ...
        subprocess.run([sys.executable, "-m", "build", "--sdist", "--outdir", str(self.dist_dir)], 
                     cwd=self.project_dir, check=True)
    ...
    def _build_wheels(self):
        ...
        subprocess.run([sys.executable, "-m", "build", "--wheel", "--outdir", str(self.dist_dir)], 
                     cwd=self.project_dir, check=True)
    ...
    def _build_platform_wheels(self):
        ...
        if system == "linux":
            ...
            subprocess.run(["cibuildwheel", "--platform", "macos"], 
                         cwd=self.project_dir,
                         env={**os.environ, "CIBW_OUTPUT_DIR": str(self.dist_dir)})
        elif system == "windows":
            subprocess.run(["cibuildwheel", "--platform", "windows"], 
                         cwd=self.project_dir,
                         env={**os.environ, "CIBW_OUTPUT_DIR": str(self.dist_dir)})
    ...
    def prepare_release(self, output_dir: Optional[Path] = None):
