...
def test_postgres_advanced_features():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert "line_count INTEGER GENERATED ALWAYS AS (end_line - start_line + 1)" in schema
        ...
        # Verify CSV content
        with open(output_dir / "test_chunks.csv", 'r') as f:
            reader = csv.reader(f)
            rows = list(reader)
        ...
        for query_name in required_queries:
            ...
            if query_name == "similarity_search":
                ...
                assert any("gin_trgm_ops" in idx for idx in exporter.get_index_statements())
