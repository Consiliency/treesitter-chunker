...
from id import (
    AmbientCredentialError,  # type: ignore
    detect_credential,
)
...
class Resolver:
    ...
    def make_trusted_publishing_token(self) -> str | None:
    ...
    def get_username_from_keyring(self) -> str | None:
        if keyring is None:
    ...
    def get_password_from_keyring(self) -> str | None:
        if keyring is None:
            logger.info("keyring module is not available")
            return None
        try:
...
class Private(Resolver):
    def prompt(self, what: str, how: Callable[..., str] | None = None) -> str:
