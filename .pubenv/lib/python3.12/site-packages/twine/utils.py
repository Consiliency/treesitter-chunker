...
# get_userpass_value.
RepositoryConfig = dict[str, str | None]
...
def get_config(path: str) -> dict[str, RepositoryConfig]:
    ...
    config = collections.defaultdict(defaults.copy)

    index_servers = parser.get(
        "distutils",
        "index-servers",
        fallback="pypi testpypi",
    ).split()
...
def _config_from_repository_url(url: str) -> RepositoryConfig:
    ...
    if parsed.username:
        ...
        config["repository"] = cast(
            str,
            rfc3986.urlparse(url).copy_with(userinfo=None).unsplit(),
        )
    config["repository"] = normalize_repository_url(cast(str, config["repository"]))
    return config
...
def check_status_code(response: requests.Response, verbose: bool) -> None:
    ...
    if response.status_code == 405 and "pypi.org" in response.url:
...
def get_userpass_value(
    ...
    key: str,
    prompt_strategy: Callable[[], str] | None = None,
) -> str | None:
...
class EnvironmentFlag(argparse.Action):
    ...
    @staticmethod
    def bool_from_env(val: str | None) -> bool:
