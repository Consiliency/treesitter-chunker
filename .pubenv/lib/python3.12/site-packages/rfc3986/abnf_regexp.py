...
REGULAR_NAME_RE = REG_NAME = "((?:{}|[{}])*)".format(
    "%[0-9A-Fa-f]{2}",
    SUB_DELIMITERS_RE + UNRESERVED_RE,
)
...
LS32_RE = f"({HEXDIG_RE}:{HEXDIG_RE}|{IPv4_RE})"
...
IP_LITERAL_RE = rf"\[({IPv6_ADDRZ_RFC4007_RE}|{IPv_FUTURE_RE})\]"

# Pattern for matching the host piece of the authority
HOST_RE = HOST_PATTERN = f"({REG_NAME}|{IPv4_RE}|{IP_LITERAL_RE})"
...
PATH_RE = (
    f"^({PATH_ABEMPTY}|{PATH_ABSOLUTE}|{PATH_NOSCHEME}|{PATH_ROOTLESS}|{PATH_EMPTY})$"
)
...
IPATH_RE = f"^(?:{IPATH_ABEMPTY}|{IPATH_ABSOLUTE}|{IPATH_NOSCHEME}|{IPATH_ROOTLESS}|{PATH_EMPTY})$"

IREGULAR_NAME_RE = IREG_NAME = "(?:{}|[{}])*".format(
    "%[0-9A-Fa-f]{2}",
    SUB_DELIMITERS_RE + IUNRESERVED_RE,
)

IHOST_RE = IHOST_PATTERN = f"({IREG_NAME}|{IPv4_RE}|{IP_LITERAL_RE})"
