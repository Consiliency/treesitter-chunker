...
class Validator:
    ...
    def check_validity_of(self, *components):
        ...
        self.validated_components.update(dict.fromkeys(components, True))
        return self
    ...
    def require_presence_of(self, *components):
        ...
        for component in components:
            if component not in self.COMPONENT_NAMES:
                raise ValueError(f'"{component}" is not a valid component')
        self.required_components.update(dict.fromkeys(components, True))
        return self
...
def host_is_valid(host, require=False):
    ...
    if validated and host is not None and misc.IPv6_MATCHER.match(host):
