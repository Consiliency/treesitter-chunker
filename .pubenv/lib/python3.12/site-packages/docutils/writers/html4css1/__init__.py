...
class Writer(writers._html_base.Writer):
    ...
    settings_spec = frontend.filter_settings_spec(
        ...
        math_output=(
            ...
            ["--math-output"],
            {"default": "HTML math.css", "validator": frontend.validate_math_output},
        ),
        xml_declaration=(
...
class HTMLTranslator(writers._html_base.HTMLTranslator):
    ...
    def visit_field_list(self, node) -> None:
        ...
        self.compact_p = None
        if "compact" in node["classes"] or (self.settings.compact_field_lists and "open" not in node["classes"]):
    ...
    def should_be_compact_paragraph(self, node) -> bool:
        ...
        """
        if isinstance(node.parent, nodes.compound | nodes.document):
...
class SimpleListChecker(writers._html_base.SimpleListChecker):
    ...
    def visit_list_item(self, node):
        ...
        if (
            ...
            and (
                isinstance(children[-1], nodes.bullet_list | nodes.enumerated_list)
            )
        ):
            children.pop()
        if len(children) <= 1:
