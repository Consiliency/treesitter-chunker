# $Id: __init__.py 10160 2025-06-10 09:57:32Z milde $
...
class LaTeXTranslator(writers.DoctreeTranslator):
    ...
    def __init__(self, document, babel_class=Babel) -> None:
        ...
        # Document parts
        self.head_prefix = [
            f"\\documentclass[{self.documentoptions}]{{{settings.documentclass}}}",
        ]
        ...
        if self.fallback_stylesheet:
            ...
            if settings.legacy_class_functions:
                ...
                self.fallbacks["_docutils.sty"] = r"\usepackage{docutils}[2024-09-24]"
        ...
        if self.use_latex_docinfo and isinstance(node, self.TITLEDATA_NODES):
            ...
            if isinstance(node, nodes.authors):
                ...
                for nn in node.findall(
                    ...
                    if isinstance(
                        nn,
                        (nodes.address, nodes.contact, nodes.organization),
                    ):
                        s = r" \quad "
            else:
                s = ""
    ...
    def depart_document(self, node) -> None:
        ...
        if self.document.get("title", ""):
            self.pdfinfo.insert(
                0,
                "  pdftitle={%s}," % self.encode(self.document.get("title", "")),
            )
        if self.pdfauthor:
    ...
    def visit_inline(self, node) -> None:
        ...
        set_anchor = not (
            isinstance(node.parent, (nodes.caption, nodes.title))
    ...
    def visit_title(self, node) -> None:
        ...
        elif isinstance(node.parent, nodes.admonition | nodes.sidebar | nodes.topic):
