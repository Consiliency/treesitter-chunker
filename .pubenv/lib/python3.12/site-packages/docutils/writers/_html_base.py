    ...
    settings_spec = (
        ...
        (
            ...
            (
                ...
                ["--math-output"],
                {"default": "MathML", "validator": frontend.validate_math_output},
            ),
            (
...
class HTMLTranslator(writers.DoctreeTranslator):
    ...
    def image_size(self, node: nodes.image) -> dict[str, str]:
        ...
        if "scale" in node and len(measures) < 2:
            ...
            if imgsize:
                for dimension, value in zip(dimensions, imgsize, strict=False):
    ...
    def prepare_svg(self, code: str, node: nodes.Element, atts: dict) -> str:
        ...
        if "style" in atts:
            ...
            svg.set(
                "style",
                " ".join(f"{k}: {v};" for k, v in declaration_dict.items()),
            )
        ...
        if ids:
            ...
            for id in ids[1:]:
                ...
                if empty or isinstance(
                    node,
                    (nodes.Sequential, nodes.docinfo, nodes.table),
                ):
    ...
    def visit_docinfo_item(self, node, name, meta=True) -> None:
        if meta:
            self.meta.append(
                f'<meta name="{name}" content="{self.attval(node.astext())}" />\n',
            )
        self.body.append(
    ...
    def visit_list_item(self, node):
        ...
        if (
            ...
            and (
                isinstance(
                    children[-1],
                    nodes.bullet_list | nodes.enumerated_list | nodes.field_list,
                )
            )
        ):
            children.pop()
