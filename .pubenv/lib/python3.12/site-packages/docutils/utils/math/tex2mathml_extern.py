...
def blahtexml(math_code, as_block=False) -> str:
    ...
    result = subprocess.run(args, input=math_code, capture_output=True, text=True, check=False)
...
def latexml(math_code, as_block=False):
    ...
    result1 = subprocess.run(args1, input=math_code, capture_output=True, text=True, check=False)
    if result1.stderr:
        result1.stderr = "\n".join(
    ...
    result2 = subprocess.run(
        args2, input=result1.stdout, capture_output=True, text=True, check=False,
    )
...
def pandoc(math_code, as_block=False):
    ...
    result = subprocess.run(
        args, input=wrap_math_code(math_code, as_block), capture_output=True, text=True, check=False,
    )
...
def ttm(math_code, as_block=False):
    ...
    try:
        result = subprocess.run(
            args, input=math_code, capture_output=True, text=True, encoding="ISO-8859-1", check=False,
        )
