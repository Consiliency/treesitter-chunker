...
class RSTState(StateWS):
    ...
    def check_subsection(self, source, style, lineno) -> bool:
        ...
        if level > mylevel + 1:
            styles = " ".join("/".join(s for s in style) for style in title_styles)
            self.parent += self.reporter.severe(
                "Inconsistent title style:" f" skip from level {mylevel} to {level}.",
...
class Body(RSTState):
        ...
        while indented:
            ...
            if attribution_lines:
                attribution, messages = self.parse_attribution(
                    attribution_lines,
                    line_offset + attribution_offset,
                )
    ...
    def disallowed_inside_substitution_definitions(self, node) -> bool:
        if (
            ...
            or (isinstance(node, nodes.footnote_reference) and node.get("auto"))
        ):
            return True
        return False
