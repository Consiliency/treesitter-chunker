...
def test_binary_total_values_length():
    ...
    large_arr = pa.array(
        [b"0000", None, b"11111", b"222222", b"3333333"],
        type="large_binary",
    )
...
def test_array_iter():
    ...
    for i, j in zip(range(10), arr, strict=False):
        assert i == j.as_py()
...
def test_map_from_arrays():
    ...
    pypairs = list(zip(pykeys, pyitems, strict=False))
...
def test_array_pickle(data, typ, pickle_module):
    ...
    for proto in range(pickle_module.HIGHEST_PROTOCOL + 1):
...
@pytest.mark.parametrize(
    "binary_type",
    [None, pa.binary(), pa.large_binary(), pa.binary_view()],
)
...
@pytest.mark.parametrize(
    "string_type",
    [None, pa.utf8(), pa.large_utf8(), pa.string_view()],
)
...
def test_binary_array_masked():
    ...
    assert masked_nulls.to_pylist() == [None]
    ...
    npa[npa == b"bbb"] = b"XXX"
    assert arrow_array.to_pylist() == ([b"aaa", b"bbb", b"ccc"] * 10)
...
def test_binary_array_strided():
    ...
    assert arrow_array.to_pylist() == [b"ab", b"ef"]
...
def test_non_cpu_array():
    ...
    arr_with_nulls = pa.Array.from_buffers(
        pa.int32(),
        4,
        [cuda_validity_buf, cuda_data_buf],
    )
