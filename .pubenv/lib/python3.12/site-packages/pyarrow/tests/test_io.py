...
def test_python_file_iterable():
    ...
    with pa.PythonFile(buf, mode="r") as f:
        for read, expected in zip(f, buf2, strict=False):
...
def check_buffer_pickling(buf, pickler):
    ...
    for protocol in range(pickler.HIGHEST_PROTOCOL + 1):
...
def test_cache_options():
    ...
    opts4 = pa.CacheOptions(
        hole_size_limit=4096,
        range_size_limit=8192,
        prefetch_limit=5,
    )
...
@pytest.mark.xfail(
    ...
    reason="umask doesn't work on Emscripten",
)
