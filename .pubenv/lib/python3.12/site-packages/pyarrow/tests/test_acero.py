...
def test_table_source():
    ...
    with pytest.raises(
        ValueError,
        match="TableSourceNode requires table which is not null",
    ):
        _ = decl.to_table()
...
@pytest.mark.parametrize(
    "source",
    [pa.record_batch({"number": [1, 2, 3]}), pa.table({"number": [1, 2, 3]})],
)
...
def test_project(table_source):
    ...
    decl = Declaration.from_sequence(
        [
            ...
            Declaration(
                "project",
                ProjectNodeOptions([pc.multiply(field("a"), 2)], ["a2"]),
            ),
        ],
    )
    result = decl.to_table()
...
def test_aggregate_scalar(table_source):
    decl = Declaration.from_sequence(
        [
            ...
            Declaration(
                "aggregate",
                AggregateNodeOptions([("a", "sum", None, "a_sum")]),
            ),
        ],
    )
    result = decl.to_table()
    ...
    with pytest.raises(
        ValueError,
        match="Function 'sum' accepts 1 arguments but 2 passed",
    ):
        _ = decl.to_table()
...
def test_aggregate_hash():
    ...
    aggr_opts = AggregateNodeOptions(
        [("a", "hash_count", pc.CountOptions("all"), "count(a)")],
        keys=["b"],
    )
    decl = Declaration.from_sequence(
        [table_source, Declaration("aggregate", aggr_opts)],
...
def test_order_by():
    ...
    with pytest.raises(
        ValueError,
        match="`ordering` must be an explicit non-empty ordering",
    ):
        _ = decl.to_table()
