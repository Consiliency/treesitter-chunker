...
def test_option_class_equality(request):
    ...
    for option1, option2 in zip(options, options[1:], strict=False):
...
def test_utf8_zfill():
    ...
    result = pc.utf8_zero_fill(
        arr,
        options=pc.ZeroFillOptions(width=5, padding="💠"),
    ).to_pylist()
    assert result == ["💠💠💠💠1", "-💠💠💠2", "+💠💠💠3"]
    ...
    result = pc.utf8_zero_fill(
        arr,
        options=pc.ZeroFillOptions(width=4, padding="x"),
    ).to_pylist()
...
def test_is_nan():
    ...
    with pytest.raises(
        ...
        match="has no kernel matching input types",
    ):
        _ = arr.is_nan()
...
def test_fsl_to_fsl_cast(value_type):
    ...
    fsl = pa.FixedSizeListArray.from_arrays(
        pa.array([1, 2, 3, 4, 5, 6], type=dtype),
        type=type,
    )
    ...
    fsl = pa.FixedSizeListArray.from_arrays(
        pa.array([1, None, None, 4, 5, 6], type=dtype),
        type=type,
    )
    assert cast_type == fsl.cast(cast_type).type
    ...
    type = pa.list_(pa.field("values", dtype), 2)
    fsl = pa.FixedSizeListArray.from_arrays(
        pa.array([None, None, None, None, None, None], type=dtype),
        type=type,
    )
    assert cast_type == fsl.cast(cast_type).type
    ...
    with decimal.localcontext() as ctx:
        ...
        for scale in range(min_scale, max_scale):
            ...
            for i in range(5):
                ...
                actual = pc.cast(
                    pa.scalar(float_val, type=float_ty),
                    decimal_ty,
                ).as_py()
...
def test_strptime():
    ...
    expected = pa.array(
        [datetime.datetime(2020, 1, 5), None, None],
        type=pa.timestamp("s"),
    )
...
def test_assume_timezone():
    ...
    with pytest.raises(
        ValueError,
        match="Timestamp doesn't exist in " f"timezone '{timezone}'",
    ):
    ...
    with pytest.raises(
        ValueError,
        match="Timestamp is ambiguous in " f"timezone '{timezone}'",
    ):
...
def test_select_k_array():
    ...
    result = pc.select_k_unstable(
        arr,
        options=pc.SelectKOptions(k=2, sort_keys=[("dummy", "descending")]),
    )
    validate_select_k(result, arr, "descending")

    result = pc.select_k_unstable(
        arr,
        options=pc.SelectKOptions(k=2, sort_keys=[("dummy", "ascending")]),
    )
    validate_select_k(result, arr, "ascending")
...
def test_select_k_table():
    ...
    for k in [0, 2, 4]:
        ...
        result = pc.select_k_unstable(
            table,
            k=k,
            sort_keys=[(pc.field("a"), "ascending"), ("b", "ascending")],
        )
        validate_select_k(
            result,
            table,
            sort_keys=[("a", "ascending"), ("b", "ascending")],
        )
    ...
    with pytest.raises(
        ValueError,
        match="select_k_unstable requires a non-empty `sort_keys`",
    ):
...
def test_sort_indices_array():
    ...
    result = pc.sort_indices(
        arr,
        options=pc.SortOptions(sort_keys=[("dummy", "descending")]),
    )
    assert result.to_pylist() == [1, 0, 3, 2]
    result = pc.sort_indices(
        arr,
...
def test_sort_indices_table():
    ...
    result = pc.sort_indices(
        table,
        sort_keys=[(pc.field("a"), "ascending")],
        null_placement="at_start",
    )
...
def test_cumulative_max(start, skip_nulls):
    ...
    for strt in starts:
        ...
        for i, arr in enumerate(arrays):
            ...
            expected = pc.max_element_wise(
                expected_arrays[i],
                strt if strt is not None else -1e9,
                skip_nulls=False,
            )
            np.testing.assert_array_almost_equal(
                result.to_numpy(zero_copy_only=False),
...
def test_random():
    ...
    arrays = [tuple(pc.random(100, initializer=i % 7).to_pylist()) for i in range(100)]
...
def test_rank_quantile_options():
    ...
    result = pc.rank_quantile(
        arr,
        options=pc.RankQuantileOptions(sort_keys=[("b", "ascending")]),
    )
    assert result.equals(expected)
...
def test_expression_serialization_substrait():
    ...
    for expr, expr_norm in zip(exprs["refs"], exprs["numeric_refs"], strict=False):
        serialized = expr.to_substrait(schema)
        deserialized = pc.Expression.from_substrait(serialized)
...
def test_list_slice_output_fixed(start, stop, step, expected, value_type, list_type):
    if list_type == "fixed":
        arr = pa.array(
            [[1, 2, 3], [4, 5, None], [6, None, None], None],
            pa.list_(pa.int8(), 3),
...
def test_winsorize():
    ...
    result = pc.winsorize(
        arr,
        options=pc.WinsorizeOptions(lower_limit=0.1, upper_limit=0.8),
    )
    assert result.to_pylist() == [8, 4, 8, 8, 5, 3, 7, 2, 2, 6]
