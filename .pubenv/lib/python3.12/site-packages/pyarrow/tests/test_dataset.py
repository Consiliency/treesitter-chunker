...
def test_filesystem_dataset(mockfs):
    ...
    fragments = [
        ...
        for path, part in zip(paths, partitions, strict=False)
    ]
    ...
    for dataset in [dataset_from_fragments, dataset_from_paths]:
        ...
        for fragment, partition, path in zip(
            fragments,
            partitions,
            paths,
            strict=False,
...
def test_partitioning():
    ...
    partitioning = ds.DirectoryPartitioning(
        schema,
        dictionaries={"key": pa.array(["first", "second", "third"])},
    )
...
def test_parquet_scan_options():
    ...
    cache_opts = pa.CacheOptions(
        hole_size_limit=2**10,
        range_size_limit=8 * 2**10,
        lazy=True,
    )
...
def test_filesystem_factory(mockfs, paths_or_selector, pre_buffer):
    ...
    for (batch, fragment), group, key in zip(
        iterator,
        [1, 2],
        ["xxx", "yyy"],
        strict=False,
...
def test_make_fragment_with_size(s3_example_simple):
    ...
    fragments_with_size = [
        ...
        for path, size in zip(paths, sizes_true, strict=False)
    ]
    ...
    fragments_with_size = [
        file_format.make_fragment(path, fs, file_size=size)
        for path, size in zip(paths, sizes_toosmall, strict=False)
    ]
    ...
    fragments_with_size = [
        file_format.make_fragment(path, fs, file_size=size)
        for path, size in zip(paths, sizes_toolarge, strict=False)
    ]
...
def test_fragments_parquet_num_row_groups(tempdir):
    ...
    fragment = original_fragment.format.make_fragment(
        original_fragment.path,
        original_fragment.filesystem,
        row_groups=[1, 3],
    )
...
def test_fragments_parquet_subset_with_nested_fields(tempdir):
    ...
    with pytest.raises(
        NotImplementedError,
        match="Function 'greater' has no kernel matching",
    ):
...
def test_construct_dataset_with_invalid_schema():
    empty = ds.dataset(
        [],
        format="ipc",
        schema=pa.schema([("a", pa.int64()), ("a", pa.string())]),
    )
...
def _create_partitioned_dataset(basedir):
    ...
    full_table = table.append_column(
        "part",
        pa.array([0] * 3 + [1] * 3 + [2] * 3, type=pa.int32()),
    )
...
def test_open_dataset_partitioned_directory(tempdir, dataset_reader, pickle_module):
    ...
    expected = table.append_column(
        "part",
        pa.array([0] * 3 + [1] * 3 + [2] * 3, type=pa.int8()),
    )
    assert result.equals(expected)
...
def test_open_dataset_from_s3_with_filesystem_uri(s3_server):
    ...
    template = f"s3://{access_key}:{secret_key}@{{}}?scheme=http&endpoint_override={host}:{port}"
    cases = [
...
def test_specified_schema(tempdir, dataset_reader):
    ...
    expected = pa.table(
        [[1, 2, 3], pa.array([None, None, None], type="int32")],
        names=["a", "c"],
    )
    _check_dataset(schema, expected)
...
def test_csv_format_options(tempdir, dataset_reader):
    ...
    dataset = ds.dataset(
        path,
        format=ds.CsvFileFormat(read_options=pa.csv.ReadOptions(skip_rows=1)),
    )
    result = dataset_reader.to_table(dataset)
...
def test_json_format_options(tempdir, dataset_reader):
    ...
    dataset = ds.dataset(
        path,
        format=ds.JsonFileFormat(read_options=pa.json.ReadOptions(block_size=64)),
    )
    result = dataset_reader.to_table(dataset)
    assert result.equals(table)
...
def test_write_to_dataset_given_null_just_works(tempdir):
    ...
    table = pa.table(
        {"part": [None, None, "a", "a"], "col": list(range(4))},
        schema=schema,
    )
...
def _sort_table(tab, sort_col):
    ...
    sorted_indices = pc.sort_indices(
        tab,
        options=pc.SortOptions([(sort_col, "ascending")]),
    )
...
def test_write_dataset_max_open_files(tempdir):
    ...
    record_batch_1 = pa.record_batch(
        data=[[1, 2, 3, 4, 0, 10], ["a", "b", "c", "d", "e", "a"]],
        names=column_names,
    )
    record_batch_2 = pa.record_batch(
        data=[[5, 6, 7, 8, 0, 1], ["a", "b", "c", "d", "e", "c"]],
        names=column_names,
    )
    record_batch_3 = pa.record_batch(
        data=[[9, 10, 11, 12, 0, 1], ["a", "b", "c", "d", "e", "d"]],
        names=column_names,
    )
...
def test_write_scanner(tempdir, dataset_reader):
    ...
    ds.write_dataset(
        dataset_reader.scanner(dataset, columns=["f1"]),
        base_dir,
        format="feather",
    )
    result = dataset_reader.to_table(ds.dataset(base_dir, format="ipc"))
...
def test_write_table_partitioned_dict(tempdir):
    ...
    result = ds.dataset(
        base_dir,
        format="ipc",
        partitioning=partitioning_read,
    ).to_table()
    assert result.equals(table)
...
def test_write_dataset_s3(s3_example_simple):
    ...
    uri_template = f"s3://{access_key}:{secret_key}@{{}}?scheme=http&endpoint_override={host}:{port}"
...
def test_write_dataset_with_scanner_use_projected_schema(tempdir):
    ...
    with pytest.raises(
        KeyError,
        match=r"'Column original_column does not exist in schema",
    ):
        ds.write_dataset(
            scanner,
            tempdir,
            partitioning=["original_column"],
            format="ipc",
        )
