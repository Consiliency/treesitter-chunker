...
np_str_int_types, pa_int_types = zip(*int_type_pairs, strict=False)
...
def test_nested_arrays(seq, inner_seq, factory):
    arr = pa.array(
        seq([inner_seq([]), inner_seq([1, 2]), None]),
...
@pytest.mark.numpy()
@pytest.mark.xfail(
    reason="Type inference for uint64 not implemented",
    ...
    for inner_seq in (np.array, list):
        ...
        if from_pandas:
            ...
            np.testing.assert_equal(
                arr.to_pylist(),
                [[1.0, 2.0], [1.0, 2.0, 3.0], [np.nan], None],
            )
...
@pytest.mark.parametrize(
    "input",
    [(pa.date32(), [10957, None]), (pa.date64(), [10957 * 86400000, None])],
)
...
def test_sequence_timestamp_with_timezone_inference():
    ...
    for dt, expected_type in zip(data, expected, strict=False):
        prepended = [dt] + data
...
def test_sequence_timestamp_with_zoneinfo_timezone_inference():
    ...
    data = [
        datetime.datetime(2007, 7, 13, 8, 23, 34, 123456),  # naive
        datetime.datetime(2008, 1, 5, 5, 0, 0, 1000, tzinfo=datetime.UTC),
        None,
        datetime.datetime(
            2006,
            1,
