...
assert_schema_released = pytest.raises(
    ValueError,
    match="Cannot import released ArrowSchema",
)

assert_array_released = pytest.raises(
    ValueError,
    match="Cannot import released ArrowArray",
)

assert_stream_released = pytest.raises(
    ValueError,
    match="Cannot import released Arrow Stream",
)
...
def make_extension_storage_schema():
    ...
    return pa.schema(
        [("ext", ParamExtType(3).storage_type)],
...
@pytest.mark.parametrize(
    "reader_factory",
    [make_ipc_stream_reader, make_py_record_batch_reader],
)
...
def test_imported_batch_reader_error():
    ...
    with pytest.raises(
        OSError,
        match="Expected to be able to read 16 bytes for message body, got 8",
    ):
...
def test_roundtrip_reader_capsule(constructor):
    ...
    for batch, expected in zip(imported_batches, batches, strict=False):
    ...
    for batch, expected in zip(imported_reader, batches, strict=False):
        assert batch.equals(expected)
...
def test_roundtrip_chunked_array_capsule_requested_schema():
    ...
    with pytest.raises(
        ValueError,
        match="Could not cast string to requested type int64",
    ):
