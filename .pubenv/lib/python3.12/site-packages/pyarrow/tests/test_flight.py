...
class NeverSendsDataFlightServer(FlightServerBase):
    ...
    def do_get(self, context, ticket):
        ...
        return flight.GeneratorStream(
            self.schema,
            itertools.repeat(self.schema.empty_table()),
        )
...
class ErrorFlightServer(FlightServerBase):
    ...
    def do_put(self, context, descriptor, reader, writer):
        if descriptor.command == b"internal":
            raise flight.FlightInternalError("foo")
        if descriptor.command == b"timedout":
            raise flight.FlightTimedOutError("foo")
        if descriptor.command == b"cancel":
            raise flight.FlightCancelledError("foo")
        if descriptor.command == b"unauthenticated":
            raise flight.FlightUnauthenticatedError("foo")
        if descriptor.command == b"unauthorized":
            raise flight.FlightUnauthorizedError("foo")
        if descriptor.command == b"protobuf":
            err_msg = b"this is an error message"
            raise flight.FlightUnauthorizedError("foo", err_msg)
...
def test_flight_list_flights():
    ...
    with (
        ConstantFlightServer() as server,
...
def test_flight_do_get_ints():
    ...
    with (
        ConstantFlightServer(options=options) as server,
        flight.connect(("localhost", server.port)) as client,
    ):
        data = client.do_get(flight.Ticket(b"ints")).read_all()
        assert data.equals(table)
    ...
    with pytest.raises(
        ...
        with (
            ConstantFlightServer(options=42) as server,
            flight.connect(("localhost", server.port)) as client,
        ):
            data = client.do_get(flight.Ticket(b"ints")).read_all()
...
def test_flight_do_get_ticket():
    ...
    table = pa.Table.from_arrays(data1, names=["a"])
    with (
        CheckTicketFlightServer(expected_ticket=b"the-ticket") as server,
        flight.connect(("localhost", server.port)) as client,
    ):
...
def test_list_actions():
    ...
    # ARROW-6392
    with (
        ListActionsErrorFlightServer() as server,
        FlightClient(("localhost", server.port)) as client,
    ):
        with pytest.raises(
            flight.FlightServerError,
    ...
    with (
        ListActionsFlightServer() as server,
        FlightClient(("localhost", server.port)) as client,
    ):
...
class ConvenienceServer(FlightServerBase):
    ...
    def do_action(self, context, action):
        ...
        if action.type == "echo":
            return [action.body]
        if action.type == "bad-action":
            return ["foo"]
        if action.type == "arrow-exception":
            raise pa.ArrowMemoryError()
        if action.type == "forever":
...
def test_flight_large_message():
    ...
    with (
        EchoFlightServer(expected_schema=data.schema) as server,
        FlightClient(("localhost", server.port)) as client,
    ):
...
def test_flight_generator_stream():
    ...
    with (
        EchoStreamFlightServer() as server,
        FlightClient(("localhost", server.port)) as client,
    ):
        writer, _ = client.do_put(flight.FlightDescriptor.for_path("test"), data.schema)
        writer.write_table(data)
...
def test_flight_invalid_generator_stream():
    ...
    with (
        InvalidStreamFlightServer() as server,
        FlightClient(("localhost", server.port)) as client,
    ):
...
def test_http_basic_unauth():
    ...
    with (
        EchoStreamFlightServer(auth_handler=basic_auth_handler) as server,
        FlightClient(("localhost", server.port)) as client,
    ):
...
def test_token_auth():
    ...
    with (
        EchoStreamFlightServer(auth_handler=token_auth_handler) as server,
        FlightClient(("localhost", server.port)) as client,
    ):
        action = flight.Action("who-am-i", b"")
...
def test_tls_fails():
    ...
    with (
        ConstantFlightServer(tls_certificates=certs["certificates"]) as s,
...
def test_flight_do_get_metadata():
    ...
    batches = []
    with (
        MetadataFlightServer() as server,
        FlightClient(("localhost", server.port)) as client,
    ):
...
def test_flight_do_get_metadata_v4():
    ...
    options = pa.ipc.IpcWriteOptions(metadata_version=pa.ipc.MetadataVersion.V4)
    with (
        MetadataFlightServer(options=options) as server,
        FlightClient(("localhost", server.port)) as client,
    ):
        reader = client.do_get(flight.Ticket(b""))
        data = reader.read_all()
...
def test_flight_do_put_limit():
    ...
    with (
        EchoFlightServer() as server,
        FlightClient(("localhost", server.port), write_size_limit_bytes=4096) as client,
    ):
        writer, metadata_reader = client.do_put(
            flight.FlightDescriptor.for_path(""),
            large_batch.schema,
        ...
        with writer:
            with pytest.raises(
                flight.FlightWriteSizeExceededError,
                match="exceeded soft limit",
            ) as excinfo:
...
def test_middleware_reject():
    ...
    with (
        ...
        assert response.body.to_pybytes() == b"password"
...
def test_middleware_mapping():
    ...
    with (
        FlightServerBase(middleware={"test": server_middleware}) as server,
        FlightClient(
            ("localhost", server.port),
...
def test_doexchange_get():
    ...
    with (
        ExchangeFlightServer() as server,
        FlightClient(("localhost", server.port)) as client,
    ):
...
def test_doexchange_echo_v4():
    ...
    options = pa.ipc.IpcWriteOptions(metadata_version=pa.ipc.MetadataVersion.V4)
    with (
        ExchangeFlightServer(options=options) as server,
        FlightClient(("localhost", server.port)) as client,
    ):
        descriptor = flight.FlightDescriptor.for_command(b"echo")
        writer, reader = client.do_exchange(descriptor)
...
def test_interrupt():
    ...
    def test(read_all):
        ...
        assert isinstance(e, (pa.ArrowCancelled, KeyboardInterrupt)) or isinstance(
            e.__context__,
            (pa.ArrowCancelled, KeyboardInterrupt),
        )

    with (
        CancelFlightServer() as server,
        FlightClient(("localhost", server.port)) as client,
    ):
...
def test_never_sends_data():
    ...
    with (
        NeverSendsDataFlightServer() as server,
        flight.connect(("localhost", server.port)) as client,
    ):
...
def test_large_metadata_client():
    ...
    del metadata
    with (
        LargeMetadataFlightServer() as server,
        flight.connect(("localhost", server.port)) as client,
    ):
        with pytest.raises(
            flight.FlightServerError,
...
def test_none_action_side_effect():
    ...
    with (
        ActionNoneFlightServer() as server,
        FlightClient(("localhost", server.port)) as client,
    ):
...
class TracingFlightServer(FlightServerBase):
    ...
    def do_action(self, context, action):
        ...
        return ((f"{key}: {value}").encode() for (key, value) in trace_context.items())
    ...
    with (
        ...
        options = flight.FlightCallOptions(
            headers=[
                ...
                (
                    b"traceparent",
                    b"00-000ff00f00f0ff000f0f00ff0f00fff0-000f0000f0f00000-00",
                ),
                (b"tracestate", b""),
            ],
        )
...
def test_headers_trailers():
    ...
    with (
        HeadersTrailersFlightServer() as server,
