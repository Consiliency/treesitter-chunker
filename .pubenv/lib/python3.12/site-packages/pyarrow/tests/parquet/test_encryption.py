...
def test_encrypted_parquet_write_no_col_key(tempdir, data_table):
    ...
    with pytest.raises(
        OSError,
        match="Either column_keys or uniform_encryption must be set",
    ):
...
def test_encrypted_parquet_encryption_configuration():
    def validate_encryption_configuration(encryption_config):
        assert encryption_config.footer_key == FOOTER_KEY_NAME
        assert encryption_config.column_keys[COL_KEY_NAME] == ["a", "b"]
        assert encryption_config.encryption_algorithm == "AES_GCM_CTR_V1"
        ...
        assert encryption_config.data_key_length_bits == 192
...
def test_encrypted_parquet_kms_configuration():
    def validate_kms_connection_config(kms_connection_config):
        assert kms_connection_config.kms_instance_id == "Instance1"
        assert kms_connection_config.kms_instance_url == "URL1"
        assert kms_connection_config.key_access_token == "MyToken"
        assert kms_connection_config.custom_kms_conf == {
            "key1": "key_material_1",
            "key2": "key_material_2",
        }
