...
def _pymap(draw, key_type, value_type, size, nullable=True):
    ...
    return list(zip(keys, values, strict=False))
...
def arrays(draw, type, size=None, nullable=True):
    ...
    elif pa.types.is_duration(ty) or pa.types.is_interval(ty):
        value = st.timedeltas()
    ...
    elif pa.types.is_list_view(ty) or pa.types.is_large_list_view(ty):
        value = _pylist(ty.value_type, size=size, nullable=nullable)
