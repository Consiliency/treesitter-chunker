...
class IpcFixture:
    ...
    def __init__(self, sink_factory=io.BytesIO):
...
@pytest.mark.parametrize(
    "sink_factory",
    [io.BytesIO, pa.BufferOutputStream],
)
...
def test_message_read_record_batch(example_messages):
    ...
    for batch, message in zip(batches, messages[1:], strict=False):
...
def test_pandas_serialize_round_trip_not_string_columns():
    df = pd.DataFrame(list(zip([1.5, 1.6, 1.7], "abc", strict=False)))
...
def test_record_batch_reader_from_arrow_stream():
    ...
    with pytest.raises(pa.lib.ArrowTypeError, match="Field 0 cannot be cast"):
        pa.RecordBatchReader.from_stream(
            wrapper,
            schema=pa.schema([pa.field("a", pa.list_(pa.int32()))]),
        )
