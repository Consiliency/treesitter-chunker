...
def test_ext_type_as_py():
    ...
    # Works for __iter__
    for result, expected in zip(arr, uuids, strict=False):
    ...
    for result, expected in zip(carr, uuids + uuids, strict=False):
        assert result.as_py() == expected
...
def test_ext_scalar_from_array():
    ...
    for s, val in zip(scalars_a, data, strict=False):
    ...
    for sa, sb in zip(scalars_a, scalars_b, strict=False):
    ...
    for s, val in zip(scalars_c, data, strict=False):
        assert isinstance(s, pa.ExtensionScalar)
        assert s.is_valid == (val is not None)
        assert s.type == ty3
        if val is not None:
...
def test_ext_scalar_from_storage():
    ...
    s = pa.ExtensionScalar.from_storage(
        ty,
        pa.scalar(b"0123456789abcdef", ty.storage_type),
    )
    assert isinstance(s, pa.ExtensionScalar)
    assert s.type == ty
    assert s.is_valid is True
...
def test_cast_to_extension_with_nested_storage():
    ...
    expected = pa.ExtensionArray.from_storage(
        ext_type,
        array.cast(ext_type.storage_type),
    )
    assert result.equals(expected)
...
@pytest.mark.parametrize(
    "data,ty",
    (([1, 2, 3], IntegerType), (["cat", "dog", "horse"], LabelType)),
)
@pytest.mark.parametrize(
    "into",
    [
