...
try:
    ...
    raise ImportError(
        f"The pyarrow installation is not built with support for 'dataset' ({exc!s})",
    ) from None
...
RecordBatch or Table, iterable of RecordBatch, RecordBatchReader, or URI
    ...
    if isinstance(source, (tuple, list)):
        if all(_is_path_like(elem) or isinstance(elem, FileInfo) for elem in source):
            return _filesystem_dataset(source, **kwargs)
        if all(isinstance(elem, Dataset) for elem in source):
            return _union_dataset(source, **kwargs)
        if all(isinstance(elem, (pa.RecordBatch, pa.Table)) for elem in source):
    ...
    if isinstance(source, (pa.RecordBatch, pa.Table, pa.RecordBatchReader)):
        return _in_memory_dataset(source, **kwargs)
    raise TypeError(
