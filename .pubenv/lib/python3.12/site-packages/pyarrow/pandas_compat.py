...
from pyarrow.lib import _pandas_api, frombytes, is_threading_enabled
...
def get_logical_type(arrow_type):
    ...
    try:
        ...
        if isinstance(arrow_type, pa.lib.DictionaryType):
            return "categorical"
        if isinstance(arrow_type, pa.lib.ListType):
...
def construct_metadata(
    ...
    if preserve_index is not False:
        ...
        for level, name in zip(levels, names, strict=False):
...
def _get_columns_to_convert_given_schema(df, schema, preserve_index):
    ...
    for name in schema.names:
        try:
            ...
            if preserve_index is None and isinstance(col, _pandas_api.pd.RangeIndex):
                raise ValueError(
...
def dataframe_to_arrays(
    ...
    if nthreads == 1:
        arrays = [
            convert_column(c, f)
            for c, f in zip(columns_to_convert, convert_fields, strict=False)
        ]
    else:
        arrays = []
    ...
    if schema is None:
        fields = []
        for name, type_ in zip(all_names, types, strict=False):
...
def table_to_dataframe(
    ...
    blocks = [
        _reconstruct_block(item, column_names, ext_columns_dtypes) for item in result
    ]
    axes = [columns, index]
...
def _deserialize_column_index(block_table, all_columns, column_indexes):
    ...
    if len(column_indexes) > 1:
        ...
        columns = _pandas_api.pd.Index(
            columns_values,
            name=column_indexes[0]["name"] if column_indexes else None,
        )
