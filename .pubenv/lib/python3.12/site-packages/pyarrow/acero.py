...
try:
    ...
    raise ImportError(
        f"The pyarrow installation is not built with support for 'acero' ({exc!s})",
    ) from None
...
def _perform_join(
    ...
    if coalesce_keys and join_type == "full outer":
        ...
        projection = Declaration(
            "project",
            ProjectNodeOptions(projections, projected_col_names),
        )
    ...
    if output_type == Table:
        return result_table
    if output_type == ds.InMemoryDataset:
...
def _sort_source(table_or_dataset, sort_keys, output_type=Table, **kwargs):
    ...
    if isinstance(table_or_dataset, ds.Dataset):
        ...
        data_source = Declaration(
            "table_source",
            TableSourceNodeOptions(table_or_dataset),
        )
