...
class FixedType:
    ...
    def __repr__(self):
        return f"FixedType({self.size!r}, {self.struct_code!r})"
...
class StringType:
    ...
    def check_data(self, data):
        if not isinstance(data, str):
            raise TypeError(f"Expected str, not {data!r}")
    ...
    def __repr__(self):
        return f"StringType({self.length_type!r})"
...
class Struct:
    ...
    def serialise(self, data, pos, endianness, fds=None):
        if not isinstance(data, tuple):
            raise TypeError(f"Expected tuple, not {data!r}")
        if len(data) != len(self.fields):
            raise ValueError(
                f"{len(data)} entries for {len(self.fields)} fields",
            )
        pad = b"\0" * padding(pos, self.alignment)
        pos += len(pad)
        res_pieces = []
        for item, field in zip(data, self.fields, strict=False):
            res_pieces.append(field.serialise(item, pos, endianness, fds=fds))
            pos += len(res_pieces[-1])
        return pad + b"".join(res_pieces)

    def __repr__(self):
        return f"{type(self).__name__}({self.fields!r})"
...
class DictEntry(Struct):
    def __init__(self, fields):
        ...
        if not isinstance(fields[0], (FixedType, StringType)):
            raise TypeError(
                f"First field in dict entry must be simple type, not {type(fields[0])}",
            )
...
class Array:
    ...
    def serialise(self, data, pos, endianness, fds=None):
        ...
        if isinstance(self.elt_type, DictEntry) and isinstance(data, dict):
            ...
            raise TypeError(f"Not suitable for array: {data!r}")
    ...
    def __repr__(self):
        return f"Array({self.elt_type!r})"
...
class Header:
    ...
    def __repr__(self):
        return f"Header({self.endianness!r}, {self.message_type!r}, {self.flags!r}, {self.protocol_version!r}, {self.body_length!r}, {self.serial!r}, fields={self.fields!r})"
...
class Message:
    ...
    def __repr__(self):
        return f"{type(self).__name__}({self.header!r}, {self.body!r})"
