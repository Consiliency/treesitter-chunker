...
class Method:
    ...
    def make_code(self):
        ...
        return (
            f"def {self.name}(self, {args}):\n"
            f"    return new_method_call(self, '{self.name}', '{signature}',\n"
            f"                           {tuple})\n"
        )
...
def generate_from_introspection(path, name, output_file, bus="SESSION"):
    ...
    print(f"Written {n_interfaces} interface wrappers to {output_file}")
