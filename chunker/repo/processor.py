...
from ..interfaces.repo import (
    ...
    GitAwareProcessor,
    RepoChunkResult,
    RepoProcessor as RepoProcessorInterface,
)
...
class RepoProcessor(RepoProcessorInterface):
    ...
    def process_repository(self, repo_path: str, incremental: bool = True,
                         ...
                         exclude_patterns: Optional[List[str]] = None) -> RepoChunkResult:
        """
    ...
    def process_files_iterator(self, repo_path: str, 
                             file_pattern: Optional[str] = None,
                             exclude_patterns: Optional[List[str]] = None) -> Iterator[FileChunkResult]:
        """
    ...
    def get_processable_files(self, repo_path: str,
                            file_pattern: Optional[str] = None,
        ...
        if self.traversal_strategy == "breadth-first":
            ...
            while dirs_to_process:
                ...
                try:
                    for item in current_dir.iterdir():
                        if item.is_dir():
                            if not exclude_spec.match_file(str(item.relative_to(repo_path))):
            ...
            for root, dirs, filenames in os.walk(repo_path):
                ...
                dirs[:] = [d for d in dirs 
                          if not exclude_spec.match_file(str(rel_root / d))]
    ...
    def _should_process_file(self, file_path: Path, file_pattern: Optional[str]) -> bool:
    ...
    def _process_single_file(self, file_path: Path, repo_path: Path) -> Optional[FileChunkResult]:
...
class GitAwareRepoProcessor(RepoProcessor, GitAwareProcessor):
    ...
    def get_changed_files(self, repo_path: str, since_commit: Optional[str] = None,
    ...
    def get_file_history(self, file_path: str, repo_path: str, 
    ...
    def load_incremental_state(self, repo_path: str) -> Optional[Dict[str, Any]]:
        """
