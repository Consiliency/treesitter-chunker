...
from .vfs import (
    VirtualFileSystem, LocalFileSystem, InMemoryFileSystem, 
    ZipFileSystem, HTTPFileSystem, CompositeFileSystem, create_vfs
)
from .chunker import chunk_text
from .types import CodeChunk
from .gc_tuning import optimized_gc, get_memory_optimizer
...
class VFSChunker:
    ...
    def chunk_file(self, 
                   ...
                   streaming: bool = False) -> Union[List[CodeChunk], Iterator[CodeChunk]]:
    ...
    def _chunk_file_streaming(self, 
                             path: str, 
                             language: str, 
                             chunker: StreamingChunker) -> Iterator[CodeChunk]:
    ...
    def chunk_directory(self,
                       ...
                       streaming: bool = False) -> Iterator[tuple[str, List[CodeChunk]]]:
        ...
        for vf in self._walk_directory(directory, recursive):
            ...
            # Check file patterns
            if file_patterns:
                if not any(self._match_pattern(vf.path, pattern) 
...
def chunk_file_from_vfs(path: str, 
...
def chunk_from_zip(zip_path: str,
                  file_path: str,
                  language: Optional[str] = None,
                  streaming: bool = False) -> Union[List[CodeChunk], Iterator[CodeChunk]]:
