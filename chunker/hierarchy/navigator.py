...
class HierarchyNavigator(HierarchyNavigatorInterface):
    ...
    def get_siblings(self, chunk_id: str, hierarchy: ChunkHierarchy) -> List[CodeChunk]:
        ...
        if chunk_id not in hierarchy.parent_map:
            siblings = [hierarchy.chunk_map[root_id] for root_id in hierarchy.root_chunks 
        ...
        siblings = [hierarchy.chunk_map[sibling_id] for sibling_id in sibling_ids 
    ...
    def get_subtree(self, chunk_id: str, hierarchy: ChunkHierarchy) -> ChunkHierarchy:
        ...
        for cid in subtree_chunks:
            ...
            if cid in hierarchy.children_map:
                children_in_subtree = [child_id for child_id in hierarchy.children_map[cid]
    ...
    def get_level_order_traversal(self, hierarchy: ChunkHierarchy) -> List[List[CodeChunk]]:
        """
        ...
        current_level = [hierarchy.chunk_map[root_id] for root_id in hierarchy.root_chunks
    ...
    def find_chunks_by_type(self, node_type: str, hierarchy: ChunkHierarchy, 
                           subtree_root: Optional[str] = None) -> List[CodeChunk]:
        """
        ...
        matching_chunks = [chunk for chunk in search_map.values() 
                         if chunk.node_type == node_type]
