...
from ..analysis import SemanticAnalyzer, ComplexityAnalyzer, CouplingAnalyzer
...
class SemanticChunker(ChunkingStrategy):
    ...
    def __init__(self):
        ...
        self.semantic_boundaries = {
            ...
            # Semantic groupings
            'namespace_definition', 'impl_item',
            'export_statement', 'import_statement',
            
            # Test boundaries
            'test_definition', 'test_suite', 'describe_block',
        }
    ...
    def chunk(self, ast: Node, source: bytes, file_path: str, language: str) -> List[CodeChunk]:
        ...
        for node, metadata in semantic_nodes:
            chunk = self._create_semantic_chunk(node, source, file_path, language, metadata)
            if chunk:
                chunks.append(chunk)
        ...
        if self.config['split_complex']:
            chunks = self._split_complex_chunks(chunks, ast, source, file_path, language)
    ...
    def _identify_semantic_nodes(self, ast: Node, source: bytes) -> List[Tuple[Node, Dict[str, Any]]]:
        ...
        def traverse(node: Node, depth: int = 0, parent_semantic: bool = False):
            ...
            complexity_info = self.complexity_analyzer.calculate_complexity(node, source)
            
            # Check if this node forms a semantic boundary
            is_boundary = (
                node.type in self.semantic_boundaries or
                complexity_info['score'] > self.config['complexity_threshold'] or
                semantic_info['cohesion_score'] > self.config['cohesion_threshold']
            )
    ...
    def _create_semantic_chunk(self, node: Node, source: bytes, file_path: str, 
                              language: str, metadata: Dict[str, Any]) -> CodeChunk:
    ...
    def _merge_related_chunks(self, chunks: List[CodeChunk], source: bytes) -> List[CodeChunk]:
        ...
        for i, chunk1 in enumerate(chunks):
            ...
            if merge_candidates:
                merged_chunk = self._merge_chunks([chunk1] + [c for _, c in merge_candidates])
    ...
    def _split_complex_chunks(self, chunks: List[CodeChunk], ast: Node, source: bytes,
        ...
        for chunk in chunks:
            ...
            should_split = (
                complexity > self.config['complexity_threshold'] * 2 or
                line_count > self.config['max_chunk_size']
            )
            
            if should_split:
                # Find the node corresponding to this chunk
                chunk_node = self._find_node_at_position(ast, chunk.byte_start, chunk.byte_end)
                if chunk_node:
                    sub_chunks = self._split_node(chunk_node, source, file_path, language, chunk)
    ...
    def _split_node(self, node: Node, source: bytes, file_path: str, 
        ...
        for i, (start_node, end_node) in enumerate(split_points):
            ...
            # Analyze sub-chunk
            sub_metadata = {
                'semantic': self.semantic_analyzer.analyze_semantics(start_node, source),
                'complexity': self.complexity_analyzer.calculate_complexity(start_node, source),
            }
    ...
    def _find_split_points(self, node: Node, source: bytes) -> List[Tuple[Node, Node]]:
        ...
        natural_boundaries = {
            ...
            'try_statement', 'switch_statement', 'case_clause',
        ...
        if boundary_nodes:
            ...
            for i in range(1, len(boundary_nodes)):
                ...
                lines_in_current = current_end.end_point[0] - current_start.start_point[0] + 1
    ...
    def _find_node_at_position(self, root: Node, start_byte: int, end_byte: int) -> Node:
    ...
    def _merge_chunks(self, chunks: List[CodeChunk]) -> CodeChunk:
        ...
        for chunk in chunks:
            if hasattr(chunk, 'metadata'):
                ...
                # Sum complexity
                merged_metadata['complexity']['score'] += \
                    chunk.metadata.get('complexity', {}).get('score', 0)
