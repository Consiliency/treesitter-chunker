...
class CompositeChunker(ChunkingStrategy):
    ...
    def _fusion_intersection(
        ...
        for strategy_chunks in position_map.values():
            if len(strategy_chunks) >= min_strategies:
                ...
                best_chunk.metadata["agreement_score"] = len(strategy_chunks) / len(
                    strategy_results,
                )
    ...
    def _is_better_chunk(self, chunk1: CodeChunk, chunk2: CodeChunk) -> bool:
        ...
        metadata1 = (
            len(chunk1.metadata)
            if hasattr(chunk1, "metadata") and chunk1.metadata
            else 0
        )
        metadata2 = (
            len(chunk2.metadata)
            if hasattr(chunk2, "metadata") and chunk2.metadata
            else 0
        )
