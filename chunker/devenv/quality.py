...
class QualityAssurance(QualityAssuranceContract):
    ...
    def check_type_coverage(self, min_coverage: float = 80.0) -> tuple[float,
        dict[str, Any]]:
        """
        ...
        try:
            cmd = [self._mypy_path, "chunker", "--html-report",
                ".mypy_coverage", "--any-exprs-report", ".mypy_coverage",
                "--linecount-report", ".mypy_coverage",
                "--linecoverage-report", ".mypy_coverage", "--no-error-summary",
                ]
            ...
            if linecount_file.exists():
                ...
                report = {"coverage_percentage": coverage_percentage,
                    ...
                    "total_lines": total_lines, "typed_lines": typed_lines,
                    "untyped_lines": total_lines - typed_lines, "files":
                ...
                for line in f:
                    ...
                    if line and not line.startswith("Total"):
                        ...
                        if len(parts) >= 4:
                            ...
                            coverage_data["files"][filename] = {"total_lines":
                                total, "typed_lines": typed, "coverage":
                                typed / total * 100 if total > 0 else 0}
    ...
    @staticmethod
    def _estimate_type_coverage(mypy_output: str) -> tuple[float, dict[str, Any],
        ]:
        ...
        report = {"coverage_percentage": coverage_percentage,
            "meets_minimum": coverage_percentage >= 80.0, "error_count":
            error_count, "files": {filename: {"errors": count,
            "estimated_coverage": max(0, 100 - count * 10)} for filename,
            count in file_errors.items()}}
        return coverage_percentage, report

    def check_test_coverage(self, min_coverage: float = 80.0) -> tuple[float,
        dict[str, Any]]:
        """
        ...
        try:
            cmd = [self._pytest_path, "--cov=chunker", "--cov-report=json",
            ...
            if coverage_json.exists():
                ...
                report = {"coverage_percentage": coverage_percentage,
                    ...
                    "total_lines": totals.get("num_statements", 0), "files":
                    {}, "uncovered_lines": {}}
                files_data = coverage_data.get("files", {})
                for filename, file_info in files_data.items():
                    if filename.startswith("chunker/"):
                        summary = file_info.get("summary", {})
                        report["files"][filename] = {"coverage": summary.
                            get("percent_covered", 0), "missing_lines":
    ...
    @staticmethod
    def _parse_coverage_text(coverage_output: str) -> tuple[float, dict[str,
        Any]]:
            ...
            if "TOTAL" in line:
                ...
                if len(parts) >= 4:
                    ...
                    try:
                        ...
                        pass
        report = {"coverage_percentage": coverage_percentage,
            "meets_minimum": coverage_percentage >= 80.0, "files": {
            filename: {"coverage": cov} for filename, cov in file_coverage.
            items()}, "uncovered_lines": {}}
        return coverage_percentage, report
