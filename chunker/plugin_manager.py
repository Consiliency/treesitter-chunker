...
class PluginRegistry:
    ...
    def register(self, plugin_class: Type[LanguagePlugin]) -> None:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to instantiate plugin {plugin_class.__name__}: {e}")
    ...
    def get_plugin(self, language: str, config: Optional[PluginConfig] = None) -> LanguagePlugin:
...
class PluginManager:
    ...
    def discover_plugins(self, directory: Optional[Path] = None) -> List[Type[LanguagePlugin]]:
    ...
    def _detect_h_file_language(self, file_path: Path) -> Optional[str]:
        ...
        if not language:
            ...
            if file_path.suffix.lower() == '.h':
                ...
                if detected_lang:
                    ...
                    if language:
                        logger.info(f"Could not detect language for {file_path}, defaulting to {language}")
