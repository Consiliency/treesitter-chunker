...
class SmartGrammarManager:
    ...
    def diagnose_grammar_issues(self, language: str) -> GrammarHealth:
                ...
                if so_file.stat().st_size == 0:
                    ...
                    try:
                        ...
                        if hasattr(lib, symbol_name):
                            ...
                            health.status = "corrupted"
                            health.issues.append(f"Missing expected symbol: {symbol_name}")
                            health.recommendations.append("Recompile the grammar from source")
        ...
        if not source_dir.exists():
            ...
            health.recommendations.append("Clone the grammar repository to grammars/ directory")
        else:
            ...
            try:
                ...
                if result.returncode == 0:
                    health.recommendations.append("Grammar source repository is available")
                else:
                ...
                health.issues.append("Could not verify grammar source repository status")
    ...
    def get_grammar_compatibility(self, language: str) -> GrammarCompatibility:
        ...
        try:
            import tree_sitter
            compatibility.tree_sitter_version = getattr(tree_sitter, '__version__', 'unknown')
        except Exception:
            pass
