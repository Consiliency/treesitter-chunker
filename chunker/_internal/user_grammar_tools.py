...
from .grammar_management import SmartGrammarManager, GrammarHealth, GrammarCompatibility
...
class UserGrammarTools:
    ...
    def install_grammar(self, language: str, repo_url: str, 
        ...
        try:
            ...
            if package_json.exists():
                try:
                    ...
                    result["warnings"].append(f"Failed to install npm dependencies: {e}")
            ...
            if so_files:
                for so_file in so_files:
                    ...
                    result["steps_completed"].append(f"Copied {so_file.name} to build directory")
            else:
            ...
            if health.status == "healthy":
                ...
                result["warnings"].append(f"Grammar installed but has issues: {health.status}")
    ...
    def get_grammar_info(self, language: str) -> Dict[str, Any]:
        ...
        if source_dir.exists():
            try:
                ...
                result["source_info"] = {
                    "repository_url": git_remote.stdout.strip() if git_remote.returncode == 0 else "unknown",
                    "latest_commit": git_log.stdout.strip() if git_log.returncode == 0 else "unknown",
                    "source_directory": str(source_dir),
                    "has_package_json": (source_dir / "package.json").exists(),
                    "has_grammar_js": (source_dir / "grammar.js").exists()
                }
            except Exception:
                result["source_info"] = {"error": "Could not retrieve source information"}
