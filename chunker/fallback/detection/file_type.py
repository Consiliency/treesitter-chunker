...
class EncodingDetector:
    ...
    @staticmethod
    def detect_encoding(file_path: str, sample_size: int = 8192) -> tuple[str,
        float]:
        """Detect file encoding.
    ...
    @staticmethod
    def read_with_encoding(file_path: str, encoding: (str | None) = None,
        ) -> tuple[str, str]:
        ...
        try:
            ...
            try:
                with Path(file_path).open(encoding=encoding, errors="replace",
                    ) as f:
                    content = f.read()
...
class FileTypeDetector(FallbackStrategy):
    ...
    def __init__(self):
        ...
        self.extension_map = {".txt": FileType.TEXT, ".text": FileType.TEXT,
            ".log": FileType.LOG, ".logs": FileType.LOG, ".out": FileType.
            LOG, ".err": FileType.LOG, ".md": FileType.MARKDOWN,
            ".markdown": FileType.MARKDOWN, ".mdown": FileType.MARKDOWN,
            ".mkd": FileType.MARKDOWN, ".csv": FileType.CSV, ".tsv":
            FileType.CSV, ".json": FileType.JSON, ".jsonl": FileType.JSON,
            ".xml": FileType.XML, ".yaml": FileType.YAML, ".yml": FileType.
            YAML, ".ini": FileType.CONFIG, ".cfg": FileType.CONFIG, ".conf":
            FileType.CONFIG, ".config": FileType.CONFIG, ".properties":
            FileType.CONFIG, ".toml": FileType.CONFIG}
        self.content_patterns = {FileType.LOG: ["^\\d{4}-\\d{2}-\\d{2}",
            "^\\[\\d{4}-\\d{2}-\\d{2}", "^\\w+ \\d+ \\d{2}:\\d{2}:\\d{2}",
            "\\b(ERROR|WARN|INFO|DEBUG|TRACE)\\b"], FileType.MARKDOWN: [
            "^#{1,6} ", "^\\* ", "^\\d+\\. ", "\\[.*\\]\\(.*\\)", "^```"]}
    ...
    def should_use_fallback(self, file_path: str, language: (str | None) = None,
        ...
        grammar_supported = {FileType.JSON: True, FileType.YAML: True,
            FileType.MARKDOWN: True}
        if file_type in grammar_supported and not language:
            return True, FallbackReason.NO_GRAMMAR
        if file_type in {FileType.TEXT, FileType.LOG, FileType.CSV,
            FileType.CONFIG}:
            return True, FallbackReason.NO_GRAMMAR
    ...
    def suggest_grammar(self, file_path: str) -> (str | None):
        ...
        """
        file_type = self.detect_file_type(file_path)
        suggestions = {FileType.JSON:
            "https://github.com/tree-sitter/tree-sitter-json", FileType.
            YAML:
    ...
    def get_metadata(self, file_path: str) -> dict[str, Any]:
        ...
        Returns:
            Metadata dictionary
        """
        metadata = {"file_type": self.detect_file_type(file_path).value,
            "size": 0, "encoding": "unknown", "mime_type": None}
        try:
    ...
    def _is_binary(cls, file_path: str, sample_size: int = 8192) -> bool:
        ...
        try:
            ...
            if b"\x00" in chunk:
                return True
            text_chars = bytearray({7, 8, 9, 10, 12, 13, 27} | set(range(32,
                256)))
