...
class OverlappingFallbackChunker(IOverlappingFallbackChunker):
    ...
    def _find_natural_line_boundary(self, lines: List[str], desired_line: int,
    ...
    def _calculate_dynamic_overlap_lines(self, lines: List[str], position: int,
        ...
        avg_line_length = sum(len(line) for line in recent_lines) / len(recent_lines) if recent_lines else 0
    ...
    def _calculate_dynamic_overlap_chars(self, content: str, position: int,
                                       min_overlap: int, max_overlap: int) -> int:
        ...
        structure_score = (paragraph_breaks * 2 + sentence_ends) / (len(recent_content) / 100.0) if recent_content else 1.0
