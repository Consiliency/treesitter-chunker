...
class OverlappingFallbackChunker(IOverlappingFallbackChunker):
    ...
    def chunk_with_overlap(self, content: str, file_path: str, chunk_size:
        ...
        if unit == "lines":
            return self._chunk_by_lines_with_overlap(content, chunk_size,
                overlap_size, strategy)
        return self._chunk_by_chars_with_overlap(content, chunk_size,
            overlap_size, strategy)
    ...
    def chunk_with_asymmetric_overlap(self, content: str, file_path: str,
        ...
        return self._chunk_by_chars_asymmetric(content, chunk_size,
            overlap_before, overlap_after)
    ...
    def chunk_with_dynamic_overlap(self, content: str, file_path: str,
        ...
        return self._chunk_by_chars_dynamic(content, chunk_size,
            min_overlap, max_overlap)
    ...
    def _chunk_by_lines_with_overlap(self, content: str, chunk_size: int,
        overlap_size: int, strategy: OverlapStrategy) -> list[CodeChunk]:
        ...
        while i < len(lines):
            ...
            if strategy == OverlapStrategy.DYNAMIC and i > 0:
                ...
                start_idx = self._find_natural_line_boundary(lines,
                    desired_line, overlap_size // 2)
            chunk_lines = lines[start_idx:end_idx]
            chunk_content = "".join(chunk_lines)
            byte_start = sum(len(line) for line in lines[:start_idx])
            byte_end = byte_start + len(chunk_content)
            chunk = CodeChunk(language=self._detect_language(), file_path=self.file_path or "", node_type="fallback_overlap_lines",
                start_line=start_idx + 1, end_line=end_idx, byte_start=byte_start, byte_end=byte_end, parent_context=f"overlap_chunk_{chunk_num}", content=chunk_content)
            chunks.append(chunk)
            i += chunk_size
            chunk_num += 1
        return chunks

    def _chunk_by_chars_with_overlap(self, content: str, chunk_size: int,
        overlap_size: int, strategy: OverlapStrategy) -> list[CodeChunk]:
        ...
        while i < len(content):
            ...
            if strategy == OverlapStrategy.DYNAMIC:
                if i > 0:
                    start = self.find_natural_overlap_boundary(content,
                        start, overlap_size // 2)
                if end < len(content):
                    end = self.find_natural_overlap_boundary(content, end,
                        overlap_size // 2)
            chunk_content = content[start:end]
            start_line = content[:start].count("\n") + 1
            end_line = content[:end].count("\n") + 1
            chunk = CodeChunk(language=self._detect_language(), file_path=self.file_path or "", node_type="fallback_overlap_chars",
                start_line=start_line, end_line=end_line, byte_start=start,
                byte_end=end, parent_context=f"overlap_chunk_{chunk_num}",
    ...
    def _chunk_by_lines_asymmetric(self, content: str, chunk_size: int,
        ...
        while i < len(lines):
            ...
            chunk = CodeChunk(language=self._detect_language(), file_path=self.file_path or "", node_type="fallback_asymmetric_lines",
                start_line=start_idx + 1, end_line=end_idx, byte_start=byte_start, byte_end=byte_end, parent_context=f"asymmetric_chunk_{chunk_num}", content=chunk_content)
            chunks.append(chunk)
            i += chunk_size
            chunk_num += 1
        return chunks

    def _chunk_by_chars_asymmetric(self, content: str, chunk_size: int,
        overlap_before: int, overlap_after: int) -> list[CodeChunk]:
        ...
        while i < len(content):
            ...
            chunk = CodeChunk(language=self._detect_language(), file_path=self.file_path or "", node_type="fallback_asymmetric_chars",
                start_line=start_line, end_line=end_line, byte_start=start,
                byte_end=end, parent_context=f"asymmetric_chunk_{chunk_num}", content=chunk_content)
            chunks.append(chunk)
            i += chunk_size
            chunk_num += 1
        return chunks

    def _chunk_by_lines_dynamic(self, content: str, chunk_size: int,
        ...
        while i < len(lines):
            if i == 0:
                ...
                overlap = self._calculate_dynamic_overlap_lines(lines, i,
                    min_overlap, max_overlap)
            ...
            chunk = CodeChunk(language=self._detect_language(), file_path=self.file_path or "", node_type="fallback_dynamic_lines",
                start_line=start_idx + 1, end_line=end_idx, byte_start=byte_start, byte_end=byte_end, parent_context=f"dynamic_chunk_{chunk_num}", content=chunk_content)
            chunks.append(chunk)
            i += chunk_size
            chunk_num += 1
        return chunks

    def _chunk_by_chars_dynamic(self, content: str, chunk_size: int,
        min_overlap: int, max_overlap: int) -> list[CodeChunk]:
        ...
        while i < len(content):
            if i == 0:
                ...
                overlap = self._calculate_dynamic_overlap_chars(content, i,
                    min_overlap, max_overlap)
                desired_start = i - overlap
                start = self.find_natural_overlap_boundary(content,
                    desired_start, overlap // 2)
                start = max(0, start)
            end = min(i + chunk_size, len(content))
            chunk_content = content[start:end]
            start_line = content[:start].count("\n") + 1
            ...
            chunk = CodeChunk(language=self._detect_language(), file_path=self.file_path or "", node_type="fallback_dynamic_chars",
                start_line=start_line, end_line=end_line, byte_start=start,
                byte_end=end, parent_context=f"dynamic_chunk_{chunk_num}",
                content=chunk_content)
            chunks.append(chunk)
            i += chunk_size
            chunk_num += 1
    ...
    @staticmethod
    def _find_natural_line_boundary(lines: list[str], desired_line: int,
        search_window: int) -> int:
    ...
    @staticmethod
    def _calculate_dynamic_overlap_lines(lines: list[str], position: int,
        min_overlap: int, max_overlap: int) -> int:
        ...
        density_ratio = 1.0 - empty_lines / len(recent_lines,
            ) if recent_lines else 0.5
        overlap = int(min_overlap + (max_overlap - min_overlap) * density_ratio,
            )
        return overlap

    @staticmethod
    def _calculate_dynamic_overlap_chars(content: str, position: int,
        min_overlap: int, max_overlap: int) -> int:
        ...
        structure_score = (paragraph_breaks * 2 + sentence_ends) / (len(
            recent_content) / 100.0) if recent_content else 1.0
