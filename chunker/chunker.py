...
def chunk_text_with_token_limit(text: str, language: str, max_tokens: int, 
                               file_path: str = "", model: str = "gpt-4",
                               extract_metadata: bool = True) -> list[CodeChunk]:
...
def chunk_file_with_token_limit(path: str | Path, language: str, max_tokens: int,
                               model: str = "gpt-4", extract_metadata: bool = True) -> list[CodeChunk]:
    ...
    """
    src = Path(path).read_text()
    return chunk_text_with_token_limit(src, language, max_tokens, str(path), model, extract_metadata)
