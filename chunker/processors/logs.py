...
class LogProcessor(SpecializedProcessor):
    ...
    def _validate_config(self) -> None:
        ...
        if self.chunk_by not in valid_chunk_by:
            raise ValueError(f"Invalid chunk_by: {self.chunk_by}. Must be one of {valid_chunk_by}")
        ...
        for format_name, pattern in self.patterns.items():
            ...
            if match:
                ...
                if 'timestamp' in groups:
                    entry.timestamp = self._parse_timestamp(groups['timestamp'], format_name)
    ...
    def _detect_log_level(self, line: str) -> Optional[str]:
    ...
    def _parse_timestamp(self, timestamp_str: str, format_name: str) -> Optional[datetime]:
        ...
        try:
            ...
            elif format_name in ('iso_timestamp', 'log4j'):
    ...
    def _group_error_contexts(self, chunks: List[TextChunk], all_entries: List[LogEntry]) -> List[TextChunk]:
    ...
    def _create_chunk(self, entries: List[LogEntry]) -> TextChunk:
        ...
        metadata = {
            ...
            'formats': list({e.metadata.get('format') for e in entries if e.metadata.get('format')}),
    ...
    def _check_emit_chunk(self) -> Optional[TextChunk]:
    ...
    def _create_chunk_from_buffer(self) -> Optional[TextChunk]:
