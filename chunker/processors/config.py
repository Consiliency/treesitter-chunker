...
class ConfigProcessor(SpecializedProcessor):
    ...
    def can_handle(self, file_path: str, content: Optional[str] = None) -> bool:
        ...
        # Check by extension
        if path.suffix.lower() in ['.ini', '.cfg', '.conf', '.toml', '.yaml', '.yml', '.json']:
            return True
    ...
    def _chunk_ini(self, content: str, structure: Dict[str, Any], 
                   file_path: str) -> List[CodeChunk]:
        ...
        if global_section['keys'] or global_section['end'] >= global_section['start']:
            global_content = '\n'.join(lines[global_section['start']:global_section['end'] + 1])
        ...
        for section_name, section_info in sections_to_process:
            if section_name in processed_sections:
                continue
            section_content = '\n'.join(lines[section_info['start']:section_info['end'] + 1])
            ...
            if self.config.group_related and len(section_content.split('\n')) < 10:
                ...
                available_sections = {k: v for k, v in structure['sections'].items() 
            ...
            chunks.append(CodeChunk(
                ...
                file_path=file_path,
                language='ini',
                byte_start=sum(len(l.encode()) + 1 for l in lines[:section_info['start']]),
                byte_end=sum(len(l.encode()) + 1 for l in lines[:section_info['end']+1]),
                metadata={
    ...
    def _chunk_toml(self, content: str, structure: Dict[str, Any], 
                    file_path: str) -> List[CodeChunk]:
        ...
        for table_name, table_info in structure['tables'].items():
            table_content = '\n'.join(lines[table_info['start']:table_info['end'] + 1])
            
            chunks.append(CodeChunk(
                content=table_content,
                start_line=table_info['start'] + 1,
                end_line=table_info['end'] + 1,
                node_type='toml_table' if not table_info['is_array'] else 'toml_array_table',
                parent_context=f"[{table_name}]" if not table_info['is_array'] else f"[[{table_name}]]",
                file_path=file_path,
                language='toml',
                byte_start=sum(len(l.encode()) + 1 for l in lines[:table_info['start']]),
                byte_end=sum(len(l.encode()) + 1 for l in lines[:table_info['end']+1]),
                metadata={
                    'table': table_name,
                    'is_array': table_info['is_array'],
                    'format': 'toml',
                    'name': f"[{table_name}]" if not table_info['is_array'] else f"[[{table_name}]]"
                }
            ))
    ...
    def _find_related_sections(self, section_name: str, 
                              all_sections: Dict[str, Any]) -> List[str]:
        ...
        if base_without_number != base_name:
            for other in all_sections:
                if other != section_name and other.lower().startswith(base_without_number):
