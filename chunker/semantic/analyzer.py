...
class TreeSitterRelationshipAnalyzer(RelationshipAnalyzer):
    ...
    def find_overloaded_functions(self, chunks: List[CodeChunk]) -> List[List[CodeChunk]]:
        ...
        for chunk in chunks:
            if chunk.node_type in ["function_definition", "method_definition", "constructor"]:
    ...
    def _extract_method_name(self, chunk: CodeChunk) -> Optional[str]:
        ...
        patterns = {
            ...
            "go": re.compile(r"func\s+(?:\([^)]+\)\s+)?(\w+)\s*\("),  # Handles receivers
        }
    ...
    def _extract_function_base_name(self, chunk: CodeChunk) -> Optional[str]:
    ...
    def _extract_class_name(self, chunk: CodeChunk) -> Optional[str]:
    ...
    def _extract_class_name_from_context(self, chunk: CodeChunk) -> Optional[str]:
    ...
    def _match_getter_pattern(self, method_name: str, language: str) -> Optional[re.Match]:
    ...
    def _match_setter_pattern(self, method_name: str, language: str) -> Optional[re.Match]:
    ...
    def _are_overloaded(self, chunk1: CodeChunk, chunk2: CodeChunk) -> bool:
        ...
        if chunk1.node_type not in ["function_definition", "method_definition", "constructor"]:
            return False
        
        if chunk2.node_type not in ["function_definition", "method_definition", "constructor"]:
            return False
    ...
    def _are_in_same_class(self, chunk1: CodeChunk, chunk2: CodeChunk) -> bool:
        ...
        return (chunk1.file_path == chunk2.file_path and 
                chunk1.parent_context == chunk2.parent_context and
                "class" in chunk1.parent_context)
    ...
    def _extract_implemented_interfaces(self, chunk: CodeChunk) -> List[str]:
        ...
        if chunk.language == "java":
            ...
            if match:
                ...
                interfaces = [p for p in parents if any(
