...
from .exceptions import ParserError, ParserInitError, ParserConfigError, LanguageNotFoundError
...
class ParserConfig:
    ...
    included_ranges: Optional[List[Range]] = None
...
class LRUCache:
    ...
    def get(self, key: str) -> Optional[Parser]:
...
class ParserPool:
    ...
    def get(self, timeout: Optional[float] = None) -> Optional[Parser]:
...
class ParserFactory:
    ...
    def __init__(self, registry: LanguageRegistry, cache_size: int = 10, pool_size: int = 5):
        ...
        logger.info(f"Initialized ParserFactory with cache_size={cache_size}, pool_size={pool_size}")
    ...
    def _create_parser(self, language: str) -> Parser:
        ...
        try:
            ...
            logger.debug(f"Created new parser for '{language}' (total: {self._parser_count})")
            ...
            if "Incompatible Language version" in str(e):
                ...
                import re
                match = re.search(r'version (\d+)\. Must be between (\d+) and (\d+)', str(e))
                if match:
    ...
    def get_parser(self, language: str, config: Optional[ParserConfig] = None) -> Parser:
