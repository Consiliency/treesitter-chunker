...
from ..chunker import chunk_file as chunk_file_original, _walk
...
class EnhancedChunker:
    ...
    def __init__(self,
        ...
        self._file_state: Dict[str, Dict[str, Any]] = {}  # Track file state for incremental
        
        logger.info("Initialized EnhancedChunker with caching and performance optimizations")
    
    def chunk_file(self, path: str | Path, language: str, force_reparse: bool = False) -> List[CodeChunk]:
        ...
        with self._monitor.measure('enhanced_chunk_file'):
            ...
            if cached_ast_data is not None:
                ...
                # Cache the AST
                self._cache.cache_ast(file_path, source_hash, tree, language, parse_time_ms)
    ...
    def chunk_file_incremental(self, path: str | Path, language: str) -> List[CodeChunk]:
        ...
        with self._monitor.measure('incremental_chunk_file'):
            ...
            with self._monitor.measure('detect_changes'):
                changes = self._incremental_parser.detect_changes(old_source, new_source)
            ...
            with self._monitor.measure('incremental_chunk_update'):
                new_chunks = self._incremental_parser.update_chunks(
                    old_state['chunks'], old_tree, new_tree, changes
                )
            ...
            if len(new_chunks) < len(old_state['chunks']):
                ...
                with self._monitor.measure('incremental_rechunk'):
                    full_chunks = self._generate_chunks(
                        new_tree.root_node, new_source, language, file_path
                    )
    ...
    def _generate_chunks(self, root_node, source: bytes, language: str, file_path: str) -> List[CodeChunk]:
