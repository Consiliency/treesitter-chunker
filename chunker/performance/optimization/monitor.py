...
class PerformanceMonitor(PerformanceMonitorInterface):
    ...
    def end_operation(self, operation_id: str) -> float:
        ...
        with self._lock:
            ...
            timing_info.duration_ms = (timing_info.end_time - timing_info.start_time) * 1000
            ...
            # Record as metric
            self.record_metric(f"operation.{timing_info.operation_name}", timing_info.duration_ms)
            
            logger.debug(f"Ended operation: {timing_info.operation_name} "
        ...
        with self._lock:
            ...
            for metric_name, values in self._metrics.items():
                ...
                if len(values) >= 10:
                    ...
                    result[metric_name]['p90'] = sorted_values[int(len(sorted_values) * 0.9)]
                    result[metric_name]['p95'] = sorted_values[int(len(sorted_values) * 0.95)]
                    result[metric_name]['p99'] = sorted_values[int(len(sorted_values) * 0.99)]
