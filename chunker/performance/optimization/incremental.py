...
class IncrementalParser(IncrementalParserInterface):
    ...
    def detect_changes(self, old_source: bytes, new_source: bytes) -> List[Tuple[int, int, int, int]]:
        ...
        old_lines = old_source.decode('utf-8', errors='replace').splitlines(keepends=True)
        new_lines = new_source.decode('utf-8', errors='replace').splitlines(keepends=True)
        ...
        for tag, old_start, old_end, new_start, new_end in matcher.get_opcodes():
            if tag == 'equal':
                ...
                old_pos = sum(len(line.encode('utf-8')) for line in old_lines[old_start:old_end])
                new_pos = sum(len(line.encode('utf-8')) for line in new_lines[new_start:new_end])
            else:
                # Calculate byte positions
                start_byte = sum(len(line.encode('utf-8')) for line in old_lines[:old_start])
                old_end_byte = sum(len(line.encode('utf-8')) for line in old_lines[:old_end])
                new_end_byte = start_byte + sum(len(line.encode('utf-8')) for line in new_lines[new_start:new_end])
    ...
    def update_chunks(self,
        ...
        logger.info(f"Incremental update: {len(affected_chunk_ids)} chunks affected out of {len(old_chunks)}")
    ...
    def _adjust_chunk_offsets(self, chunk: CodeChunk, 
                             changed_ranges: List[Tuple[int, int, int, int]]) -> CodeChunk:
