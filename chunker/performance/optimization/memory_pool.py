    ...
    def acquire(self, resource_type: str) -> Any:
        ...
        with self._lock:
            ...
            if pool:
                ...
                logger.debug(f"Acquired {resource_type} from pool (pool size: {len(pool)})")
            else:
                # Create new resource
    ...
    def release(self, resource: Any) -> None:
        ...
        with self._lock:
            ...
            if len(pool) < self._max_size:
                ...
                logger.debug(f"Released {resource_type} to pool (pool size: {len(pool)})")
            else:
