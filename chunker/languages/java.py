...
class JavaChunker(LanguageChunker):
    ...
    def should_chunk_node(self, node: Node) -> bool:
        ...
        if node.type == "class_body" and node.parent and node.parent.type != "class_declaration":
            return False
    ...
    def extract_chunk_info(self, node: Node, source_code: bytes) -> dict:
        ...
        if node.type == "class_declaration":
            ...
            if (info.get("method_name") == "main" and 
                "static" in info.get("modifiers", []) and
                "public" in info.get("modifiers", [])):
                info["is_main"] = True
