    ...
    def extract_display_name(node: Node, _source: bytes) -> str:
        ...
        if node.type == "class_declaration":
            ...
            if name_node:
                ...
                if type_node:
                    return (
                        f"{type_node.text.decode('utf-8')} {method_name}(...)")
...
class JavaConfig(LanguageConfig):
    ...
    def __init__(self):
        super().__init__()
        self._chunk_rules = [ChunkRule(node_types={"class_declaration",
            "interface_declaration", "enum_declaration",
            "annotation_type_declaration", "record_declaration"},
            include_children=True, priority=1, metadata={"name": "classes",
            "min_lines": 1, "max_lines": 2000}), ChunkRule(node_types={
            "method_declaration", "constructor_declaration"},
            include_children=True, priority=1, metadata={"name": "methods",
            "min_lines": 1, "max_lines": 500}), ChunkRule(node_types={
            "field_declaration"}, include_children=True, priority=1,
            metadata={"name": "fields", "min_lines": 1, "max_lines": 50}),
            ChunkRule(node_types={"static_initializer"}, include_children=True, priority=1, metadata={"name": "static_blocks",
            "min_lines": 2, "max_lines": 200})]
        self._scope_node_types = {"program", "class_declaration",
            "interface_declaration", "enum_declaration",
