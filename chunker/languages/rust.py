        ...
        if node.type == "impl_item":
            ...
            for child in node.children:
                if child.type in {"type_identifier", "generic_type",
                    ...
                    impl_type = chunk.content.encode("utf-8")[child.
    ...
    def process_node(self, node: Node, source: bytes, file_path: str,
        ...
        if node.type == "function_item" and not self.config.custom_options.get(
            ...
            if prev_sibling and prev_sibling.type == "attribute_item":
                attr_content = source[prev_sibling.start_byte:prev_sibling.
        ...
        for child in node.children:
            if child.type == "visibility_modifier":
                visibility = source[child.start_byte:child.end_byte].decode(
                    "utf-8") + " "
                break
        chunk = self.create_chunk(node, source, file_path, parent_context)
        if chunk and self.should_include_chunk(chunk):
            if visibility and visibility.strip() in {"pub", "pub(crate)",
                "pub(super)"}:
