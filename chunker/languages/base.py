...
class LanguageConfigRegistry:
    ...
    def register(self, config: LanguageConfig, aliases: Optional[List[str]] = None) -> None:
        ...
        if errors:
            raise ValueError(f"Invalid configuration for {config.language_id}: " + "; ".join(errors))
    ...
    def get(self, language_id: str) -> Optional[LanguageConfig]:
