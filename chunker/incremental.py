...
from .interfaces.incremental import (
    IncrementalProcessor, ChunkCache, ChangeDetector, IncrementalIndex,
    ChunkChange, ChunkDiff, CacheEntry, ChangeType
)
...
class DefaultIncrementalProcessor(IncrementalProcessor):
    ...
    def compute_diff(self, old_chunks: List[CodeChunk], 
        ...
        for old_chunk, new_chunk in moved_pairs:
            ...
            deleted_chunks = [c for c in deleted_chunks if c.chunk_id != old_chunk.chunk_id]
            
            # Update changes list
            changes = [c for c in changes if c.chunk_id not in (old_chunk.chunk_id, new_chunk.chunk_id)]
    ...
    def update_chunks(self, old_chunks: List[CodeChunk], 
        ...
        for change in diff.changes:
            ...
            elif change.change_type in (ChangeType.ADDED, ChangeType.MODIFIED, ChangeType.MOVED):
                if change.new_chunk:
        ...
        for old_chunk in old_chunks:
            ...
            for new_chunk in new_chunks:
                ...
                if (similarity > 0.85 and 
                    (old_chunk.start_line != new_chunk.start_line or
                     old_chunk.file_path != new_chunk.file_path)):
...
class SimpleIncrementalIndex(IncrementalIndex):
    ...
    def update_chunk(self, old_chunk: Optional[CodeChunk], 
        ...
        self.update_log.append({
            ...
            'action': 'delete' if not new_chunk else ('add' if not old_chunk else 'update')
        })
