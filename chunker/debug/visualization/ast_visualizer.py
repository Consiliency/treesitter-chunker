...
from tree_sitter import Node, Tree as TSTree
...
class ASTVisualizer:
    ...
    def _print_tree(
        ...
        highlight_nodes: Optional[Set[str]] = None,
        _depth: int = 0,
        _rich_tree: Optional[Tree] = None
    ) -> None:
        """Print AST as a rich tree to console."""
        if _rich_tree is None:
            _rich_tree = Tree(
                self._format_node(
                    node, content, show_positions, 
        ...
        for child in node.children:
            ...
            if child.child_count > 0:
                self._print_tree(
                    child, content, chunks, max_depth,
                    show_positions, highlight_nodes, 
                    _depth + 1, child_tree
                )
    ...
    def _format_node(
        ...
        content: bytes,
        show_positions: bool,
        chunks: Optional[List[CodeChunk]],
        ...
        # Field name if available (field_name is accessed through parent)
        # Skip field name for now as it requires parent tracking
        ...
        # Node text preview (for leaf nodes)
    ...
    def _is_chunk_boundary(self, node: Node, chunks: Optional[List[CodeChunk]]) -> bool:
        """Check if node is at a chunk boundary."""
        if not chunks:
            return False
            
        for chunk in chunks:
            if (node.start_byte == chunk.byte_start and 
                node.end_byte == chunk.byte_end):
                return True
        return False
    
    def _node_in_chunk(self, node: Node, chunks: List[CodeChunk]) -> bool:
    ...
    def _render_graph(
        ...
        self._add_graph_node(
            dot, node, content, chunks, 
            max_depth, highlight_nodes, 0
        )
    ...
    def _add_graph_node(
        self,
        dot: Any,  # graphviz.Digraph when available
        node: Node,
        content: bytes,
        chunks: Optional[List[CodeChunk]],
        ...
        if max_depth is None or depth < max_depth:
            for child in node.children:
                self._add_graph_node(
                    dot, child, content, chunks,
                    max_depth, highlight_nodes, 
                    depth + 1, node_id
                )
    ...
    def _to_json(
        ...
        def node_to_dict(n: Node, d: int = 0) -> Dict[str, Any]:
            ...
            if n.child_count == 0:
                result['text'] = content[n.start_byte:n.end_byte].decode('utf-8', errors='replace')
