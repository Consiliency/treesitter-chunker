...
class NodeExplorer:
    ...
    def _display_current_node(self) -> None:
        ...
        if self.current_node.child_count == 0:
            ...
            if len(content) > 100:
                content = content[:97] + "..."
            lines.append(f"[bold]Content:[/bold] {repr(content)}")
...
"""
    ...
    def _go_to_parent(self) -> None:
        ...
        self.console.print("[yellow]Parent navigation requires tracking - going back instead[/yellow]")
        self._go_back()
    ...
    def _show_siblings(self) -> None:
        ...
        for i, sibling in enumerate(info.siblings):
            ...
            table.add_row(
                ...
                sibling.type,
                "",  # Skip field name
                repr(preview) if preview else f"[dim]{sibling.child_count} children[/dim]"
            )
    ...
    def _find_nodes(self, node_type: str) -> None:
        ...
        if not found:
            self.console.print(f"[yellow]No nodes of type '{node_type}' found.[/yellow]")
            return
    ...
    def _test_query(self, query_string: str) -> None:
        ...
        try:
            ...
            self.console.print(f"[green]Found {len(matches)} matches in subtree[/green]")
