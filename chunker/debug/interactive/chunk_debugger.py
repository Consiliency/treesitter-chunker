...
class ChunkDebugger:
    ...
    def analyze_file(
        ...
        if max_chunk_size or min_chunk_size:
            analysis['size_issues'] = self._check_sizes(
                chunks, max_chunk_size, min_chunk_size
            )
            
        # Trace chunking decisions
        if show_decisions:
            analysis['decisions'] = self._trace_decisions(
                tree.root_node, content, chunks
            )
    ...
    def _find_overlaps(self, chunks: List[CodeChunk]) -> List[Tuple[int, int]]:
    ...
    def _trace_decisions(
        ...
        def analyze_node(n: Node, depth: int = 0) -> None:
            # Check if this node became a chunk
            is_chunk = any(
                c.byte_start == n.start_byte and c.byte_end == n.end_byte
                for c in chunks
            )
            
            # Determine why it was/wasn't chunked
            decision = {
                'node_type': n.type,
                'depth': depth,
            ...
            if is_chunk:
                ...
                if n.type not in ['function_definition', 'class_definition',
                                 'method_definition', 'function_declaration']:
                    decision['reasons'].append(f"Node type '{n.type}' not configured for chunking")
    ...
    def _display_analysis(
        ...
        self.console.print(
            Panel(
                f"[bold]Chunk Analysis:[/bold] {Path(file_path).name}",
                expand=False
            )
        )
    ...
    def _display_decision_tree(self, decisions: List[Dict[str, Any]]) -> None:
        ...
        # Show sample of non-chunked nodes
        if not_chunked:
            self.console.print(
            ...
            for decision in examples:
                self.console.print(
                    f"  • {decision['node_type']} ({decision['size']} bytes): "
