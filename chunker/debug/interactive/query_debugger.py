...
class QueryDebugger:
    ...
    def debug_query(
        ...
        try:
            ...
            self._display_results(
                matches, source_code, query_string,
                show_captures, highlight_matches
            )
            
            return matches
            
        except Exception as e:
            self.console.print(f"[red]Query Error:[/red] {str(e)}")
    ...
    def _execute_query(
        ...
        self.console.print("[yellow]Note: Query execution is currently limited due to API changes[/yellow]")
        return []
    ...
    def _highlight_matches(
        ...
        for line_no, line in enumerate(lines):
            if line_no in match_map:
                ...
                indicator = f"[yellow]// ← Match {', '.join(map(str, match_nums))}[/yellow]"
    ...
    def _suggest_query_fix(self, query_string: str, error: str) -> None:
        ...
        if "Expected" in error and "(" in error:
            suggestions.append("Ensure all node types are properly parenthesized: (node_type)")
            
        # Check for invalid capture names
        if "@" in query_string and "capture" in error.lower():
            suggestions.append("Capture names must start with @ and contain only letters/numbers")
