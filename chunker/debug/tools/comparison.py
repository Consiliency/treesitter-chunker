...
class ChunkComparison(ChunkComparisonContract):
    ...
    def compare_strategies(self, file_path: str, language: str, strategies:
        ...
        strategy_map = {"default": self._chunk_default, "adaptive": self.
            _chunk_adaptive, "composite": self._chunk_composite,
            "hierarchical": self._chunk_hierarchical, "semantic": self.
            _chunk_semantic, "token_aware": self._chunk_token_aware,
        ...
        for strategy in strategies:
            try:
                ...
                results[strategy] = {"chunk_count": len(chunks),
                    "total_lines": sum(chunk_sizes), "average_lines": sum(
                    chunk_sizes) / len(chunk_sizes) if chunks else 0,
                    "min_lines": min(chunk_sizes) if chunks else 0,
                    "max_lines": max(chunk_sizes) if chunks else 0,
                    "average_bytes": sum(byte_sizes) / len(byte_sizes) if
                    chunks else 0, "chunks": [{"id": c.chunk_id, "type": c.
                    node_type, "lines": f"{c.start_line}-{c.end_line}",
                    "size": c.end_line - c.start_line + 1} for c in chunks]}
        ...
        for i, strat1 in enumerate(strategies):
            ...
            for strat2 in strategies[i + 1:]:
                if strat2 not in all_chunks:
                    continue
                overlap_count = self._calculate_overlap(all_chunks[strat1],
                    all_chunks[strat2])
                overlaps[f"{strat1}_vs_{strat2}"] = {"overlapping_chunks":
                    overlap_count, "similarity": overlap_count / max(len(
                    all_chunks[strat1]), len(all_chunks[strat2])) if
                    all_chunks[strat1] or all_chunks[strat2] else 0}
        differences = []
        if len(strategies) == 2 and all(s in all_chunks for s in strategies):
            ...
            differences.extend({"strategy": strategies[0], "unique_chunk":
                {"type": c1.node_type, "lines":
                f"{c1.start_line}-{c1.end_line}"}} for c1 in chunks1 if not
                any(self._chunks_overlap(c1, c2) for c2 in chunks2))
            differences.extend({"strategy": strategies[1], "unique_chunk":
                {"type": c2.node_type, "lines":
                f"{c2.start_line}-{c2.end_line}"}} for c2 in chunks2 if not
                any(self._chunks_overlap(c1, c2) for c1 in chunks1))
        return {"file_path": file_path, "language": language, "strategies":
            results, "overlaps": overlaps, "differences": differences[:10],
            "summary": {"total_strategies": len(strategies), "successful":
            sum(1 for r in results.values() if "error" not in r), "failed":
    ...
    def _chunk_token_aware(cls, file_path: str, language: str):
        ...
        chunker = TreeSitterTokenAwareChunker(token_counter=counter,
            max_tokens=1000)
        return chunker.chunk_file(file_path, language)
    ...
    def _chunks_overlap(chunk1, chunk2):
        ...
        return not (chunk1.end_line < chunk2.start_line or chunk2.end_line <
            chunk1.start_line)
