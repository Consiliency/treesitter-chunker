...
class GraphMLExporter(GraphExporter):
    ...
    def __init__(self):
        self._node_attributes: list[str] = ["node_type", "language",
            "file_path", "start_line", "end_line", "parent_context"]
    ...
    def get_schema(self) -> dict[str, Any]:
        ...
        return {"fmt": "graphml", "version": "1.0", "node_attributes": self
            ._node_attributes, "edge_attributes": self._edge_attributes,
            "layout_algorithm": self._layout_algorithm}

    def _build_graphml(self, chunks: list[CodeChunk], relationships: list[
        ChunkRelationship], metadata: (ExportMetadata | None)) -> ET.Element:
        """Build GraphML XML structure."""
        graphml = ET.Element("graphml", {"xmlns":
            "http://graphml.graphdrawing.org/xmlns", "xmlns:xsi":
        ...
        for i, attr in enumerate(self._node_attributes):
            key = ET.SubElement(graphml, "key", {"id": f"d{i}", "for":
                "node", "attr.name": attr, "attr.type": "string"})
        edge_attr_offset = len(self._node_attributes)
        for i, attr in enumerate(self._edge_attributes):
            key = ET.SubElement(graphml, "key", {"id":
                f"d{edge_attr_offset + i}", "for": "edge", "attr.name":
                attr, "attr.type": "string"})
        graph = ET.SubElement(graphml, "graph", {"id": "G", "edgedefault":
            "directed"})
        for chunk in chunks:
        ...
        for idx, rel in enumerate(relationships):
            edge = ET.SubElement(graph, "edge", {"id": f"e{idx}", "source":
                rel.source_chunk_id, "target": rel.target_chunk_id})
            ...
            if rel.metadata:
                for key, value in rel.metadata.items():
                    if key in self._edge_attributes[1:]:
                        ...
                        data = ET.SubElement(edge, "data", {"key":
                            f"d{edge_attr_offset + attr_idx}"})
                        data.text = str(value)
        return graphml
...
class DOTExporter(GraphExporter):
    ...
    def __init__(self):
        ...
        self._node_style: dict[str, str] = {"shape": "box", "style":
            "rounded,filled", "fillcolor": "lightblue"}
        self._edge_style: dict[str, dict[str, str]] = {RelationshipType.
            CALLS.value: {"color": "blue", "style": "solid"},
            RelationshipType.INHERITS.value: {"color": "red", "style":
            "solid", "arrowhead": "empty"}, RelationshipType.IMPLEMENTS.
            value: {"color": "green", "style": "dashed", "arrowhead":
            "empty"}, RelationshipType.IMPORTS.value: {"color": "gray",
            "style": "dotted"}, RelationshipType.USES.value: {"color":
            "purple", "style": "dashed"}, RelationshipType.DEPENDS_ON.value:
    ...
    def get_schema(self) -> dict[str, Any]:
        """Get the export schema."""
        return {"fmt": "dot", "version": "1.0", "node_attributes": self.
            _node_attributes, "edge_attributes": self._edge_attributes,
            "layout_algorithm": self._layout_algorithm, "node_style": self.
            _node_style, "edge_style": self._edge_style}

    def set_node_style(self, style: dict[str, str]) -> None:
        """Set default node style attributes."""
        self._node_style.update(style)

    def set_edge_style(self, relationship_type: str, style: dict[str, str],
        ) -> None:
        """Set edge style for specific relationship type."""
        self._edge_style[relationship_type] = style

    def _build_dot(self, chunks: list[CodeChunk], relationships: list[
        ChunkRelationship], metadata: (ExportMetadata | None)) -> str:
        """Build DOT fmt content."""
        lines = []
        lines.append("digraph CodeStructure {")
        lines.append(f"  rankdir={self._layout_algorithm.upper()};")
        lines.append("  node [" + ", ".join(f'{k}="{v}"' for k, v in self.
            _node_style.items()) + "];")
        if metadata:
            lines.append(
                f"  // Generated by TreeSitter Chunker - {metadata.created_at}",
        ...
        for rel in relationships:
            edge_style = self._edge_style.get(rel.relationship_type.value,
    ...
    def _stream_dot(self, chunk_iterator: Iterator[CodeChunk],
        ...
        output.write("  node [" + ", ".join(f'{k}="{v}"' for k, v in self.
            _node_style.items()) + "];\n")
        output.write("\n")
        output.write("  // Nodes\n")
