...
class GraphMLExporter(GraphExporter):
    ...
    def _build_graphml(self, 
                      chunks: List[CodeChunk],
                      relationships: List[ChunkRelationship],
                      metadata: Optional[ExportMetadata]) -> ET.Element:
...
class DOTExporter(GraphExporter):
    ...
    def __init__(self):
        ...
        self._edge_style: Dict[str, Dict[str, str]] = {
            ...
            RelationshipType.INHERITS.value: {'color': 'red', 'style': 'solid', 'arrowhead': 'empty'},
            RelationshipType.IMPLEMENTS.value: {'color': 'green', 'style': 'dashed', 'arrowhead': 'empty'},
    ...
    def _build_dot(self,
        ...
        lines.append('  node [' + ', '.join(f'{k}="{v}"' for k, v in self._node_style.items()) + '];')
        
        # Add metadata as comment
        if metadata:
            lines.append(f'  // Generated by TreeSitter Chunker - {metadata.created_at}')
            lines.append(f'  // Chunks: {metadata.chunk_count}, Relationships: {metadata.relationship_count}')
    ...
    def _stream_dot(self,
        ...
        output.write('  node [' + ', '.join(f'{k}="{v}"' for k, v in self._node_style.items()) + '];\n')
        output.write('\n')
