...
class SQLiteExporter(DatabaseExporter):
    ...
    def _insert_metadata(self, conn: sqlite3.Connection, metadata: ExportMetadata) -> None:
    ...
    def _insert_relationships(self, conn: sqlite3.Connection, 
    ...
    def _create_indexes(self, conn: sqlite3.Connection) -> None:
        ...
        for column in self._indexes:
            if column in ['chunk_id', 'file_path', 'node_type', 'language', 'parent_chunk_id']:
...
class PostgreSQLExporter(DatabaseExporter):
    ...
    def _generate_insert_chunks(self, chunks: List[CodeChunk]) -> List[str]:
        ...
        for i in range(0, len(chunks), self._batch_size):
            ...
            lines.append("(chunk_id, language, file_path, node_type, start_line, end_line,")
            lines.append(" byte_start, byte_end, parent_context, content, parent_chunk_id,")
            ...
            for chunk in batch:
                ...
                parent_context = chunk.parent_context.replace("'", "''") if chunk.parent_context else 'NULL'
                parent_chunk_id = f"'{chunk.parent_chunk_id}'" if chunk.parent_chunk_id else 'NULL'
    ...
    def _generate_insert_relationships(self, relationships: List[ChunkRelationship]) -> List[str]:
        ...
        for i in range(0, len(relationships), self._batch_size):
            ...
            lines.append("(source_chunk_id, target_chunk_id, relationship_type, metadata)")
            lines.append("VALUES")
            
            values = []
            for rel in batch:
                metadata = f"'{json.dumps(rel.metadata)}'::jsonb" if rel.metadata else 'NULL'
                value = (
