...
class Neo4jExporter(StructuredExporter):
    ...
    def _generate_relationship_queries(self, relationships: List[ChunkRelationship]) -> List[str]:
        ...
        for rel_type, typed_relationships in relationships_by_type.items():
            ...
            for i in range(0, len(typed_relationships), self._batch_size):
                ...
                if any(r.get('metadata') for r in batch_data):
                    query_lines.append("SET r.metadata = CASE WHEN rel.metadata IS NOT NULL THEN rel.metadata ELSE null END")
    ...
    def _stream_cypher(self,
        ...
        output.write(f"CREATE CONSTRAINT IF NOT EXISTS FOR (c:{self._node_label}) REQUIRE c.chunk_id IS UNIQUE;\n")
        output.write(f"CREATE INDEX IF NOT EXISTS FOR (c:{self._node_label}) ON (c.file_path);\n")
        output.write(f"CREATE INDEX IF NOT EXISTS FOR (c:{self._node_label}) ON (c.node_type);\n")
        output.write(f"CREATE INDEX IF NOT EXISTS FOR (c:{self._node_label}) ON (c.language);\n")
        output.write("\n")
        ...
        for rel in relationship_iterator:
            ...
            if len(relationships_by_type[rel_type]) >= self._batch_size:
                self._write_relationship_batch(
                    output, rel_type, relationships_by_type[rel_type]
                )
                relationships_by_type[rel_type] = []
    ...
    def _write_relationship_batch(self, 
                                 output: io.IOBase,
                                 rel_type: str,
        ...
        if any(r.get('metadata') for r in batch_data):
            output.write("SET r.metadata = CASE WHEN rel.metadata IS NOT NULL THEN rel.metadata ELSE null END\n")
