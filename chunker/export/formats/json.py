...
class StructuredJSONExporter(StructuredExporter):
    ...
    def export(self, chunks: list[CodeChunk], relationships: list[
        ...
        """
        data = {"metadata": self._build_metadata(metadata, chunks,
            relationships), "chunks": [(self._chunk_to_dict(c) if hasattr(c,
            "chunk_id") else c) for c in chunks], "relationships": [(self.
            _relationship_to_dict(r) if hasattr(r, "source_chunk_id") else
            r) for r in relationships]}
        json_str = json.dumps(data, indent=self.indent)
        if isinstance(output, str | Path):
            output_path = Path(output)
            if self.compress:
                with gzip.Path(f"{output_path}.gz").open("wt", encoding="utf-8",
                    ) as f:
                    f.write(json_str)
            else:
    ...
    def get_schema(self) -> dict[str, Any]:
        ...
        return {"fmt": "json", "version": "1.0", "indent": self.indent,
            "compress": self.compress, "structure": {"metadata":
            "Export metadata object", "chunks": "Array of chunk objects",
            "relationships": "Array of relationship objects"}}

    def _build_metadata(self, metadata: (ExportMetadata | None), chunks:
        list[CodeChunk], relationships: list[ChunkRelationship]) -> dict[str,
        Any]:
        """Build metadata object."""
        if metadata:
            return {"fmt": metadata.fmt.value, "version": metadata.version,
        ...
        return {"fmt": "json", "version": "1.0", "created_at": datetime.now
            (timezone.utc).isoformat(), "source_files": source_files,
            "chunk_count": len(chunks), "relationship_count": len(
            relationships), "options": {"indent": self.indent, "compress":
            self.compress}}
    ...
    def _chunk_to_dict(chunk: CodeChunk) -> dict[str, Any]:
        ...
        return {"chunk_id": chunk.chunk_id, "language": chunk.language,
            ...
            "parent_context": chunk.parent_context, "content": chunk.
            content, "parent_chunk_id": chunk.parent_chunk_id, "references":
            chunk.references, "dependencies": chunk.dependencies}
    ...
    def _relationship_to_dict(rel: ChunkRelationship) -> dict[str, Any]:
        ...
        return {"source_chunk_id": rel.source_chunk_id, "target_chunk_id":
            ...
            .value, "metadata": rel.metadata or {}}
...
class StructuredJSONLExporter(StructuredExporter):
    ...
    def get_schema(self) -> dict[str, Any]:
        """Get the export schema."""
        return {"fmt": "jsonl", "version": "1.0", "compress": self.compress,
            "record_types": [{"type": "metadata", "description":
            "Export metadata"}, {"type": "chunk", "description":
            "Code chunk data"}, {"type": "relationship", "description":
            "Chunk relationship"}]}

    def _write_jsonl(self, chunks: list[CodeChunk], relationships: list[
        ChunkRelationship], file_path: io.IOBase, metadata: (ExportMetadata |
        None)) -> None:
        """Write data as JSONL to file_path."""
        if metadata:
            meta_record = {"type": "metadata", "data": {"fmt": metadata.fmt
                .value, "version": metadata.version, "created_at": metadata
                .created_at, "source_files": metadata.source_files,
                "chunk_count": metadata.chunk_count, "relationship_count":
                metadata.relationship_count, "options": metadata.options}}
        else:
            source_files = list({c.file_path for c in chunks})
            meta_record = {"type": "metadata", "data": {"fmt": "jsonl",
                "version": "1.0", "created_at": datetime.now(timezone.utc).
                isoformat(), "source_files": source_files, "chunk_count":
                len(chunks), "relationship_count": len(relationships),
        ...
        for rel in relationships:
            record = {"type": "relationship", "data": self.
                _relationship_to_dict(rel)}
            json.dump(record, file_path, separators=(",", ":"))
            file_path.write("\n")

    def _stream_jsonl(self, chunk_iterator: Iterator[CodeChunk],
        relationship_iterator: Iterator[ChunkRelationship], file_path: io.
        IOBase) -> None:
        """Stream data as JSONL to file_path."""
        meta_record = {"type": "metadata", "data": {"fmt": "jsonl",
            "version": "1.0", "created_at": datetime.now(timezone.utc).
            isoformat(), "streaming": True, "options": {"compress": self.
            compress}}}
        json.dump(meta_record, file_path, separators=(",", ":"))
        file_path.write("\n")
        file_path.flush()
