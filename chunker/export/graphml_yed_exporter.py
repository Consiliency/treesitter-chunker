...
class GraphMLyEdExporter(GraphMLExporter):
    ...
    def _create_key_elements(self, root: ET.Element) -> None:
        ...
        # Add yEd-specific keys for nodes
        key = ET.SubElement(root, "key")
        key.set("for", "node")
        key.set("id", "d6")
        key.set("yfiles.type", "nodegraphics")
        
        # Add yEd-specific keys for edges
        key = ET.SubElement(root, "key")
        key.set("for", "edge")
        key.set("id", "d10")
        key.set("yfiles.type", "edgegraphics")
    
    def _create_node_element(self, graph: ET.Element, node_id: str, node: Any) -> None:
        ...
        # Get style for chunk type
        chunk_type = node.properties.get("chunk_type", "unknown")
        style = self.default_node_styles.get(chunk_type, self.default_node_styles.get("function"))
    ...
    def _create_edge_element(self, graph: ET.Element, edge: Any, edge_id: int) -> None:
    ...
    def export_string(self, pretty_print: bool = True, use_yed: bool = True, **options) -> str:
        ...
        if use_yed:
            ...
            # Create root with yEd namespace
            ...
            # Add nodes and edges with yEd extensions
            ...
            # Use parent implementation without yEd extensions
