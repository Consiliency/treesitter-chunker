...
class CMetadataExtractor(BaseMetadataExtractor):
    ...
    def extract_signature(self, node: Node, source: bytes) -> SignatureInfo | None:
        ...
        if node.type == "function_definition":
            ...
            for i, child in enumerate(node.children):
                if child == declarator or (child.type == "pointer_declarator" and 
                                          declarator in self._get_all_descendants(child)):
        ...
        if node.type == "function_definition":
            for child in node.children:
                ...
                elif child.type in {"type_qualifier", "inline"}:
                    modifier = self._get_node_text(child, source)
                    if modifier:
                        modifiers.append(modifier)
    ...
    def _extract_c_call_info(self, call_node: Node, func_node: Node, source: bytes) -> dict[str, Any] | None:
    ...
    def extract_dependencies(self, node: Node, source: bytes) -> set[str]:
        ...
        c_builtins = {
            ...
            "size_t", "FILE", "void", "int", "char", "float", "double",
    ...
    def _find_all_declarators(self, node: Node) -> list[Node]:
        ...
        def find_declarators(n: Node):
            if n.type in {"init_declarator", "declarator", "pointer_declarator", 
