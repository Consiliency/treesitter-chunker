...
class PythonMetadataExtractor(BaseMetadataExtractor):
    ...
    def extract_signature(self, node: Node, source: bytes) -> Optional[SignatureInfo]:
        ...
        if node.parent and node.parent.type == 'decorated_definition':
            ...
            decorators = [self._get_node_text(d, source).strip('@') for d in decorator_nodes]
    ...
    def extract_dependencies(self, node: Node, source: bytes) -> Set[str]:
        ...
        builtins = {
            ...
            'delattr', 'None', 'True', 'False', 'self', 'cls', 'super', 'object',
            'Exception', 'ValueError', 'TypeError', 'KeyError', 'IndexError', 'AttributeError',
            'open', 'file', 'input', 'zip', 'map', 'filter', 'sorted', 'reversed', 'enumerate',
        ...
        if param_node.type == 'identifier':
            ...
            for i, child in enumerate(param_node.children):
                if child.type == '=' and i + 1 < len(param_node.children):
                    param_info['default'] = self._get_node_text(param_node.children[i + 1], source)
    ...
    def _find_string_node(self, node: Node) -> Optional[Node]:
    ...
    def _extract_defined_symbols(self, node: Node, source: bytes) -> Set[str]:
        ...
        def collect_definitions(n: Node, depth: int):
            ...
            elif n.type in ('list_comprehension', 'dictionary_comprehension', 
