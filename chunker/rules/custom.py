...
from ..interfaces.rules import CustomRule, RegexRule, CommentBlockRule, RuleMatch
...
class BaseRegexRule(RegexRule):
    ...
    def __init__(self, name: str, description: str, pattern: str, 
                 priority: int = 0, cross_boundaries: bool = True,
                 multiline: bool = True):
        self._name = name
        self._description = description
        self._priority = priority
    ...
    def matches(self, node: Node, source: bytes) -> bool:
        ...
        if not self._cross_boundaries:
            ...
            return bool(self._pattern.search(node_text.decode('utf-8', errors='replace')))
        return False  # Regex rules that cross boundaries are handled differently
