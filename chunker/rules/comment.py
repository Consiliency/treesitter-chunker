...
from ..interfaces.rules import CommentBlockRule
...
class TodoBlockRule(BaseCommentBlockRule):
    ...
    def __init__(self, keywords: List[str] = None, include_context_lines: int = 2,
                 priority: int = 55):
        """
        ...
        self.keywords = keywords or ['TODO', 'FIXME', 'HACK', 'NOTE', 'XXX', 'BUG', 'OPTIMIZE', 'REFACTOR']
...
class DocumentationBlockRule(BaseCommentBlockRule):
    ...
    def matches(self, node: Node, source: bytes) -> bool:
        ...
        if node.type in ['string', 'string_literal', 'template_string']:
            ...
            parent = node.parent
            if parent and parent.type in ['function_definition', 'class_definition', 'method_definition']:
        ...
        if parent and parent.type in ['function_definition', 'class_definition', 'method_definition']:
            ...
            for child in parent.children:
                if child.type in ['identifier', 'property_identifier']:
                    entity_name = source[child.start_byte:child.end_byte].decode('utf-8', errors='replace')
...
class HeaderCommentRule(BaseCommentBlockRule):
    ...
    def __init__(self, max_lines_from_start: int = 50, priority: int = 85):
        ...
        self._header_patterns = [
            re.compile(r'\b(?:copyright|©|\(c\)|license|author|created|modified)\b', re.IGNORECASE),
...
class InlineCommentGroupRule(BaseCommentBlockRule):
    ...
    def __init__(self, max_gap_lines: int = 1, min_comments: int = 2, priority: int = 25):
        """
    ...
    def _find_comment_group(self, start_node: Node, source: bytes) -> Optional[List[Node]]:
...
def create_comment_rule_chain(*rules: BaseCommentBlockRule) -> List[BaseCommentBlockRule]:
    """
