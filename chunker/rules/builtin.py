...
from .custom import BaseRegexRule, BaseCommentBlockRule, MetadataRule
...
class TodoCommentRule(BaseRegexRule):
    ...
    def __init__(self, priority: int = 50):
        super().__init__(
            ...
            pattern=r'(?:#|//|/\*|\*)\s*(TODO|FIXME|HACK|NOTE|XXX|BUG|OPTIMIZE|REFACTOR)\s*:?\s*(.+?)(?:\*/|$)',
            priority=priority,
...
class CopyrightHeaderRule(BaseRegexRule):
    ...
    def __init__(self, priority: int = 90):
        super().__init__(
            ...
            pattern=r'(?:#|//|/\*)\s*(?:Copyright|©|\(c\)|License).*?(?:\*/|$)',
            priority=priority,
            cross_boundaries=True,
            multiline=True
        )
...
class DocstringRule(BaseRegexRule):
    ...
    def __init__(self, priority: int = 60):
        super().__init__(
            ...
            # Matches Python docstrings, JSDoc blocks, and similar
            pattern=r'(?:"""[\s\S]*?"""|\'\'\'[\s\S]*?\'\'\'|/\*\*[\s\S]*?\*/)',
            priority=priority,
            cross_boundaries=False,  # Docstrings usually within nodes
            multiline=True
        )
...
class ImportBlockRule(BaseRegexRule):
    ...
    def __init__(self, priority: int = 70):
        super().__init__(
            ...
            # Matches consecutive import/from/require/use statements
...
class CustomMarkerRule(BaseRegexRule):
    ...
    def __init__(self, marker_start: str = "CHUNK_START", marker_end: str = "CHUNK_END", 
                 priority: int = 80):
        # Build pattern to match content between markers
        pattern = rf'(?:#|//|/\*)\s*{marker_start}.*?\n([\s\S]*?)(?:#|//|/\*)\s*{marker_end}'
...
class SectionHeaderRule(BaseRegexRule):
    ...
    def __init__(self, priority: int = 40):
        super().__init__(
            ...
            # Matches headers like: # === Section Name ===, // --- Section ---, etc.
            pattern=r'(?:^|\n)\s*(?:#|//)\s*(?:={3,}|#{2,}|-{3,})\s*(.+?)\s*(?:={3,}|-{3,}|#{2,})?(?:\n|$)',
            priority=priority,
            cross_boundaries=True,
            multiline=True
...
class ConfigurationBlockRule(BaseRegexRule):
    ...
    def __init__(self, priority: int = 30):
        super().__init__(
            ...
            # Matches config blocks like: /* config: { ... } */
            pattern=r'(?:/\*|#|//)\s*(?:config|configuration|settings|options)\s*:\s*[\{\[][\s\S]*?[\}\]][\s\S]*?(?:\*/|\n)',
            priority=priority,
            cross_boundaries=True,
            multiline=True
        )
...
class DebugStatementRule(BaseRegexRule):
    ...
    def __init__(self, priority: int = 10):
        super().__init__(
            ...
            # Matches common debug patterns
            pattern=r'(?:console\.(?:log|debug|info|warn|error)|print(?:ln)?|debug|logger\.(?:debug|info)|System\.out\.println?)\s*\([^;]+\);?',
            priority=priority,
...
class TestAnnotationRule(BaseRegexRule):
    ...
    def __init__(self, priority: int = 35):
        super().__init__(
            ...
            # Matches @test, @skip, test cases, etc.
            pattern=r'(?:@(?:test|skip|ignore|disabled)|(?:it|test|describe)\s*\(["\'].*?["\']\s*,|TEST_CASE\s*\()',
            priority=priority,
            cross_boundaries=False,
            multiline=False
        )
