...
class LanguageNotFoundError(LanguageError):
    ...
    def _get_user_guidance(self, language: str, available: list[str]) -> str:
        ...
        common_languages = {
            "python", "javascript", "rust", "go", "c", "cpp", "java", "csharp", 
            "typescript", "ruby", "php", "kotlin", "swift", "dart"
        }
        
        if language.lower() in common_languages:
            if not available:
                guidance_parts.append("💡 This appears to be a common language that should be available.")
                guidance_parts.append("   The grammar libraries may not be compiled yet.")
                guidance_parts.append("   Try running the grammar compilation process first.")
            else:
                guidance_parts.append("💡 This language is not currently compiled but may be available.")
                guidance_parts.append("   Check if the grammar source is available in the grammars/ directory.")
        
        # Provide specific guidance for missing grammars
        if not available:
            guidance_parts.append("\n🔧 To resolve this issue:")
            guidance_parts.append("   1. Check that grammar libraries are compiled")
            guidance_parts.append("   2. Verify the grammars/ directory contains source repositories")
            guidance_parts.append("   3. Run grammar compilation scripts if needed")
        else:
            guidance_parts.append(f"\n🔧 To add support for '{language}':")
            guidance_parts.append("   1. Check if grammar source exists in grammars/ directory")
            guidance_parts.append("   2. Compile the grammar to a .so library")
            guidance_parts.append("   3. Place the .so file in chunker/data/grammars/build/")
        
        # Add troubleshooting tips
        guidance_parts.append("\n💡 Troubleshooting tips:")
        guidance_parts.append("   - Verify the language name is correct (check for typos)")
        guidance_parts.append("   - Check if the language uses a different name (e.g., 'csharp' vs 'c_sharp')")
        guidance_parts.append("   - Ensure the grammar is compatible with your tree-sitter version")
...
class LanguageLoadError(LanguageError):
    ...
    def _get_loading_guidance(self, language: str, reason: str) -> str:
        ...
        if "symbol" in reason.lower():
            ...
            guidance_parts.append("   1. The grammar library may be corrupted or incomplete")
            guidance_parts.append("   2. Try recompiling the grammar from source")
            guidance_parts.append("   3. Check if the grammar is compatible with your tree-sitter version")
        
        elif "version" in reason.lower():
            guidance_parts.append("🔧 This appears to be a version compatibility issue:")
            guidance_parts.append("   1. The grammar may be compiled for a different tree-sitter version")
            guidance_parts.append("   2. Try recompiling the grammar with your current tree-sitter")
            ...
            guidance_parts.append("   1. Verify the grammar library file exists and is not corrupted")
        ...
        guidance_parts.append("   - Verify the grammar supports your target language version")
        guidance_parts.append("   - Consider using a different grammar version if available")
...
class LibraryNotFoundError(LibraryError):
    ...
    def _get_library_guidance(self, path: Path) -> str:
        ...
        if "chunker/data/grammars/build" in str(path):
            guidance_parts.append("🔧 This is a package grammar library that should be available:")
            guidance_parts.append("   1. The grammar libraries may not be compiled yet")
            guidance_parts.append("   2. Check if the package was built with grammar support")
            guidance_parts.append("   3. Try reinstalling the package or building from source")
        ...
        # Check if this is a user grammar directory
            ...
            guidance_parts.append("   2. Try installing the grammar using the CLI tools")
            guidance_parts.append("   3. Check if the grammar source is available")
        
        # General guidance
        guidance_parts.append("\n💡 To resolve this issue:")
        guidance_parts.append("   1. Verify the grammar source exists in the grammars/ directory")
        guidance_parts.append("   2. Compile the grammar to a .so library")
        guidance_parts.append("   3. Ensure the .so file is placed in the correct directory")
        
        # Add troubleshooting tips
        guidance_parts.append("\n🔍 Troubleshooting steps:")
        guidance_parts.append("   - Check if the grammar repository exists in grammars/")
...
class ParsingError(ChunkerError):
    ...
    def _get_parsing_guidance(self, language: str, reason: str, code_sample: str = None) -> str:
        ...
        if "syntax" in reason.lower():
            ...
            guidance_parts.append("   1. Your code may use syntax not supported by the current grammar")
            guidance_parts.append("   2. The grammar may be outdated for your language version")
            guidance_parts.append("   3. Try updating the grammar or using compatible syntax")
        
        elif "version" in reason.lower():
            guidance_parts.append("🔧 This appears to be a version compatibility issue:")
            guidance_parts.append("   1. Your code may use features from a newer language version")
            guidance_parts.append("   2. The grammar may not support the language version you're using")
            guidance_parts.append("   3. Consider updating the grammar or using an older language version")
        
        elif "feature" in reason.lower() or "construct" in reason.lower():
            guidance_parts.append("🔧 This appears to be a feature compatibility issue:")
            guidance_parts.append("   1. Your code uses language features not yet supported by the grammar")
            guidance_parts.append("   2. The grammar may be incomplete or experimental")
            guidance_parts.append("   3. Try simplifying the code or using supported features")
        
        else:
            guidance_parts.append("🔧 General parsing troubleshooting:")
            guidance_parts.append("   1. Check if your code uses standard, supported syntax")
            guidance_parts.append("   2. Verify the grammar supports your language version")
            guidance_parts.append("   3. Consider updating the grammar if available")
        
        # Add specific guidance for the language
        guidance_parts.append(f"\n💡 For {language} specifically:")
        guidance_parts.append("   - Check if the grammar supports your language version")
        ...
        if code_sample:
            ...
            guidance_parts.append("   - Check for language features that may not be supported")
        ...
        guidance_parts.append("   2. Use alternative syntax that's more widely supported")
        guidance_parts.append("   3. Check if there are grammar forks with better support")
