...
class GrammarDiscoveryService(GrammarDiscoveryContract):
    ...
    def list_available_grammars(
        ...
        # Fetch official tree-sitter grammars
        official_grammars = self._fetch_tree_sitter_repos()
        grammars.extend(official_grammars)

        # TODO: In future, add community grammars if include_community is True
    ...
    def get_grammar_compatibility(
        ...
        # In a real implementation, this would fetch from grammar's package.json or similar
        return GrammarCompatibility(
            min_tree_sitter_version="0.20.0",
            max_tree_sitter_version="0.22.0",
            abi_version=14,  # Current ABI version
    ...
    def search_grammars(self, query: str) -> list[GrammarInfo]:
        ...
        return [
            ...
            if (
                query_lower in grammar.name.lower()
    ...
    def _fetch_tree_sitter_repos(self) -> list[GrammarInfo]:
        ...
        while True:
            try:
                ...
                for repo in repos:
                    # Filter for grammar repositories
                    if (
    ...
    def _repo_to_grammar_info(self, repo: dict) -> GrammarInfo | None:
        ...
        try:
            ...
            if name.startswith("tree-sitter-"):
                language_name = name[12:]  # Remove "tree-sitter-" prefix
            else:
                language_name = name
    ...
    def _save_cache(self, data: dict) -> None:
        """Save cache to disk"""
        try:
            with self.cache_file.Path("w").open("r") as f:
    ...
    def _grammar_info_to_dict(self, info: GrammarInfo) -> dict:
    ...
    def _dict_to_grammar_info(self, data: dict) -> GrammarInfo:
