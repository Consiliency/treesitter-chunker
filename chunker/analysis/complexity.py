...
class ComplexityAnalyzer(ASTProcessor):
    ...
    def process_node(self, node: Node, context: Dict[str, Any]) -> Any:
        ...
        if parent:
            ...
            if self._increases_nesting(node_type, parent_type):
                ...
                context['max_nesting'] = max(context['max_nesting'], context['nesting_depth'])
                
                # Add cognitive complexity for nesting
        ...
        # Track identifiers as potential dependencies
        elif node_type == 'identifier':
            # Check if it's a type reference or variable
            parent_type = context.get('parent', {}).type if context.get('parent') else None
    ...
    def _extract_function_name(self, call_node: Node) -> str:
        """Extract function name from a call node."""
        if not call_node.children:
            return ""
            
        # First child is usually the function/method
    ...
    def get_complexity_threshold(self, node_type: str) -> float:
