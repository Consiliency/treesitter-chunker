...
class LanguageDetectorImpl(LanguageDetector):
    ...
    EXTENSIONS = {".py": "python", ".js": "javascript", ".jsx":
        "javascript", ".ts": "typescript", ".tsx": "typescript", ".java":
        "java", ".cpp": "cpp", ".cc": "cpp", ".cxx": "cpp", ".c": "c", ".h":
        "c", ".hpp": "cpp", ".rs": "rust", ".go": "go", ".rb": "ruby",
        ".php": "php", ".swift": "swift", ".kt": "kotlin", ".scala":
        "scala", ".r": "r", ".m": "objc", ".mm": "objc", ".cs": "csharp",
        ".vb": "vb", ".fs": "fsharp", ".ml": "ocaml", ".lua": "lua", ".pl":
        "perl", ".sh": "bash", ".bash": "bash", ".zsh": "bash", ".fish":
        "bash", ".ps1": "powershell", ".psm1": "powershell", ".html":
        "html", ".htm": "html", ".xml": "xml", ".css": "css", ".scss":
        "scss", ".sass": "sass", ".less": "less", ".json": "json", ".yaml":
        "yaml", ".yml": "yaml", ".toml": "toml", ".ini": "ini", ".cfg":
        "ini", ".md": "markdown", ".markdown": "markdown", ".rst": "rst",
        ".tex": "latex", ".sql": "sql", ".graphql": "graphql", ".gql":
        "graphql", ".ipynb": "jupyter"}
    SHEBANG_PATTERNS = {"python[0-9\\.]*": "python", "node": "javascript",
        "ruby": "ruby", "perl": "perl", "bash": "bash", "sh": "bash", "zsh":
        "bash", "fish": "bash", "lua": "lua", "php": "php"}
    CONTENT_PATTERNS = {"python": [
        "^\\s*import\\s+\\w+(?:\\s*,\\s*\\w+)*\\s*$",
        "^\\s*from\\s+[\\w\\.]+\\s+import", "^\\s*def\\s+\\w+\\s*\\(",
        "^\\s*class\\s+\\w+\\s*[:\\(]",
        '^\\s*if\\s+__name__\\s*==\\s*["\\\']__main__["\\\']', "^\\s*@\\w+",
        ], "javascript": ["^\\s*const\\s+\\w+\\s*=",
        "^\\s*let\\s+\\w+\\s*=", "^\\s*var\\s+\\w+\\s*=",
        "^\\s*function\\s+\\w+\\s*\\(", "^\\s*class\\s+\\w+\\s*[\\{]",
        '^\\s*import\\s+.*\\s+from\\s+["\\\']',
        "^\\s*export\\s+(default\\s+)?"], "typescript": [
        "^\\s*interface\\s+\\w+\\s*[\\{]", "^\\s*type\\s+\\w+\\s*=",
        "^\\s*enum\\s+\\w+\\s*[\\{]",
        ":\\s*(string|number|boolean|any|void|never|unknown)\\s*[;,\\)\\}]",
        ], "java": ["^\\s*package\\s+[\\w\\.]+;",
        "^\\s*import\\s+[\\w\\.]+\\.*;?\\s*$",
        "^\\s*public\\s+class\\s+\\w+", "^\\s*private\\s+\\w+\\s+\\w+;",
        "^\\s*public\\s+static\\s+void\\s+main",
        "^\\s*(public|private|protected)\\s+\\w+\\s+\\w+\\s*[;=\\(]"], "go":
        ["^\\s*package\\s+\\w+", "^\\s*import\\s+\\(",
        "^\\s*func\\s+\\w+\\s*\\(", "^\\s*type\\s+\\w+\\s+struct\\s*\\{",
        "^\\s*var\\s+\\w+\\s+\\w+"], "rust": ["^\\s*use\\s+\\w+",
        "^\\s*fn\\s+\\w+\\s*\\(", "^\\s*struct\\s+\\w+\\s*[\\{\\(]",
        "^\\s*impl\\s+\\w+", "^\\s*let\\s+(mut\\s+)?\\w+",
        "^\\s*pub\\s+(fn|struct|enum|trait)"], "ruby": [
        '^\\s*require\\s+["\\\']', '^\\s*require_relative\\s+["\\\']',
        "^\\s*def\\s+\\w+", "^\\s*class\\s+\\w+", "^\\s*module\\s+\\w+",
        "^\\s*attr_(reader|writer|accessor)\\s+"], "php": ["<\\?php",
        "^\\s*namespace\\s+[\\w\\\\\\\\]+;", "^\\s*use\\s+[\\w\\\\\\\\]+;",
        "^\\s*class\\s+\\w+", "^\\s*function\\s+\\w+\\s*\\(", "\\$\\w+\\s*="]}
            ...
            if content.startswith("#!"):
                ...
                for pattern, lang in self.SHEBANG_PATTERNS.items():
                    if re.search(pattern, first_line):
                        return lang, 0.95
            if language:
                content_lang, content_conf = self.detect_from_content(content,
                    hint=language)
    ...
    def detect_from_content(self, content: str, hint: (str | None) = None,
        ) -> tuple[str, float]:
    ...
    def detect_multiple(self, content: str) -> list[tuple[str, float]]:
        ...
        script_blocks = re.findall(r"<script[^>]*>(.*?)</script>", content,
            re.DOTALL | re.IGNORECASE)
        language_blocks.extend(("javascript", len(block)) for block in
            script_blocks)
        style_blocks = re.findall(r"<style[^>]*>(.*?)</style>", content, re.
        ...
        if language_blocks:
            ...
            results = sorted(language_percentages.items(), key=lambda x: x[
                1], reverse=True)
            return results
...
class ProjectAnalyzerImpl(ProjectAnalyzer):
    ...
    def analyze_structure(self, project_path: str) -> dict[str, Any]:
        ...
        analysis = {"project_path": str(project_root), "languages":
            defaultdict(int), "file_count": 0, "total_lines": 0,
            "framework_indicators": {}, "project_type": "unknown",
            "structure": {"has_backend": False, "has_frontend": False,
            "has_tests": False, "has_docs": False, "has_config": False}}
        framework_files = {"package.json": ["javascript", "node", "npm"],
            ...
            "maven"], "build.gradle": ["java", "gradle"], "Gemfile": [
            "ruby"], "composer.json": ["php"], "CMakeLists.txt": ["cpp",
            "cmake"], "Makefile": ["make"], "Dockerfile": ["docker"],
            "docker-compose.yml": ["docker"], ".gitignore": ["git"]}
        for root, dirs, files in os.walk(project_root):
            ...
            for file_path in files:
                ...
                try:
                    lang, confidence = self.detector.detect_from_file(str(
                        file_path))
                    if confidence > 0.5:
                    ...
                    if any(part in path_parts for part in ["src", "lib",
                        "app", "backend", "server", "api"]):
                        analysis["structure"]["has_backend"] = True
                    if any(part in path_parts for part in ["frontend",
                        "client", "web", "static", "public", "src/components"]
                        ):
                        analysis["structure"]["has_frontend"] = True
                    if any(part in path_parts for part in ["test", "tests",
                        "spec", "__tests__"]):
                        analysis["structure"]["has_tests"] = True
                    if any(part in path_parts for part in ["docs",
    ...
    def _determine_project_type(analysis: dict[str, Any]) -> str:
        ...
        if structure["has_frontend"]:
            if ("javascript" in indicators or ("typescript" in indicators and
                "node" in indicators)):
        ...
        if any(key in indicators for key in ["npm", "python", "rust", "go"]
            ...
            return "library"
        if ("swift" in languages or "kotlin" in languages or "java" in
            languages) and ("android" in str(analysis["project_path"]).
            lower() or "ios" in str(analysis["project_path"]).lower()):
            return "mobile_app"
    ...
    def find_api_boundaries(chunks: list[CodeChunk]) -> list[dict[str, Any]]:
        ...
        for chunk in chunks:
            ...
            if any(pattern in path_lower for pattern in ["api/", "/api/",
                "routes/", "controllers/", "endpoints/"]):
                api_chunks.append(chunk)
            elif any(pattern in path_lower for pattern in ["backend/",
                "server/", "src/main/"]):
                backend_chunks.append(chunk)
            elif any(pattern in path_lower for pattern in ["frontend/",
                "client/", "src/components/", "pages/"]):
                frontend_chunks.append(chunk)
            if chunk.language in {"python", "javascript", "typescript",
                "java", "go"}:
                rest_patterns = [
                ...
                for pattern in rest_patterns:
                    if re.search(pattern, chunk.content):
                        endpoint_match = re.search(
                            '["\\\']([/\\w\\-\\{\\}:]+)["\\\']', chunk.content)
                        if endpoint_match:
                            api_boundaries.append({"type": "rest_endpoint",
                                "chunk_id": chunk.chunk_id, "endpoint":
                                endpoint_match.group(1), "method":
                                "detected", "language": chunk.language,
                                "file_path": chunk.file_path})
                graphql_patterns = ["type\\s+Query\\s*\\{",
                    "type\\s+Mutation\\s*\\{", "@(Query|Mutation|Resolver)"]
                api_boundaries.extend({"type": "graphql_schema", "chunk_id":
                    chunk.chunk_id, "language": chunk.language, "file_path":
                    chunk.file_path} for pattern in graphql_patterns if re.
                    search(pattern, chunk.content))
        api_boundaries.extend({"type": "grpc_service", "chunk_id": chunk.
            chunk_id, "file_path": chunk.file_path} for chunk in chunks if
            chunk.language in {"proto", "protobuf"} or ".proto" in chunk.
            file_path)
        return api_boundaries

    @classmethod
    def suggest_chunk_grouping(cls, chunks: list[CodeChunk]) -> dict[str,
        list[CodeChunk]]:
        ...
        for chunk in chunks:
            ...
            for i, part in enumerate(path_parts):
                if part in {"features", "modules", "components", "services",
                    "domains"} and i + 1 < len(path_parts):
...
class MultiLanguageProcessorImpl(MultiLanguageProcessor):
    ...
    def __init__(self, detector: (LanguageDetector | None) = None, analyzer:
        (ProjectAnalyzer | None) = None):
        self.detector = detector or LanguageDetectorImpl()
        ...
        try:
            ...
            self._supported_languages = {"python", "javascript",
                "typescript", "java", "go", "rust", "c", "cpp", "ruby",
                "php", "swift", "kotlin", "csharp"}
    ...
    def identify_language_regions(self, file_path: str, content: str) -> list[
        LanguageRegion]:
        ...
        if file_path.endswith((".jsx", ".tsx")):
            ...
            regions.extend(self._identify_embedded_regions(content,
                primary_lang))
        if not regions and content.strip():
            regions.append(LanguageRegion(language=primary_lang, start_pos=0, end_pos=len(content), start_line=1, end_line=len(lines),
                embedding_type=None, parent_language=None))
        return regions

    @classmethod
    def _identify_jsx_regions(cls, content: str, base_language: str) -> list[
        LanguageRegion]:
        """Identify JSX/TSX regions."""
        regions = []
        lines = content.split("\n")
        regions.append(LanguageRegion(language=base_language, start_pos=0,
            end_pos=len(content), start_line=1, end_line=len(lines),
            embedding_type=EmbeddedLanguageType.TEMPLATE, parent_language=None),
            )
        style_pattern = r"style\\s*=\\s*\\{\\{([^}]+)\\}\\}"
        for match in re.finditer(style_pattern, content):
            start_line = content[:match.start()].count("\n") + 1
            end_line = content[:match.end()].count("\n") + 1
            regions.append(LanguageRegion(language="css", start_pos=match.
                start(1), end_pos=match.end(1), start_line=start_line,
                end_line=end_line, embedding_type=EmbeddedLanguageType.
                STYLE, parent_language=base_language))
        return regions
    ...
    def _identify_html_regions(cls, content: str) -> list[LanguageRegion]:
        ...
        regions = []
        regions.append(LanguageRegion(language="html", start_pos=0, end_pos=len(content), start_line=1, end_line=content.count("\n") + 1,
            embedding_type=None, parent_language=None))
        script_pattern = "<script[^>]*>(.*?)</script>"
        for match in re.finditer(script_pattern, content, re.DOTALL | re.
            IGNORECASE):
            start_line = content[:match.start(1)].count("\n") + 1
            end_line = content[:match.end(1)].count("\n") + 1
            regions.append(LanguageRegion(language="javascript", start_pos=match.start(1), end_pos=match.end(1), start_line=start_line,
                end_line=end_line, embedding_type=EmbeddedLanguageType.
                SCRIPT, parent_language="html"))
        style_pattern = "<style[^>]*>(.*?)</style>"
        for match in re.finditer(style_pattern, content, re.DOTALL | re.
            IGNORECASE):
            start_line = content[:match.start(1)].count("\n") + 1
            end_line = content[:match.end(1)].count("\n") + 1
            regions.append(LanguageRegion(language="css", start_pos=match.
    ...
    def _identify_markdown_regions(cls, content: str) -> list[LanguageRegion]:
        ...
        regions = []
        regions.append(LanguageRegion(language="markdown", start_pos=0,
            end_pos=len(content), start_line=1, end_line=content.count("\n",
            ) + 1, embedding_type=None, parent_language=None))
        code_block_pattern = r"```(\\w+)?\\n(.*?)```"
        for match in re.finditer(code_block_pattern, content, re.DOTALL):
            language = match.group(1) or "text"
            start_line = content[:match.start(2)].count("\n") + 1
            end_line = content[:match.end(2)].count("\n") + 1
            regions.append(LanguageRegion(language=language, start_pos=match.start(2), end_pos=match.end(2), start_line=start_line,
                end_line=end_line, embedding_type=EmbeddedLanguageType.
                DOCUMENTATION, parent_language="markdown"))
        return regions
        ...
        try:
            ...
            for cell in notebook.get("cells", []):
                ...
                if cell_type == "code":
                    ...
                    regions.append(LanguageRegion(language=language,
                        start_pos=current_pos, end_pos=current_pos + len(
                        source), start_line=current_line, end_line=current_line + lines_in_cell - 1, embedding_type=EmbeddedLanguageType.SCRIPT, parent_language="jupyter"),
                        )
                elif cell_type == "markdown":
                    lines_in_cell = source.count("\n") + 1
                    regions.append(LanguageRegion(language="markdown",
                        start_pos=current_pos, end_pos=current_pos + len(
                        source), start_line=current_line, end_line=current_line + lines_in_cell - 1, embedding_type=EmbeddedLanguageType.DOCUMENTATION, parent_language="jupyter"))
                current_pos += len(source)
                current_line += source.count("\n") + 1
        except json.JSONDecodeError:
            regions.append(LanguageRegion(language="text", start_pos=0,
                end_pos=len(content), start_line=1, end_line=content.count(
                "\n") + 1, embedding_type=None, parent_language=None))
        return regions

    @classmethod
    def _identify_embedded_regions(cls, content: str, primary_language: str,
        ) -> list[LanguageRegion]:
        """Identify embedded language regions in regular source files."""
        regions = []
        sql_pattern = '["\\\'](\\s*SELECT\\s+.*?\\s+FROM\\s+.*?)["\\\']'
        for match in re.finditer(sql_pattern, content, re.IGNORECASE | re.
            DOTALL):
            start_line = content[:match.start(1)].count("\n") + 1
            end_line = content[:match.end(1)].count("\n") + 1
            regions.append(LanguageRegion(language="sql", start_pos=match.
                start(1), end_pos=match.end(1), start_line=start_line,
                end_line=end_line, embedding_type=EmbeddedLanguageType.
                QUERY, parent_language=primary_language))
        ...
        for match in re.finditer(graphql_pattern, content):
            ...
            if group:
                start_line = content[:match.start()].count("\n") + 1
                end_line = content[:match.end()].count("\n") + 1
                regions.append(LanguageRegion(language="graphql", start_pos=match.start(), end_pos=match.end(), start_line=start_line, end_line=end_line, embedding_type=EmbeddedLanguageType.QUERY, parent_language=primary_language))
        string_pattern = '["\\\'](\\{.*?\\}|\\[.*?\\])["\\\']'
        for match in re.finditer(string_pattern, content, re.DOTALL):
            try:
                ...
                start_line = content[:match.start(1)].count("\n") + 1
                end_line = content[:match.end(1)].count("\n") + 1
                regions.append(LanguageRegion(language="json", start_pos=match.start(1), end_pos=match.end(1), start_line=start_line, end_line=end_line, embedding_type=EmbeddedLanguageType.CONFIGURATION, parent_language=primary_language))
            except (FileNotFoundError, OSError):
                pass
        return regions

    def process_mixed_file(self, file_path: str, primary_language: str,
        content: (str | None) = None) -> list[CodeChunk]:
        ...
        for region in regions:
            ...
            try:
                ...
                region_chunks = chunk_file(file_path=file_path, content=region_content, language=region.language)
                for chunk in region_chunks:
                    ...
                    if region.parent_language:
                        chunk.metadata["parent_language"
                            ] = region.parent_language
                    chunks.append(chunk)
            except (FileNotFoundError, IndexError, KeyError) as e:
                chunk = CodeChunk(language=region.language, file_path=file_path, node_type="region", start_line=region.
                    start_line, end_line=region.end_line, byte_start=region
                    .start_pos, byte_end=region.end_pos, parent_context="",
                    content=region_content, metadata={"embedding_type":
                    region.embedding_type.value if region.embedding_type else
                    None, "parent_language": region.parent_language,
                    "parse_error": str(e)})
                chunks.append(chunk)
        return chunks
    ...
    def extract_embedded_code(content: str, host_language: str,
        ...
        if host_language == "html" and target_language == "javascript":
            ...
            snippets.extend((match.group(1), match.start(1), match.end(1)) for
                match in re.finditer(pattern, content, re.DOTALL | re.
                IGNORECASE))
            event_pattern = 'on\\w+\\s*=\\s*["\\\']([^"\\\']+)["\\\']'
            snippets.extend((match.group(1), match.start(1), match.end(1)) for
                match in re.finditer(event_pattern, content))
            ...
            snippets.extend((match.group(1), match.start(1), match.end(1)) for
                match in re.finditer(style_pattern, content))
        elif host_language == "markdown" and target_language:
            pattern = f"```{target_language}\\n(.*?)```"
            snippets.extend((match.group(1), match.start(1), match.end(1)) for
                match in re.finditer(pattern, content, re.DOTALL))
        elif target_language == "sql":
            sql_patterns = ['["\\\'](\\s*SELECT\\s+.*?\\s+FROM\\s+.*?)["\\\']',
                '["\\\'](\\s*INSERT\\s+INTO\\s+.*?)["\\\']',
                '["\\\'](\\s*UPDATE\\s+.*?\\s+SET\\s+.*?)["\\\']',
                '["\\\'](\\s*DELETE\\s+FROM\\s+.*?)["\\\']']
            for pattern in sql_patterns:
                snippets.extend((match.group(1), match.start(1), match.end(
                    1)) for match in re.finditer(pattern, content, re.
            ...
            for pattern in patterns:
                snippets.extend((match.group(1), match.start(1), match.end(
                    1)) for match in re.finditer(pattern, content))
        return snippets

    @classmethod
    def cross_language_references(cls, chunks: list[CodeChunk]) -> list[
        CrossLanguageReference]:
        ...
        for chunk in chunks:
            if chunk.node_type in {"function_definition",
                "class_definition", "method_definition"}:
                name_match = re.search(r"(?:function|class|def)\\s+(\\w+)",
                    chunk.content)
                if name_match:
                ...
                for pattern in api_call_patterns:
                    for match in re.finditer(pattern, chunk.content):
                        ...
                        if endpoint in api_endpoints:
                            references.extend(CrossLanguageReference(
                                source_chunk=chunk, target_chunk=target_chunk, reference_type="api_call",
                                confidence=0.8) for target_chunk in
                                api_endpoints[endpoint] if target_chunk.
                                language != chunk.language)
            if chunk.node_type in {"interface_declaration",
                "type_alias_declaration", "struct_declaration",
                "class_definition"}:
                type_patterns = ["(?:interface|type|class|struct)\\s+(\\w+)",
                    "type\\s+(\\w+)\\s+struct"]
                type_name = None
                ...
                if type_name:
                    for other_chunk in chunks:
                        if (other_chunk != chunk and other_chunk.language !=
                            chunk.language) and other_chunk.node_type in {
                            "interface_declaration",
                            "type_alias_declaration", "struct_declaration",
                            "class_definition"}:
                            for pattern in type_patterns:
                                other_match = re.search(pattern,
                                    other_chunk.content)
                                if other_match and other_match.group(1,
                                    ) == type_name:
                                    references.append(CrossLanguageReference
                                        (source_chunk=chunk, target_chunk=other_chunk, reference_type="shared_type", confidence=0.6))
                                    break
            if "sql" in chunk.content.lower(
                ) or "query" in chunk.content.lower():
                table_patterns = ["FROM\\s+(\\w+)",
                    "INSERT\\s+INTO\\s+(\\w+)", "UPDATE\\s+(\\w+)",
                    "CREATE\\s+TABLE\\s+(\\w+)"]
                for pattern in table_patterns:
                    for match in re.finditer(pattern, chunk.content, re.
                        IGNORECASE):
                        table_name = match.group(1)
                        references.extend(CrossLanguageReference(
                            source_chunk=chunk, target_chunk=other_chunk,
                            reference_type="database_reference", confidence=0.5) for other_chunk in chunks if (other_chunk !=
                            chunk and table_name in other_chunk.content) and
                            other_chunk.language != chunk.language)
        return references

    def group_by_feature(self, chunks: list[CodeChunk]) -> dict[str, list[
        CodeChunk]]:
        ...
        for chunk in chunks:
            ...
            for i, part in enumerate(parts):
                if part in {"features", "modules", "components", "domains",
                    "services"} and i + 1 < len(parts):
        ...
        for chunk in chunks:
            if chunk.node_type in {"class_definition", "function_definition"}:
                name_match = re.search(r"(?:class|function|def)\\s+(\\w+)",
                    chunk.content)
                if name_match:
                    name = name_match.group(1)
                    base_name = re.sub(
        ...
        for ref in references:
            ...
            if (source_feature and target_feature and source_feature !=
                target_feature):
