...
class ChunkerConfig:
    ...
    def _parse_plugin_config(cls, config_dict: dict[str, Any]) -> PluginConfig:
        ...
        known_fields = {"enabled", "chunk_types", "min_chunk_size",
            "max_chunk_size"}
        custom_options = {key: value for key, value in config_dict.items() if
            key not in known_fields}
        return PluginConfig(enabled=enabled, chunk_types=chunk_types,
    ...
    def _prepare_save_data(self) -> dict[str, Any]:
        ...
        if self.default_plugin_config != PluginConfig():
            chunker["default_plugin_config"] = self._plugin_config_to_dict(self
    ...
    def create_example_config(cls, config_path: Path) -> None:
        ...
        example_data = {"chunker": {"plugin_dirs": ["./plugins",
            "~/.chunker/plugins"], "enabled_languages": ["python", "rust",
            "javascript", "c", "cpp"], "default_plugin_config": {
            "min_chunk_size": 3, "max_chunk_size": 500}}, "languages": {
            "python": {"enabled": True, "chunk_types": [
            "function_definition", "class_definition",
            "async_function_definition"], "include_docstrings": True},
            "rust": {"enabled": True, "chunk_types": ["function_item",
            "impl_item", "struct_item", "enum_item", "trait_item"]},
            "javascript": {"enabled": True, "chunk_types": [
            "function_declaration", "method_definition",
            "class_declaration", "arrow_function"], "include_jsx": True}}}
        config = cls()
    ...
    def _expand_env_vars(self, data: Any) -> Any:
        ...
        if isinstance(data, dict):
            return {key: self._expand_env_vars(value) for key, value in
                data.items()}
    ...
    def _apply_env_overrides(self) -> None:
        ...
        for env_var, value in os.environ.items():
            ...
            if config_path == "enabled_languages":
                ...
                logger.info("Set enabled_languages from env: %s", self.
                    enabled_languages)
                continue
            ...
            if len(path_parts) >= 2 and path_parts[0] == "languages":
                if len(path_parts) >= 3:
                    ...
                    if setting == "enabled":
                        self.plugin_configs[lang].enabled = value.lower(
                            ) == "true"
                        ...
                        self.plugin_configs[lang].chunk_types = set(value.
                            split(","))
                    else:
                        self.plugin_configs[lang].custom_options[setting
                            ] = value
                    logger.info("Set %s.%s from env: %s", lang, setting, value)
            elif len(path_parts) >= 2 and path_parts[0
                ] == "default" and path_parts[1] == "plugin" and path_parts[2
                ] == "config":
                ...
                logger.info("Set default_plugin_config.%s from env: %s",
                    setting, value)

    @classmethod
    def get_env_var_info(cls) -> dict[str, str]:
        """Get information about supported environment variables."""
        return {f"{cls.ENV_PREFIX}ENABLED_LANGUAGES":
            "Comma-separated list of enabled languages",
            f"{cls.ENV_PREFIX}PLUGIN_DIRS":
            "Comma-separated list of plugin directories",
            f"{cls.ENV_PREFIX}LANGUAGES_<LANG>_ENABLED":
            "Enable/disable specific language (true/false)",
            f"{cls.ENV_PREFIX}LANGUAGES_<LANG>_MIN_CHUNK_SIZE":
            "Minimum chunk size for language",
            f"{cls.ENV_PREFIX}LANGUAGES_<LANG>_MAX_CHUNK_SIZE":
            "Maximum chunk size for language",
            f"{cls.ENV_PREFIX}LANGUAGES_<LANG>_CHUNK_TYPES":
            "Comma-separated list of chunk types",
            f"{cls.ENV_PREFIX}DEFAULT_PLUGIN_CONFIG_MIN_CHUNK_SIZE":
            "Default minimum chunk size",
            f"{cls.ENV_PREFIX}DEFAULT_PLUGIN_CONFIG_MAX_CHUNK_SIZE":
