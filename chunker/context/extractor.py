...
class BaseContextExtractor(ContextExtractor):
        ...
        while current:
            if self._is_scope_node(current):
                ...
                if declaration:
                    ...
                    parent_contexts.append(ContextItem(
                ...
                if sibling == node:
                    ...
                    for j in range(i - 1, -1, -1):
                        ...
                        if self._is_decorator_node(prev_sibling):
                            content = source[prev_sibling.start_byte:prev_sibling.end_byte].decode('utf-8')
                            start_line = source[:prev_sibling.start_byte].count(b'\n') + 1
                            
                            decorators.append(ContextItem(
    ...
    def build_context_prefix(self, context_items: List[ContextItem], max_size: Optional[int] = None) -> str:
        ...
        for context_type in [ContextType.DECORATOR, ContextType.PARENT_SCOPE, 
                            ContextType.DEPENDENCY, ContextType.NAMESPACE, 
