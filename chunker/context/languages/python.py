...
class PythonContextExtractor(BaseContextExtractor):
    ...
    def _extract_scope_declaration(self, node: Node, source: bytes) -> Optional[str]:
        ...
        if node.type == 'function_definition':
            ...
            for child in node.children:
                if child.type == ':':
                    declaration = source[node.start_byte:child.end_byte].decode('utf-8').strip()
                    return declaration
    ...
    def _is_definition_context(self, identifier_node: Node) -> bool:
        ...
        if parent.type == 'aliased_import' or parent.type == 'dotted_name':
            if parent.parent and parent.parent.type in ('import_statement', 'import_from_statement'):
                return True
    ...
    def _find_definition(self, name: str, scope_node: Node, ast: Node, source: bytes) -> Optional[ContextItem]:
        ...
        def find_definition(node: Node, target_name: str) -> Optional[Node]:
...
class PythonSymbolResolver(BaseSymbolResolver):
    ...
    def _get_defined_name(self, node: Node) -> Optional[str]:
        ...
        elif node.type in ('typed_parameter', 'default_parameter', 'typed_default_parameter'):
            ...
            for child in import_node.children:
                if child.type == 'dotted_name':
                    ...
                    for subchild in child.children:
                        if subchild.type == 'identifier' and subchild.prev_sibling and subchild.prev_sibling.type == 'as':
                            # This is the alias
                            pass
