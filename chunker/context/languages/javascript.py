...
class JavaScriptContextExtractor(BaseContextExtractor):
    ...
    def _extract_type_declaration(self, node: Node, source: bytes) -> Optional[str]:
        ...
        if node.type == 'class_declaration':
            ...
            for child in node.children:
                if child.type == 'class_body':
                    ...
                    declaration = source[node.start_byte:child.start_byte].decode('utf-8').strip()
    ...
    def _extract_scope_declaration(self, node: Node, source: bytes) -> Optional[str]:
        ...
        if node.type == 'function_declaration':
            ...
            for child in node.children:
                if child.type in ('statement_block', '=>'):
                    end_byte = child.start_byte if child.type == 'statement_block' else child.end_byte
            ...
            if parent and parent.type in ('lexical_declaration', 'variable_declaration'):
                ...
                for child in node.children:
                    if child.type == '=':
                        ...
                        declaration = source[parent.start_byte:child.end_byte].decode('utf-8').strip()
                        return declaration + " ..."
    ...
    def _find_references_in_node(self, node: Node, source: bytes) -> List[tuple[str, Node]]:
    ...
    def _is_definition_context(self, identifier_node: Node) -> bool:
        ...
        if parent.type in ('variable_declarator', 'const_declaration', 'let_declaration'):
        ...
        if parent.type == 'property_identifier' or parent.type == 'shorthand_property_identifier':
            grandparent = parent.parent
    ...
    def _find_definition(self, name: str, scope_node: Node, ast: Node, source: bytes) -> Optional[ContextItem]:
        ...
        if def_node:
            ...
            elif def_node.type in ('function_declaration', 'function_expression', 
...
class JavaScriptSymbolResolver(BaseSymbolResolver):
    ...
    def _is_definition_node(self, node: Node) -> bool:
        ...
        return node.type in ('function_declaration', 'class_declaration', 
                           ...
                           'let_declaration', 'method_definition',
    ...
    def _get_defined_name(self, node: Node) -> Optional[str]:
