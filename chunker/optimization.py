...
class ChunkOptimizer(ChunkOptimizerInterface):
    ...
    def optimize_for_llm(self, chunks: List[CodeChunk], 
        ...
        original_tokens = [self.token_counter.count_tokens(chunk.content, model) for chunk in chunks]
        avg_tokens_before = sum(original_tokens) / len(original_tokens) if original_tokens else 0
        ...
        if strategy == OptimizationStrategy.AGGRESSIVE:
            ...
            optimized_chunks = self._aggressive_optimization(optimized_chunks, model, max_tokens)
        elif strategy == OptimizationStrategy.CONSERVATIVE:
            # Conservative: minimal changes, only split oversized chunks
            optimized_chunks = self._conservative_optimization(optimized_chunks, model, max_tokens)
        elif strategy == OptimizationStrategy.BALANCED:
            # Balanced: smart merging and splitting while preserving structure
            optimized_chunks = self._balanced_optimization(optimized_chunks, model, max_tokens)
            ...
            optimized_chunks = self._preserve_structure_optimization(optimized_chunks, model, max_tokens)
        
        # Calculate optimized metrics
        optimized_count = len(optimized_chunks)
        optimized_tokens = [self.token_counter.count_tokens(chunk.content, model) for chunk in optimized_chunks]
        avg_tokens_after = sum(optimized_tokens) / len(optimized_tokens) if optimized_tokens else 0
        ...
        if current_group:
            if merged_chunks and current_tokens < min_tokens:
                ...
                if last_tokens + current_tokens < min_tokens * 2:
                    ...
                    if not preserve_boundaries or self._can_merge_chunks(last_merged, current_group[0]):
                        # Merge with last chunk
                        merged_chunks[-1] = self._merge_chunk_group([last_merged] + current_group)
                    else:
    ...
    def optimize_for_embedding(self, chunks: List[CodeChunk],
        ...
        for chunk in chunks:
            chunk_tokens = self.token_counter.count_tokens(chunk.content, embedding_model)
            
            if chunk_tokens <= max_tokens:
                optimized.append(chunk)
            else:
                # Split into semantically meaningful sub-chunks
                sub_chunks = self._split_for_embedding(chunk, max_tokens, embedding_model)
    ...
    def _aggressive_optimization(self, chunks: List[CodeChunk], model: str, max_tokens: int) -> List[CodeChunk]:
    ...
    def _conservative_optimization(self, chunks: List[CodeChunk], model: str, max_tokens: int) -> List[CodeChunk]:
    ...
    def _balanced_optimization(self, chunks: List[CodeChunk], model: str, max_tokens: int) -> List[CodeChunk]:
        ...
        for group in chunk_groups:
            group_tokens = sum(self.token_counter.count_tokens(c.content, model) for c in group)
    ...
    def _preserve_structure_optimization(self, chunks: List[CodeChunk], model: str, max_tokens: int) -> List[CodeChunk]:
    ...
    def _structure_preserving_split(self, chunk: CodeChunk, max_tokens: int, model: str) -> List[CodeChunk]:
            ...
            if pos > start:
                ...
                if sub_content:
                    ...
                    if sub_tokens <= max_tokens:
                        ...
                        sub_chunks.extend(self._minimal_split(
    ...
    def _split_at_boundaries(self, chunk: CodeChunk, boundaries: List[int], max_tokens: int) -> List[CodeChunk]:
        ...
        for boundary_idx in line_boundaries:
            if boundary_idx > start_idx:
                ...
                if sub_content.strip():
                    sub_chunk = self._create_sub_chunk(chunk, sub_content, start_idx, boundary_idx - 1)
        ...
        if start_idx < len(lines):
            ...
            if sub_content.strip():
                sub_chunk = self._create_sub_chunk(chunk, sub_content, start_idx, len(lines) - 1)
                sub_chunks.append(sub_chunk)
    ...
    def _create_sub_chunk(self, parent: CodeChunk, content: str, start_line_offset: int, end_line_offset: int) -> CodeChunk:
    ...
    def _minimal_split(self, chunk: CodeChunk, max_tokens: int, model: str) -> List[CodeChunk]:
        ...
        if len(parts) > 1:
            ...
            for part in parts:
                ...
                if test_tokens <= max_tokens:
                    ...
                    if current_content:
                        sub_chunks.append(self._create_sub_chunk(
                            chunk, current_content, 0, 0  # Line numbers would need calculation
                        ))
                    current_content = part
            
            if current_content:
                sub_chunks.append(self._create_sub_chunk(
                    chunk, current_content, 0, 0
                ))
    ...
    def _identify_related_chunks(self, chunks: List[CodeChunk]) -> List[List[CodeChunk]]:
        ...
        for file_chunks in file_groups.values():
            ...
            for chunk in file_chunks:
                if not current_group:
                    current_group.append(chunk)
                elif self._are_chunks_related(current_group[-1], chunk):
                    current_group.append(chunk)
                else:
    ...
    def _split_for_embedding(self, chunk: CodeChunk, max_tokens: int, model: str) -> List[CodeChunk]:
            ...
            if split_pos > start:
                ...
                if sub_content:
                    ...
                    if sub_tokens <= max_tokens:
                        ...
                        sub_chunks.extend(self._token_based_split(
        ...
        if start < len(content):
            ...
            if sub_content:
                sub_chunks.append(self._create_embedding_chunk(
                    chunk, sub_content, 'remainder'
                ))
        
        return sub_chunks if sub_chunks else [chunk]
    
    def _create_embedding_chunk(self, parent: CodeChunk, content: str, chunk_type: str) -> CodeChunk:
    ...
    def _ensure_semantic_coherence(self, chunks: List[CodeChunk], model: str, max_tokens: int) -> List[CodeChunk]:
    ...
    def _calculate_coherence_score(self, chunks: List[CodeChunk]) -> float:
        ...
        for chunk in chunks:
            ...
            if re.search(r'^\s*\)', content):  # Starts with closing paren
                score *= 0.7
            if re.search(r'^\s*(else|elif|except|finally)', content):  # Orphaned control flow
                score *= 0.6
