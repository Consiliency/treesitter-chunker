...
class CrossCompiler:
    ...
    def _repair_wheel(
        ...
        cmd = [
            "docker", "run", "--rm",
            "-v", f"{wheel_path.parent}:/wheels",
            "-v", f"{output_dir}:/output",
            image,
            "auditwheel", "repair", f"/wheels/{wheel_path.name}",
            "-w", "/output"
        ]
        ...
        try:
            ...
            if result.returncode == 0:
                # Find repaired wheel
                repaired = list(output_dir.glob(f"*manylinux*.whl"))
                if repaired:
                    return repaired[0]
        except Exception:
            pass
    ...
    def get_supported_platforms(self) -> dict[str, dict[str, Any]]:
        ...
        return {
            ...
            "manylinux": {
                "versions": ["manylinux1", "manylinux2010", "manylinux2014", "manylinux_2_28"],
