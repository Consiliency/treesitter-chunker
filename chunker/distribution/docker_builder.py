...
class DockerBuilder:
    ...
    def build_image(self, tag: str, platforms: (list[str] | None) = None,
        dockerfile_path: (Path | None) = None) -> tuple[bool, str]:
        """
        ...
        try:
            subprocess.run([self.docker_cmd, "info"], capture_output=True,
                check=True)
    ...
    def _check_buildx(self) -> bool:
        ...
        if not self.docker_cmd:
            return False
        try:
            subprocess.run([self.docker_cmd, "buildx", "version"],
                capture_output=True, check=True)
            return True
        except (subprocess.CalledProcessError, FileNotFoundError):
            return False

    def _build_multiplatform(self, tag: str, platforms: list[str],
        dockerfile_path: Path) -> tuple[bool, str]:
        """Build multi-platform image using buildx"""
        try:
            builder_name = "treesitter-chunker-builder"
            inspect_result = subprocess.run([self.docker_cmd, "buildx",
                "inspect", builder_name], capture_output=True, check=False)
            if inspect_result.returncode != 0:
                subprocess.run([self.docker_cmd, "buildx", "create",
                    "--name", builder_name, "--use"], check=True)
            subprocess.run([self.docker_cmd, "buildx", "use", builder_name],
                check=True)
            platform_str = ",".join(platforms)
            build_cmd = [self.docker_cmd, "buildx", "build", "--platform",
                platform_str, "-t", tag, "-f", str(dockerfile_path), ".",
                "--push"]
    ...
    def _build_single_platform(self, tag: str, dockerfile_path: Path) -> tuple[
        bool, str]:
        """Build single platform image"""
        try:
            build_cmd = [self.docker_cmd, "build", "-t", tag, "-f", str(
                dockerfile_path), "."]
            subprocess.run(build_cmd, capture_output=True, text=True, check=True)
            inspect_result = subprocess.run([self.docker_cmd, "inspect",
                tag, "--format={{.Id}}"], capture_output=True, text=True,
                check=True)
    ...
    def verify_image(self, tag: str) -> tuple[bool, dict[str, Any]]:
        ...
        try:
            result = subprocess.run([self.docker_cmd, "inspect", tag],
                capture_output=True, text=True, check=True)
            image_info = json.loads(result.stdout)[0]
            return True, {"id": image_info["Id"], "created": image_info[
                "Created"], "size": image_info["Size"], "architecture":
                image_info["Architecture"], "os": image_info["Os"],
