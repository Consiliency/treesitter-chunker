...
class TreeSitterTokenAwareChunker(TokenAwareChunker):
    ...
    def chunk(self, ast: tree_sitter.Node, source: bytes, file_path: str, language: str) -> List[CodeChunk]:
        """
    ...
    def _split_large_chunk(self, chunk: CodeChunk, max_tokens: int, model: str) -> List[CodeChunk]:
        """
    ...
    def _split_class_chunk(self, chunk: CodeChunk, max_tokens: int, model: str) -> List[CodeChunk]:
        """
        ...
        for i, line in enumerate(lines):
            ...
            if any(pattern in stripped for pattern in ['def ', 'function ', 'func ', 'fn ', 'public ', 'private ', 'protected ']):
                # Check if it's indented (likely a method within the class)
                if line.startswith((' ', '\t')) and not line.startswith('        '):  # Not too deeply nested
        ...
        for i, start_idx in enumerate(method_starts):
            ...
            if current_tokens + method_tokens > max_tokens and current_part != class_header_lines:
    ...
    def _split_by_lines(self, chunk: CodeChunk, max_tokens: int, model: str) -> List[CodeChunk]:
        """Split a chunk by lines while trying to preserve logical groups."""
        text_parts = self.token_counter.split_text_by_tokens(chunk.content, max_tokens, model)
    ...
    def _create_sub_chunk(self, original_chunk: CodeChunk, content: str, index: int) -> CodeChunk:
        ...
        new_chunk = CodeChunk(
            ...
            metadata={
                "token_count": self.token_counter.count_tokens(content, original_chunk.metadata.get("tokenizer_model", "gpt-4")),
                "tokenizer_model": original_chunk.metadata.get("tokenizer_model", "gpt-4"),
                "is_split": True,
                "split_index": index + 1,
