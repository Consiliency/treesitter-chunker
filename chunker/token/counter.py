...
class TiktokenCounter(TokenCounter):
    ...
    def split_text_by_tokens(self, text: str, max_tokens: int, model: str = "gpt-4") -> List[str]:
        ...
        for line in lines:
            ...
            if len(line_tokens) > max_tokens:
                ...
                current_chunk_tokens = encoding.encode(line_chunks[-1]) if line_chunks[-1] else []
    ...
    def _split_long_line(self, line: str, max_tokens: int, encoding: tiktoken.Encoding) -> List[str]:
        ...
        for sentence in sentences:
            ...
            if len(sentence_tokens) > max_tokens:
                ...
                word_chunks = self._split_sentence_by_words(sentence, max_tokens, encoding)
                chunks.extend(word_chunks[:-1])
                current_chunk = [word_chunks[-1]] if word_chunks[-1] else []
                current_tokens = encoding.encode(word_chunks[-1]) if word_chunks[-1] else []
    ...
    def _split_sentence_by_words(self, sentence: str, max_tokens: int, encoding: tiktoken.Encoding) -> List[str]:
