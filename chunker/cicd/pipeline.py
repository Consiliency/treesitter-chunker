        ...
        try:
            ...
            if "on" in workflow:
                ...
                if not isinstance(on_section, dict | list | str):
                    errors.append(
                        "'on' section must be a string, list, or object")
        ...
        for version in python_versions:
            for platform in platforms:
                ...
                if platform == "windows-latest" and version == "3.8":
                    ...
                    tests_run = 150
                    tests_passed = 145
                    errors = ["test_windows_path_handling failed",
                ...
                results[key] = {"status": status, "tests_run": tests_run,
                    "tests_passed": tests_passed, "duration": round(
                    duration, 2), "errors": errors}
        return results

    @classmethod
    def build_distribution(cls, version: str, platforms: list[str]) -> dict[
        str, Any]:
        ...
        build_logs = {}
        for platform in platforms:
            platform_map = {"linux": "manylinux2014_x86_64", "darwin":
                "macosx_10_9_x86_64", "win32": "win_amd64"}
            wheel_platform = platform_map.get(platform, platform)
            wheel_name = (
                f"treesitter_chunker-{version}-py3-none-{wheel_platform}.whl")
            wheel_path = dist_dir / wheel_name
            wheel_path.write_text(
                f"# Dummy wheel for {platform}\nVersion: {version}\n")
            wheels.append(str(wheel_path))
            with wheel_path.open("rb") as f:
                checksum = hashlib.sha256(f.read()).hexdigest()
            checksums[wheel_name] = checksum
            build_logs[platform] = [f"Building wheel for {platform}...",
                f"Platform tag: {wheel_platform}", "Python: 3.8-3.11",
                f"Wheel created: {wheel_name}", f"Checksum: {checksum}",
                "Build successful"]
        ...
        return {"wheels": wheels, "sdist": str(sdist_path), "checksums":
            checksums, "build_logs": build_logs}

    @staticmethod
    def create_release(version: str, artifacts: list[Path], _changelog: str,
        ) -> dict[str, Any]:
        ...
        """
        if not version:
            return {"release_url": "", "tag": "", "uploaded_artifacts": [],
                "status": "failed"}
        version_to_validate = version[1:] if version.startswith("v",
            ) else version
        ...
        return {"release_url": release_url, "tag": tag,
            "uploaded_artifacts": uploaded_artifacts, "status": "published"}
