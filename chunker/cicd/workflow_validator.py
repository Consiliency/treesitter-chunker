...
class WorkflowValidator:
    ...
    VALID_RUNNERS: ClassVar[set[str]] = {"ubuntu-latest", "ubuntu-22.04",
        "ubuntu-20.04", "windows-latest", "windows-2022", "windows-2019",
        "macos-latest", "macos-13", "macos-12", "macos-11"}
    VALID_EVENTS: ClassVar[set[str]] = {"push", "pull_request",
        "workflow_dispatch", "schedule", "release", "issues",
        "issue_comment", "pull_request_review",
        "pull_request_review_comment", "workflow_call", "workflow_run",
        "repository_dispatch", "page_build", "project", "project_card",
        "project_column", "public", "watch", "fork", "create", "delete"}
    COMMON_ACTIONS: ClassVar[set[str]] = {"actions/checkout",
    ...
    def validate_file(self, workflow_path: Path) -> tuple[bool, list[str],
        list[str]]:
    ...
    def _validate_name(self, workflow: dict):
        ...
        if "name" not in workflow:
            ...
            self.warnings.append("Workflow name is very long (>255 characters)",
                )
    ...
    def _validate_pr_config(self, config: dict):
        ...
        if "types" in config:
            ...
            if not isinstance(types, list):
                ...
                valid_types = {"opened", "closed", "reopened",
                    "synchronize", "assigned", "unassigned", "labeled",
                    "unlabeled", "review_requested",
                    "review_request_removed", "ready_for_review",
                    "converted_to_draft"}
                ...
                for key in env:
                    if not isinstance(key, str):
                        ...
                        self.warnings.append(
                            f"Non-standard env var name: {key}")
    ...
    def _validate_job(self, job_id: str, job: dict):
        ...
        if "runs-on" not in job:
            self.errors.append(
                f"Job '{job_id}' missing required field: 'runs-on'")
        else:
            runner = job["runs-on"]
        ...
        if "steps" not in job:
            self.errors.append(
                f"Job '{job_id}' missing required field: 'steps'")
        else:
            steps = job["steps"]
            if not isinstance(steps, list):
                ...
                self.errors.append(
                    f"Job '{job_id}' must have at least one step")
            else:
        ...
        if "needs" in job:
            ...
            if isinstance(needs, str):
                ...
                self.errors.append(
                    f"Job '{job_id}' 'needs' must be string or list")
        if "strategy" in job:
    ...
    def _validate_step(self, job_id: str, step_index: int, step: dict):
        ...
        if not isinstance(step, dict):
            self.errors.append(
                f"Job '{job_id}' step {step_index} must be a dictionary")
            return
        step_name = step.get("name", f"step {step_index}")
        if "uses" not in step and "run" not in step:
            self.errors.append(
                f"Job '{job_id}' {step_name} must have 'uses' or 'run'")
    ...
    def _validate_strategy(self, job_id: str, strategy: dict):
        ...
        if "matrix" in strategy:
            ...
            if not isinstance(matrix, dict):
                self.errors.append(
                    f"Job '{job_id}' matrix must be a dictionary")
            else:
                dimensions = [k for k in matrix if k not in {"include",
                    "exclude"}]
                if not dimensions:
                    self.errors.append(
    ...
    def _validate_permissions(self, workflow: dict):
        ...
        if "permissions" in workflow:
            ...
            valid_scopes = {"actions", "checks", "contents", "deployments",
...
def validate_all_workflows(workflows_dir: Path) -> dict[str, tuple[bool,
    list[str], list[str]]]:
