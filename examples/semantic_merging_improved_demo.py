...
from chunker.semantic import TreeSitterSemanticMerger, TreeSitterRelationshipAnalyzer, MergeConfig
...
def demonstrate_targeted_merging():
    ...
    python_person_chunks = [c for c in chunks if c.language == "python" and "Person" in c.parent_context]
    java_calc_chunks = [c for c in chunks if c.language == "java"]
    other_chunks = [c for c in chunks if c not in python_person_chunks and c not in java_calc_chunks]
    ...
    for chunk in merged_python:
        methods = [line.strip() for line in chunk.content.split('\n') if line.strip().startswith('def ')]
        if len(methods) > 1:
            print(f"    - Merged: {', '.join(m.split('(')[0].replace('def ', '') for m in methods)}")
        else:
...
def demonstrate_configuration_impact():
    ...
    for name, config in configs:
        ...
        merged_count = sum(1 for c in merged if "def " in c.content and c.content.count("def ") > 1)
