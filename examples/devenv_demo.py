...
def demo_pre_commit_setup():
    ...
    if (project_root / ".git").exists() and (project_root / ".pre-commit-config.yaml").exists():
...
""")
    ...
    for issue in issues[:5]:  # Show first 5 issues
        print(f"  - [{issue['tool']}] {issue['file']}:{issue['line']}:{issue['column']} "
...
def demo_formatting():
    ...
    with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as f:
        f.write("""def poorly_formatted(x,y,z):
    result=x+y+z
    if result>10:
        return True
    else:
...
""")
    ...
    success, files = dev_env.format_code([format_file], check_only=True)
    print(f"\nFormatting check: {'passed' if success else 'needs formatting'}")
    
    if not success:
        # Apply formatting
        success, modified = dev_env.format_code([format_file], check_only=False)
...
def demo_quality_checks():
    ...
    if "error" in type_report:
        ...
        print(f"Meets minimum (80%): {'✓' if type_report.get('meets_minimum') else '✗'}")
        
        if "files" in type_report and type_report["files"]:
    ...
    if "error" in test_report:
        ...
        print(f"Meets minimum (80%): {'✓' if test_report.get('meets_minimum') else '✗'}")
