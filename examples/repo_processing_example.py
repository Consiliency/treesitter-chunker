...
def basic_repo_processing():
    ...
    if result.file_results:
        ...
        for i, chunk in enumerate(first_file.chunks[:3]):
            print(f"  Chunk {i+1}: {chunk.chunk_type} '{chunk.name}' (lines {chunk.start_line}-{chunk.end_line})")
...
def memory_efficient_processing():
    ...
    for file_result in processor.process_files_iterator("."):
        ...
        if file_count <= 3:
            print(f"Processing: {file_result.file_path} ({len(file_result.chunks)} chunks)")
...
def parallel_processing():
    ...
    print(f"Single-threaded: {time_single:.2f}s for {result_single.total_chunks} chunks")
    print(f"Multi-threaded (4 workers): {time_multi:.2f}s for {result_multi.total_chunks} chunks")
