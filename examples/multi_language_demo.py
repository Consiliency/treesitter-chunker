...
'''
    ...
    for name, content in samples.items():
        ...
        if len(languages) > 1:
            print(f"  Multiple languages detected: {', '.join(f'{l}({p:.1%})' for l, p in languages)}")
...
def demo_project_analysis():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        api_dir = backend / "api"
        api_dir.mkdir()
        (api_dir / "users.py").write_text("from flask import Blueprint\n\nusers_bp = Blueprint('users', __name__)")
        (api_dir / "auth.py").write_text("from flask import Blueprint\n\nauth_bp = Blueprint('auth', __name__)")
        
        # Frontend structure
        frontend = project_root / "frontend"
        frontend.mkdir()
        (frontend / "package.json").write_text('{"name": "my-app", "dependencies": {"react": "^18.0.0"}}')
        
        src_dir = frontend / "src"
        src_dir.mkdir()
        (src_dir / "App.tsx").write_text("import React from 'react';\n\nexport const App = () => <div>Hello</div>;")
        (src_dir / "index.js").write_text("import { App } from './App';\n\nReactDOM.render(<App />, document.getElementById('root'));")
        
        # Tests
        tests = project_root / "tests"
        tests.mkdir()
        (tests / "test_api.py").write_text("import pytest\n\ndef test_users_endpoint():\n    pass")
