...
def print_hierarchy(hierarchy, navigator, indent_size=2):
    ...
    def print_node(chunk_id, level=0):
        ...
        print(f"{indent}├─ {chunk.node_type}: {chunk.content.split()[0][:30]}... (line {chunk.start_line})")
...
'''
    ...
    for cls in classes:
        ...
        for method in methods:
            print(f"      - {method.content.split()[1].rstrip('('):} at line {method.start_line}")
    ...
    process_text = next((c for c in chunks if "process_text" in c.content and c.node_type == "function_definition"), None)
    ...
    if classes:
        ...
        print(f"Subtree rooted at '{main_class.content.split()[1]}' contains {len(subtree.chunk_map)} chunks")
