...
def main():
    ...
    for dep_chunk, dep_meta in dependencies[:3]:  # Show top 3
        print(f"  - {dep_chunk.node_type} at line {dep_chunk.start_line} "
    ...
    for usage_chunk, usage_meta in usages[:3]:  # Show top 3
        print(f"  - {usage_chunk.node_type} at line {usage_chunk.start_line} "
    ...
    for struct_chunk, struct_meta in structural[:3]:  # Show top 3
        print(f"  - {struct_chunk.node_type} at line {struct_chunk.start_line} "
    ...
    selected_by_relevance = relevance_strategy.select_context(
        target_chunk, all_candidates, max_tokens=1000
    )
