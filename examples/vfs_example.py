...
from chunker import (
    InMemoryFileSystem, ZipFileSystem, CompositeFileSystem,
    VFSChunker, chunk_from_url, chunk_from_zip,
    optimized_gc
)
...
""")
    ...
    for chunk in chunks:
        chunk_count += 1
        if chunk_count <= 3:
            print(f"  Chunk {chunk_count}: {chunk.node_type} at line {chunk.start_line}")
