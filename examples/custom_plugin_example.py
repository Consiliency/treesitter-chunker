...
from chunker import get_plugin_manager, CodeChunk
...
class GoPlugin(LanguagePlugin):
    ...
    def process_node(
        ...
        if (node.type == "function_declaration" and 
            not self.config.custom_options.get("include_tests", True)):
            ...
            if chunk:
                ...
                for child in node.children:
                    if child.type == "parameter_list":
                        receiver_info = source[child.start_byte:child.end_byte].decode('utf-8')
...
def demonstrate_custom_plugins():
    ...
    go_config = PluginConfig(
        chunk_types={"function_declaration", "method_declaration", "interface_declaration"},
        min_chunk_size=2,
