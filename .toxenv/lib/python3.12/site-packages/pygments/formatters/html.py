    ...
    **Subclassing the HTML formatter**
    ...
    def get_background_style_defs(self, arg=None):
        ...
        if hl_color is not None:
            lines.insert(
                0,
                "{} {{ background-color: {} }}".format(prefix("hll"), hl_color),
            )
    ...
    def _wrap_full(self, inner, outfile):
        if self.cssfile:
            if pathlib.Path(self.cssfile).is_absolute():
                ...
                try:
                    ...
                    cssfilename = os.path.join(
                        pathlib.Path(filename).parent, self.cssfile
                    )
            ...
            try:
                if not pathlib.Path(cssfilename).exists() or not self.noclobber_cssfile:
