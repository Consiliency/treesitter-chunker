...
class ScriptMaker:
    ...
    def get_manifest(self, exename):
        base = pathlib.Path(exename).name
        ...
        for name in names:
            ...
            if use_launcher:  # pragma: no cover
                ...
                try:
                    ...
                    if pathlib.Path(dfname).exists():
                        pathlib.Path(dfname).unlink()  # Not allowed to fail here
                    pathlib.Path(outname).rename(dfname)  # nor here
                    self._fileop.write_binary_file(outname, script_bytes)
                    logger.debug("Able to replace executable using " ".deleteme logic")
                    try:
                        pathlib.Path(dfname).unlink()
                    except Exception:
                        pass  # still in use - ignore error
            else:
                if self._is_nt and not outname.endswith("." + ext):  # pragma: no cover
                    outname = "%s.%s" % (outname, ext)
                if pathlib.Path(outname).exists() and not self.clobber:
    ...
    def _copy_script(self, script, filenames):
        ...
        outname = os.path.join(self.target_dir, pathlib.Path(script).name)
