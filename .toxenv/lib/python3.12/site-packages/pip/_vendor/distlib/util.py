...
class FileOperator:
    ...
    def newer(self, source, target):
        ...
        if not pathlib.Path(source).exists():
            raise DistlibException(
                "file '%r' does not exist" % pathlib.Path(source).resolve(),
            )
    ...
    def copy_file(self, infile, outfile, check=True):
        ...
        self.ensure_dir(pathlib.Path(outfile).parent)
        logger.info("Copying %s to %s", infile, outfile)
        if not self.dry_run:
            msg = None
            if check:
                if pathlib.Path(outfile).is_symlink():
                    msg = "%s is a symlink" % outfile
                elif (
                    pathlib.Path(outfile).exists()
                    and not pathlib.Path(outfile).is_file()
                ):
    ...
    def copy_stream(self, instream, outfile, encoding=None):
        assert not pathlib.Path(outfile).is_dir()
        self.ensure_dir(pathlib.Path(outfile).parent)
    ...
    def write_binary_file(self, path, data):
        self.ensure_dir(pathlib.Path(path).parent)
        if not self.dry_run:
        ...
        if os.name == "posix" or (os.name == "java" and os._name == "posix"):
            ...
            for f in files:
                if self.dry_run:
                    ...
                    pathlib.Path(f).chmod(mode)

    set_executable_mode = lambda s, f: s.set_mode(0o555, 0o7777, f)

    def ensure_dir(self, path):
        path = pathlib.Path(path).resolve()
        if path not in self.ensured and not pathlib.Path(path).exists():
    ...
    def ensure_removed(self, path):
        if pathlib.Path(path).exists():
            if pathlib.Path(path).is_dir() and not pathlib.Path(path).is_symlink():
        ...
        if not self.dry_run:
            ...
            for d in dirs:
                ...
                if flist:
                    ...
                    pathlib.Path(sd).rmdir()
                pathlib.Path(d).rmdir()  # should fail if non-empty
        self._init_record()
...
def get_cache_base(suffix=None):
    ...
    if os.name == "nt" and "LOCALAPPDATA" in os.environ:
        ...
        result = pathlib.Path("~").expanduser()
    # we use 'isdir' instead of 'exists', because we want to
    # fail if there's a file with that name
    if pathlib.Path(result).is_dir():
...
def path_to_cache_dir(path, use_abspath=True):
    ...
    #. ``'.cache'`` is appended.
    """
    d, p = os.path.splitdrive(pathlib.Path(path).resolve() if use_abspath else path)
    if d:
        d = d.replace(":", "---")
    p = p.replace(os.sep, "--")
    return d + p + ".cache"
...
NAME_VERSION_RE = re.compile(r"(?P<name>[\w .-]+)\s*\(\s*(?P<ver>[^\s)]+)\)$")
...
class Cache:
    ...
    def __init__(self, base):
        ...
        # we use 'isdir' instead of 'exists', because we want to
        # fail if there's a file with that name
        if not pathlib.Path(base).is_dir():  # pragma: no cover
            os.makedirs(base)
        if (os.stat(base).st_mode & 0o77) != 0:
            logger.warning("Directory '%s' is not private", base)
        self.base = pathlib.Path(os.path.normpath(base)).resolve()
    ...
    def clear(self):
        ...
        for fn in os.listdir(self.base):
            ...
            try:
                if pathlib.Path(fn).is_symlink() or pathlib.Path(fn).is_file():
                    pathlib.Path(fn).unlink()
                elif pathlib.Path(fn).is_dir():
                    shutil.rmtree(fn)
            except Exception:
...
def unarchive(archive_filename, dest_dir, format=None, check=True):
    ...
    def check_path(path):
        ...
        p = pathlib.Path(os.path.join(dest_dir, path)).resolve()
...
class PyPIRCFile:
    ...
    def __init__(self, fn=None, url=None):
        if fn is None:
            fn = os.path.join(pathlib.Path("~").expanduser(), ".pypirc")
        self.filename = fn
        self.url = url
