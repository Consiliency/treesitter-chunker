...
def parse_editable(editable_req: str) -> tuple[str | None, str, set[str]]:
    ...
    if pathlib.Path(url_no_extras).is_dir():
...
def _get_url_from_path(path: str, name: str) -> str | None:
    ...
    """
    if _looks_like_path(name) and pathlib.Path(path).is_dir():
...
def parse_req_from_line(name: str, line_source: str | None) -> RequirementParts:
    ...
    name = name.strip()
    req_as_string = None
    path = os.path.normpath(pathlib.Path(name).resolve())
    link = None
    ...
    if link:
        ...
        if link.scheme == "file" and re.search(r"\.\./", link.url):
            link = Link(
                path_to_url(os.path.normpath(pathlib.Path(link.path).resolve()))
            )
        # wheel file
        if link.is_wheel:
