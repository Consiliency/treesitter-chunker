...
from typing import IO, Any, BinaryIO, NewType, Protocol, Union, cast
...
def message_about_scripts_not_on_PATH(scripts: Sequence[str]) -> str | None:
    ...
    for destfile in scripts:
        parent_dir = pathlib.Path(destfile).parent
        script_name = pathlib.Path(destfile).name
    ...
    not_warn_dirs.append(
        os.path.normcase(os.path.normpath(pathlib.Path(sys.executable).parent)),
    )
...
class ZipBackedFile:
    ...
    def save(self) -> None:
        ...
        if pathlib.Path(self.dest_path).exists():
            pathlib.Path(self.dest_path).unlink()
...
def _install_wheel(
    ...
    for file in files:
        ...
        # uninstalled.
        parent_dir = pathlib.Path(file.dest_path).parent
    ...
    def pyc_source_file_paths() -> Generator[str, None, None]:
        ...
        for installed_path in sorted(set(installed.values())):
            ...
            if not pathlib.Path(full_installed_path).is_file():
                continue
        ...
        with (
            ...
            for path in pyc_source_file_paths():
                ...
                if success:
                    ...
                    assert pathlib.Path(pyc_path).exists()
                    pyc_record_path = cast(
                        "RecordPath",
                        pyc_path.replace(os.path.sep, "/"),
                    )
    ...
    def _generate_file(path: str, **kwargs: Any) -> Generator[BinaryIO, None, None]:
        with adjacent_tmp_file(path, **kwargs) as f:
            yield f
        pathlib.Path(f.name).chmod(generated_file_mode)
        replace(f.name, path)
