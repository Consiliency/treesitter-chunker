...
class Downloader:
    ...
    def _attempt_resumes_or_redownloads(
        ...
        if download.is_incomplete():
            pathlib.Path(download.output_file.name).unlink()
    ...
    def _cache_resumed_download(
        ...
        if not isinstance(adapter, CacheControlAdapter):
            logger.debug(
                "Skipping resume download caching: no cache controller for %s",
                url,
            )
            return
        ...
        logger.debug(
            "Cached resumed download as complete response for future use: %s",
            url,
        )
