...
class InstallCommand(RequirementCommand):
    ...
    def run(self, options: Values, args: list[str]) -> int:
        ...
        if options.target_dir:
            ...
            options.target_dir = Path(options.target_dir).resolve()
            if (
                # fmt: off
                Path(options.target_dir).exists() and
                not Path(options.target_dir).is_dir()
                # fmt: on
            ):
                raise CommandError(
    ...
    def _handle_target_dir(
        ...
        if Path(purelib_dir).exists():
            lib_dir_list.append(purelib_dir)
        if Path(platlib_dir).exists() and platlib_dir != purelib_dir:
        ...
        for lib_dir in lib_dir_list:
            for item in os.listdir(lib_dir):
                ...
                if Path(target_item_dir).exists():
                    if not upgrade:
                        logger.warning(
                        ...
                        continue
                    if Path(target_item_dir).is_symlink():
                        logger.warning(
                        ...
                        continue
                    if Path(target_item_dir).is_dir():
                        shutil.rmtree(target_item_dir)
                    else:
                        Path(target_item_dir).unlink()
