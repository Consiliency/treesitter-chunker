...
def get_config_path(config_level: Lit_config_levels) -> str:
    ...
    if config_level == "user":
        ...
        return osp.normpath(
            pathlib.Path(osp.join(config_home, "git", "config")).expanduser(),
        )
    if config_level == "global":
        return osp.normpath(pathlib.Path("~/.gitconfig").expanduser())
...
class GitConfigParser(cp.RawConfigParser, metaclass=MetaParserBuilder):
    ...
    def __init__(
        ...
        if file_or_files is not None:
            self._file_or_files: PathLike | BytesIO | Sequence[PathLike | BytesIO] = (
                file_or_files
            )
            ...
            if self._has_includes():
                for _, include_path in self._included_paths():
                    if include_path.startswith("~"):
                        include_path = pathlib.Path(include_path).expanduser()
                    if not pathlib.Path(include_path).is_absolute():
                        if not file_ok:
                            continue
                        ...
                        include_path = osp.join(
                            pathlib.Path(file_path).parent, include_path,
                        )
    ...
    def set_value(
        self, section: str, option: str, value: str | bytes | int | float | bool,
    ) -> "GitConfigParser":
