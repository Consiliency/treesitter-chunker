    ...
    def pump_stream(
        ...
        try:
            ...
            if "I/O operation on closed file" not in str(ex):
                ...
                raise CommandError(
                    [f"<{name}-pump>"] + remove_password_if_present(cmdline),
                    ex,
                ) from ex
        finally:
            stream.close()
...
class Git(metaclass=_GitMeta):
    ...
    * https://learn.microsoft.com/en-us/windows/win32/api/processthreadsapi/nf-processthreadsapi-createprocessw
    ...
    def refresh(cls, path: None | PathLike = None) -> bool:
        ...
        if path is not None:
            new_git = pathlib.Path(path).expanduser()
            new_git = pathlib.Path(new_git).resolve()
        else:
    ...
    def polish_url(cls, url: str, is_cygwin: None | bool = None) -> PathLike:
        ...
        if is_cygwin:
            ...
            if url.startswith("~"):
                url = pathlib.Path(url).expanduser()
    ...
    def check_unsafe_options(
        ...
        for option in options:
            for unsafe_option, bare_option in zip(
                unsafe_options,
                bare_unsafe_options,
                strict=False,
            ):
    ...
    def execute(
        ...
        **subprocess_kwargs: Any,
        ...
        try:
            cwd = self._working_dir or pathlib.Path.cwd()  # type: Union[None, str]
        ...
        if self.GIT_PYTHON_TRACE == "full":
            ...
            def as_text(stdout_value: bytes | str) -> str:
                return (
                    not output_stream and safe_decode(stdout_value)
                ) or "<OUTPUT_STREAM>"
    ...
    def transform_kwargs(
        self,
        split_single_char_options: bool = True,
        **kwargs: Any,
    ) -> list[str]:
