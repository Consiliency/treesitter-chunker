...
def _read_env_flag(name: str, default: bool) -> bool:
    ...
    if adjusted_value in {"1", "true", "yes"}:
        return True
    _logger.warning(
        "%s has unrecognized value %r, treating as %r.",
        name,
        value,
        default,
    )
...
def rmtree(path: PathLike) -> None:
    ...
    def handler(function: Callable, path: PathLike, _excinfo: Any) -> None:
        ...
        pathlib.Path(path).chmod(stat.S_IWUSR)
...
def _cygexpath(drive: str | None, path: str) -> str:
    if pathlib.Path(path).is_absolute() and not drive:
        # Invoked from `cygpath()` directly with `D:Apps\123`?
        #  It's an error, leave it alone just slashes)
        p = path  # convert to str if AnyPath given
    else:
        p = path and osp.normpath(osp.expandvars(pathlib.Path(path).expanduser()))
...
def _is_cygwin_git(git_executable: str) -> bool:
    ...
    if is_cygwin is None:
        is_cygwin = False
        try:
            git_dir = pathlib.Path(git_executable).parent
            if not git_dir:
...
def is_cygwin_git(git_executable: None | PathLike) -> bool:
    ...
    if sys.platform != "cygwin" or git_executable is None:
        return False
...
def finalize_process(
    proc: Union[subprocess.Popen, "Git.AutoInterrupt"],
    **kwargs: Any,
) -> None:
...
def expand_path(p: None | PathLike, expand_vars: bool = True) -> PathLike | None:
    if isinstance(p, pathlib.Path):
        return p.resolve()
    try:
        p = pathlib.Path(p).expanduser()  # type: ignore[arg-type]
        if expand_vars:
            p = osp.expandvars(p)
        return osp.normpath(pathlib.Path(p).resolve())
    except Exception:
        return None
...
class Actor:
    ...
    def committer(
        ...
        return cls._main_actor(
            cls.env_committer_name,
            cls.env_committer_email,
            config_reader,
        )
...
class LockFile:
    ...
    def _obtain_lock_or_raise(self) -> None:
        ...
        if pathlib.Path(lock_file).is_file():
            raise OSError(
    ...
    def _obtain_lock(self) -> None:
        ...
        while True:
            try:
                ...
                curtime = time.time()
                if not pathlib.Path(
                    pathlib.Path(self._lock_file_path()).parent,
                ).is_dir():
                    msg = (
