...
class Submodule(IndexObject, TraversableIterableObj):
    ...
    __slots__ = ("__weakref__", "_branch_path", "_name", "_parent_commit", "_url")
    ...
    def _config_parser(
        cls,
        repo: "Repo",
        parent_commit: Union["Commit", None],
        read_only: bool,
    ) -> SubmoduleConfigParser:
        """
        :return:
    ...
    def _clone_repo(
        ...
        **kwargs: Any,
        ...
        if cls._need_gitfile_submodules(repo.git):
            ...
            module_abspath_dir = pathlib.Path(module_abspath).parent
            if not pathlib.Path(module_abspath_dir).is_dir():
    ...
    def _to_relative_path(cls, parent_repo: "Repo", path: PathLike) -> PathLike:
        ...
        if pathlib.Path(path).is_absolute() and parent_repo.working_tree_dir:
    ...
    def _write_git_file_and_module_config(
        cls,
        working_tree_dir: PathLike,
        module_abspath: PathLike,
    ) -> None:
        ...
        if sys.platform == "win32" and pathlib.Path(git_file).is_file():
            pathlib.Path(git_file).unlink()
    ...
    def update(
        ...
        try:
            ...
            try:
                ...
                if not dry_run and pathlib.Path(checkout_module_abspath).is_dir():
                    try:
                        pathlib.Path(checkout_module_abspath).rmdir()
                    except OSError as e:
                        raise OSError(
                ...
                if mrepo.head.commit.binsha != self.NULL_BIN_SHA:
                    ...
                    if (
                        ...
                        raise RepositoryDirtyError(
                            mrepo,
                            "Cannot reset a dirty repository",
                        )
    ...
    def move(
        ...
        if pathlib.Path(module_checkout_abspath).is_file():
            raise ValueError(
        ...
        if module:
            if pathlib.Path(module_checkout_abspath).exists():
                if len(os.listdir(module_checkout_abspath)):
                    raise ValueError("Destination module directory was not empty")
                # END handle non-emptiness

                if pathlib.Path(module_checkout_abspath).is_symlink():
                    pathlib.Path(module_checkout_abspath).unlink()
                else:
                    pathlib.Path(module_checkout_abspath).rmdir()
                # END handle link
            else:
        ...
        if module and pathlib.Path(cur_path).exists():
            os.renames(cur_path, module_checkout_abspath)
            renamed_module = True

            if pathlib.Path(osp.join(module_checkout_abspath, ".git")).is_file():
    ...
    def remove(
        ...
        if module and self.module_exists():
            ...
            if force:
                ...
                if pathlib.Path(mp).is_symlink():
                    method = os.remove
                elif pathlib.Path(mp).is_dir():
                    method = rmtree
                elif pathlib.Path(mp).exists():
            ...
            if not dry_run and pathlib.Path(git_dir).is_dir():
                self._clear_cache()
                rmtree(git_dir)
    ...
    def config_writer(
        self,
        index: Union["IndexFile", None] = None,
        write: bool = True,
