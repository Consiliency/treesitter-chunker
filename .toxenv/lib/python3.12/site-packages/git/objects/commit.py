...
# typing ------------------------------------------------------------------
from typing import IO, TYPE_CHECKING, Any, Dict, List, Tuple, Union, cast
...
class Commit(base.Object, TraversableIterableObj, Diffable, Serializable):
    ...
    def iter_parents(
        self, paths: PathLike | Sequence[PathLike] = "", **kwargs: Any
    ) -> Iterator["Commit"]:
    ...
    def stats(self) -> Stats:
        ...
        def process_lines(lines: list[str]) -> str:
            text = ""
            for file_info, line in zip(lines, lines[len(lines) // 2 :], strict=False):
    ...
    def _iter_from_process_or_stream(
        cls, repo: "Repo", proc_or_stream: Popen | IO
    ) -> Iterator["Commit"]:
