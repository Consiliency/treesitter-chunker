...
__all__ = ["INDEX", "NULL_TREE", "Diff", "DiffConstants", "DiffIndex", "Diffable"]
...
class DiffIndex(list[T_Diff]):
    ...
    * ``T`` = Changed in the type
    ...
    def iter_change_type(self, change_type: Lit_change_type) -> Iterator[T_Diff]:
        ...
        for diffidx in self:
            if (
                diffidx.change_type == change_type
                or (change_type == "A" and diffidx.new_file)
                or (change_type == "D" and diffidx.deleted_file)
                or (change_type == "C" and diffidx.copied_file)
                or (change_type == "R" and diffidx.renamed_file)
                or (
                    change_type == "M"
                    and diffidx.a_blob
                    and diffidx.b_blob
...
class Diff:
    ...
    def __str__(self) -> str:
        ...
        line = None
        line_length = 0
        for b, n in zip((self.a_blob, self.b_blob), ("lhs", "rhs"), strict=False):
            if b:
    ...
    def _index_from_patch_format(
        cls,
        repo: "Repo",
        proc: Union["Popen", "Git.AutoInterrupt"],
    ) -> DiffIndex["Diff"]:
