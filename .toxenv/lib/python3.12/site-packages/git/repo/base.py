...
class Repo:
    ...
    * :attr:`git_dir` is the ``.git`` repository directory, which is always set.
    ...
    def __init__(
        ...
        epath = epath or path or Path.cwd()
        ...
        while curpath:
            ...
            if is_git_dir(curpath):
                ...
                # working tree.
                self._working_tree_dir = Path(git_dir).parent
    ...
    def _get_config_path(
        self,
        config_level: Lit_config_levels,
        ...
        if config_level == "user":
            ...
            return osp.normpath(
                Path(osp.join(config_home, "git", "config")).expanduser(),
            )
        if config_level == "global":
            return osp.normpath(Path("~/.gitconfig").expanduser())
    ...
    def config_writer(
        self,
        config_level: Lit_config_levels = "repository",
    ) -> GitConfigParser:
        """
        :return:
    ...
    def _get_alternates(self) -> list[str]:
        ...
        if Path(alternates_path).exists():
    ...
    def _set_alternates(self, alts: list[str]) -> None:
        ...
        if not alts:
            if Path(alternates_path).is_file():
                Path(alternates_path).unlink()
        else:
    ...
    def is_dirty(
        ...
        if index:
            ...
            if Path(self.index.path).is_file() and len(
    ...
    def init(
        ...
        **kwargs: Any,
        ...
        if mkdir and path and not Path(path).exists():
    ...
    def clone_from(
        ...
        **kwargs: Any,
        ...
        """
        git = cls.GitCommandWrapperType(Path.cwd())
        if env is not None:
    ...
    def has_separate_working_tree(self) -> bool:
        ...
        if self.working_tree_dir:
            return Path(osp.join(self.working_tree_dir, ".git")).is_file()
    ...
    def currently_rebasing_on(self) -> Commit | None:
        ...
        if not Path(rebase_head_file).is_file():
            return None
