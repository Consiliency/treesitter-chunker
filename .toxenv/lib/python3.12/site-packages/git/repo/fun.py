...
def is_git_dir(d: PathLike) -> bool:
    ...
    if Path(d).is_dir():
        if (
            Path(osp.join(d, "objects")).is_dir()
            or "GIT_OBJECT_DIRECTORY" in os.environ
        ) and Path(osp.join(d, "refs")).is_dir():
            headref = osp.join(d, "HEAD")
            return Path(headref).is_file() or (
                Path(headref).is_symlink()
                and Path(headref).readlink().startswith("refs")
            )
        if (
            Path(osp.join(d, "gitdir")).is_file()
            and Path(osp.join(d, "commondir")).is_file()
            and Path(osp.join(d, "gitfile")).is_file()
        ):
    ...
    try:
        ...
        if content.startswith("gitdir: "):
            ...
            if not Path(path).is_absolute():
                path = osp.normpath(osp.join(Path(d).parent, path))
