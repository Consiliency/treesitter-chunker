...
class RefLogEntry(tuple[str, str, Actor, tuple[int, int], str]):
    ...
    def format(self) -> str:
        ...
        act = self.actor
        time = self.time
        return f"{self.oldhexsha} {self.newhexsha} {act.name} <{act.email}> {time[0]!s} {altz_to_utctz_str(time[1])}\t{self.message}\n"
    ...
    def from_line(cls, line: bytes) -> "RefLogEntry":
        ...
        if len(fields) == 1:
            ...
            raise ValueError(
                "Line must have up to two TAB-separated fields. Got %s"
                % repr(line_str),
            )
