...
# typing ------------------------------------------------------------------
from typing import TYPE_CHECKING, Any, List, Tuple, Type, TypeVar, Union, cast
...
class SymbolicReference:
    ...
    def _check_ref_name_valid(ref_path: PathLike) -> None:
        ...
        if previous == "@" and one_before_previous is None:
            raise ValueError(
    ...
    def log_append(
        ...
        try:
            committer_or_reader: Actor | GitConfigParser = self.commit.committer
        except ValueError:
    ...
    def delete(cls, repo: "Repo", path: PathLike) -> None:
                ...
                with open(pack_file_path, "rb") as reader:
                    ...
                    for line_bytes in reader:
                        ...
                        if (line.startswith("#") or full_ref_path != line_ref) and (
                            ...
                            or (dropped_last_line and not line.startswith("^"))
                        ):
        ...
        if pathlib.Path(reflog_path).is_file():
            pathlib.Path(reflog_path).unlink()
        # END remove reflog
    ...
    def _create(
        ...
        if not force and pathlib.Path(abs_ref_path).is_file():
    ...
    def rename(self, new_path: PathLike, force: bool = False) -> "SymbolicReference":
        ...
        if pathlib.Path(new_abs_path).is_file():
            if not force:
            ...
            pathlib.Path(new_abs_path).unlink()
        # END handle existing target file

        dname = pathlib.Path(new_abs_path).parent
        if not pathlib.Path(dname).is_dir():
            os.makedirs(dname)
        # END create directory

        pathlib.Path(cur_abs_path).rename(new_abs_path)
        self.path = new_path
