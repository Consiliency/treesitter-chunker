...
__all__ = ["FetchInfo", "PushInfo", "Remote", "RemoteProgress"]
...
class PushInfo(IterableObj):
    ...
    def old_commit(self) -> Union["Commit", None]:
        return (
            self._old_commit_sha and self._remote.repo.commit(self._old_commit_sha)
        ) or None
    ...
    def remote_ref(self) -> RemoteReference | TagReference:
        ...
        if self.remote_ref_string.startswith("refs/heads"):
...
class FetchInfo(IterableObj):
    ...
    __slots__ = ("flags", "note", "old_commit", "ref", "remote_ref_path")
...
class Remote(LazyMixin, IterableObj):
    ...
    __slots__ = ("_config_reader", "name", "repo")
    ...
    def _get_fetch_info_from_stderr(
        ...
        for err_line, fetch_line in zip(
            fetch_info_lines, fetch_head_info, strict=False
        ):
            try:
