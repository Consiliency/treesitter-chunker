...
__all__ = ["BaseIndexEntry", "BlobFilter", "IndexEntry", "StageType"]
...
# typing ----------------------------------------------------------------------
from typing import TYPE_CHECKING, NamedTuple, Tuple, Union, cast
    ...
    def __call__(self, stage_blob: tuple[StageType, Blob]) -> bool:
        ...
        for pathlike in self.paths:
            ...
            if len(filter_parts) > len(blob_parts):
                continue
            if all(i == j for i, j in zip(filter_parts, blob_parts, strict=False)):
                return True
        return False
