...
__all__ = ["TemporaryFileSwap", "default_index", "git_working_dir", "post_clear_cache"]
...
class TemporaryFileSwap:
    ...
    def __init__(self, file_path: PathLike) -> None:
        ...
        with contextlib.suppress(OSError):  # It may be that the source does not exist.
            pathlib.Path(self.file_path).replace(self.tmp_file_path)
    ...
    def __exit__(
        ...
        if pathlib.Path(self.tmp_file_path).is_file():
            pathlib.Path(self.tmp_file_path).replace(self.file_path)
        return False
