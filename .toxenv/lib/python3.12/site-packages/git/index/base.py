...
__all__ = ["CheckoutError", "IndexFile", "StageType"]
...
# typing -----------------------------------------------------------------------------
from typing import IO, TYPE_CHECKING, Any, BinaryIO, Dict, List, NoReturn, Tuple, Union
...
class IndexFile(LazyMixin, git_diff.Diffable, Serializable):
    ...
    __slots__ = ("_extension_data", "_file_path", "entries", "repo", "version")
    ...
    def _iter_expand_paths(
        ...
        for path in paths:
            abs_path = str(path)
            if not pathlib.Path(abs_path).is_absolute():
            ...
            if not pathlib.Path(abs_path).exists() and (
    ...
    def entry_key(
        cls,
        *entry: BaseIndexEntry | PathLike | StageType,
    ...
    def _store_path(self, filepath: PathLike, fprogress: Callable) -> BaseIndexEntry:
        ...
        if S_ISLNK(st.st_mode):
            ...
            def open_stream() -> BinaryIO:
                return BytesIO(
                    force_bytes(pathlib.Path(filepath).readlink(), encoding=defenc),
                )
    ...
    def _remove_commit_editmsg(self) -> None:
        pathlib.Path(self._commit_editmsg_filepath()).unlink()
    ...
    def checkout(
        ...
        **kwargs: Any,
        ...
        def handle_stderr(
            proc: "Popen[bytes]",
            iter_checked_out_files: Iterable[PathLike],
        ) -> None:
