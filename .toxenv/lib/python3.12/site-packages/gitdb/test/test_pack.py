...
def bin_sha_from_filename(filename):
    return to_bin_sha(os.path.splitext(pathlib.Path(filename).name)[0][5:])
...
class TestPack(TestBase):
    ...
    def test_pack_entity(self, rw_dir):
        ...
        for packinfo, indexinfo in (
            ...
            count = 0
            for info, stream in zip(
                entity.info_iter(), entity.stream_iter(), strict=False
            ):
                count += 1
        ...
        for ppath, ipath, num_obj in zip(
            ...
            assert pathlib.Path(ppath).stat().st_size > 100
            ...
            if ipath is not None:
                ifile.close()
                assert pathlib.Path(ipath).stat().st_size > 100
