...
__all__ = ("PackEntity", "PackFile", "PackIndexFile")
...
class PackFile(LazyMixin):
    ...
    **Note:** at some point, this might be implemented using streams as well, or
    ...
    __slots__ = ("_cursor", "_packpath", "_size", "_version")
...
class PackEntity(LazyMixin):
    ...
    def _set_cache_(self, attr):
        ...
        if len(offsets_sorted) == 1:
            ...
            consecutive = zip(iter_offsets, iter_offsets_plus_one, strict=False)
        ...
        if use_crc:
            ...
            while cur_pos < next_offset:
                ...
                this_crc_value = crc_update(
                    pack_data[cur_pos : cur_pos + size],
                    this_crc_value,
                )
                cur_pos += size
    ...
    def create(
        ...
        **Note:** for more information on the other parameters see the write_pack method
        ...
        pathlib.Path(pack_path).rename(new_pack_path)
        pathlib.Path(index_path).rename(new_index_path)
