    ...
    * A list of one byte commands (cmd) which are followed by a specific protocol:
    ...
    def _set_cache_brute_(self, attr):
        ...
        for (dbuf, offset, src_size, target_size), dstream in zip(
            reversed(buffer_info_list), reversed(self._dstreams), strict=False
        ):
...
class FDCompressedSha1Writer(Sha1Writer):
    ...
    **Note:** for this to work, you have to use the close-method of this instance"""
    ...
    # default exception
    exc = OSError("Failed to write all bytes to filedescriptor")
...
class FDStream:
    ...
    def read(self, count=0):
        if count == 0:
            count = pathlib.Path(self._filepath).stat().st_size
