...
def load(f, _dict=dict, decoder=None):
    ...
    if _ispath(f):
        ...
        if not [path for path in f if pathlib.Path(path).exists()]:
...
def loads(s, _dict=dict, decoder=None):
    ...
    for idx, line in enumerate(s):
        ...
        if line[0] == "[":
            ...
            if line[-1] != "}":
                raise TomlDecodeError(
                    "Line breaks are not allowed in inlineobjects",
                    original,
                    pos,
                )
            try:
...
_escape_to_escapedchars = dict(zip(_escapes, _escapedchars, strict=False))
