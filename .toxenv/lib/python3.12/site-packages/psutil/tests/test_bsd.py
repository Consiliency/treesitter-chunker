...
class BSDTestCase(PsutilTestCase):
    ...
    @pytest.mark.skipif(
        not shutil.which("sysctl"),
        reason="sysctl cmd not available",
    )
    ...
    @pytest.mark.skipif(
        ...
        reason="ifconfig cmd not available",
    )
...
class FreeBSDSystemTestCase(PsutilTestCase):
    ...
    def test_cpu_stats_ctx_switches(self):
        assert (
            abs(
                psutil.cpu_stats().ctx_switches - sysctl("vm.stats.sys.v_swtch"),
            )
            < 1000
        )
    ...
    def test_cpu_stats_soft_interrupts(self):
        assert (
            abs(
                psutil.cpu_stats().soft_interrupts - sysctl("vm.stats.sys.v_soft"),
            )
            < 1000
        )
