...
class TestFetchAllProcesses(PsutilTestCase):
    ...
    def exe(self, ret, info):
        ...
        if ret:
            ...
            assert pathlib.Path(ret).is_absolute(), ret
            # Note: os.stat() may return False even if the file is there
            # hence we skip the test, see:
            # http://stackoverflow.com/questions/3112546/os-path-exists-lies
            if POSIX and pathlib.Path(ret).is_file():
                if hasattr(os, "access") and hasattr(os, "X_OK"):
                    # XXX: may fail on MACOS
                    try:
                        assert os.access(ret, os.X_OK)
                    except AssertionError:
                        if pathlib.Path(ret).exists() and not CI_TESTING:
                            raise
    ...
    def open_files(self, ret, info):
        ...
        for f in ret:
            ...
            if WINDOWS:
                ...
                continue
            assert pathlib.Path(f.path).is_absolute(), f
            try:
                st = os.stat(f.path)
    ...
    def terminal(self, ret, info):
        ...
        if ret is not None:
            assert pathlib.Path(ret).is_absolute(), ret
            assert pathlib.Path(ret).exists(), ret
