...
class TestSystemCPUCountLogical(PsutilTestCase):
    @pytest.mark.skipif(
        not pathlib.Path("/sys/devices/system/cpu/online").exists(),
    ...
    @pytest.mark.skipif(
        not pathlib.Path("/sys/devices/system/cpu").exists(),
    ...
    @pytest.mark.skipif(
        not shutil.which("nproc"),
        reason="nproc utility not available",
    )
    ...
    @pytest.mark.skipif(
        not shutil.which("lscpu"),
        reason="lscpu utility not available",
    )
...
class TestSystemCPUCountCores(PsutilTestCase):
    ...
    @pytest.mark.skipif(
        platform.machine() not in {"x86_64", "i686"},
        reason="x86_64/i686 only",
    )
...
class TestSystemCPUFrequency(PsutilTestCase):
    ...
    @pytest.mark.skipif(
        AARCH64,
        reason="aarch64 does not always expose frequency",
    )
    ...
    @pytest.mark.skipif(
        AARCH64,
        reason="aarch64 does not report mhz in /proc/cpuinfo",
    )
...
class TestSystemNetIfStats(PsutilTestCase):
    @pytest.mark.skipif(
        not shutil.which("ifconfig"),
        reason="ifconfig utility not available",
    )
...
class TestSystemDiskPartitions(PsutilTestCase):
    @pytest.mark.skipif(
        ...
        reason="os.statvfs() not available",
    )
    @skip_on_not_implemented()
...
class TestSystemDiskIoCounters(PsutilTestCase):
    ...
    def test_emulate_use_sysfs(self):
        ...
        with mock.patch(
            "psutil._pslinux.os.path.exists",
            create=True,
            side_effect=exists,
        ):
...
class TestRootFsDeviceFinder(PsutilTestCase):
    ...
    def test_call_methods(self):
        ...
        if pathlib.Path("/proc/partitions").exists():
            finder.ask_proc_partitions()
        else:
            with pytest.raises(FileNotFoundError):
                finder.ask_proc_partitions()
        if pathlib.Path(f"/sys/dev/block/{self.major}:{self.minor}/uevent").exists():
    ...
    @pytest.mark.skipif(
        not shutil.which("findmnt"),
        reason="findmnt utility not available",
    )
    @pytest.mark.skipif(GITHUB_ACTIONS, reason="unsupported on GITHUB_ACTIONS")
...
class TestSensorsBattery(PsutilTestCase):
    @pytest.mark.skipif(
        not shutil.which("acpi"),
        reason="acpi utility not available",
    )
...
class TestProcess(PsutilTestCase):
        ...
        def get_test_file(fname):
            ...
            while True:
                for file in p.open_files():
                    if file.path == pathlib.Path(fname).resolve():
                        return file
    ...
    def test_terminal_mocked(self):
        with mock.patch(
            "psutil._pslinux._psposix.get_terminal_map",
            return_value={},
        ) as m:
    ...
    def test_rlimit_zombie(self):
        ...
        with mock.patch(
            "resource.prlimit",
            side_effect=OSError(errno.ENOSYS, ""),
        ) as m1:
            with mock.patch(
                "psutil._pslinux.Process._is_zombie",
                return_value=True,
            ) as m2:
                p = psutil.Process()
                p.name()
