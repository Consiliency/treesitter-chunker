...
class TestFSAPIs(BaseUnicodeTest):
    ...
    def test_proc_name(self):
        ...
        if self.expect_exact_path_match():
            assert name == pathlib.Path(self.funky_name).name
    ...
    @pytest.mark.skipif(not POSIX, reason="POSIX only")
    @pytest.mark.skipif(
        not HAS_NET_CONNECTIONS_UNIX,
        reason="can't list UNIX sockets",
    )
    @skip_on_access_denied()
    def test_net_connections(self):
        def find_sock(cons):
            for conn in cons:
                if pathlib.Path(conn.laddr).name.startswith(TESTFN_PREFIX):
                    return conn
