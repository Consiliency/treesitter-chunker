...
class TestProcess(PsutilTestCase):
    ...
    def test_ionice_linux(self):
        ...
        with pytest.raises(
            ValueError,
            match="'ioclass' argument must be specified",
        ):
            p.ionice(value=1)
    ...
    @pytest.mark.skipif(
        not WINDOWS,
        reason="not supported on this win version",
    )
    def test_ionice_win(self):
        ...
        with pytest.raises(
            TypeError,
            match="value argument not accepted on Windows",
        ):
    ...
    def test_memory_maps(self):
        ...
        for nt in maps:
            ...
            if BSD and nt.path == "pvclock":
                continue
            assert pathlib.Path(nt.path).is_absolute(), nt.path

            if POSIX:
                try:
                    assert (
                        pathlib.Path(nt.path).exists()
                        or pathlib.Path(nt.path).is_symlink()
                    ), nt.path
                    ...
                    if f"{nt.path} (deleted)" not in data:
                        raise
            elif "64" not in pathlib.Path(nt.path).name:
    ...
    def test_name(self):
        p = self.spawn_psproc()
        name = p.name().lower()
        pyexe = pathlib.Path(os.path.realpath(sys.executable)).name.lower()
    ...
    def test_long_name(self):
        ...
        if OPENBSD:
            ...
            try:
                assert p.name() == pathlib.Path(pyexe).name
            except AssertionError:
                if p.status() == psutil.STATUS_ZOMBIE:
                    assert pathlib.Path(pyexe).name.startswith(p.name())
                else:
                    raise
        else:
            assert p.name() == pathlib.Path(pyexe).name
    ...
    def test_cwd_2(self):
        ...
        p = self.spawn_psproc(cmd)
        call_until(lambda: p.cwd() == pathlib.Path(pathlib.Path.cwd()).parent)
    ...
    def test_open_files(self):
        ...
        for file in files:
            assert pathlib.Path(file.path).is_file(), file
    ...
    @pytest.mark.skipif(
        OPENBSD or NETBSD,
        reason="not reliable on OPENBSD & NETBSD",
    )
    ...
    @pytest.mark.skipif(
        NETBSD,
        reason="sometimes fails on `assert is_running()`",
    )
