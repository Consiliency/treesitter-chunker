...
class TestSyncTestUtils(PsutilTestCase):
    ...
    def test_wait_for_file(self):
        ...
        assert not pathlib.Path(testfn).exists()
    ...
    def test_wait_for_file_no_delete(self):
        ...
        assert pathlib.Path(testfn).exists()
...
class TestFSTestUtils(PsutilTestCase):
    ...
    def test_safe_mkdir(self):
        testfn = self.get_testfn()
        safe_mkdir(testfn)
        assert pathlib.Path(testfn).is_dir()
        safe_mkdir(testfn)
        assert pathlib.Path(testfn).is_dir()
    ...
    def test_chdir(self):
        ...
        os.mkdir(testfn)
        with chdir(testfn):
            assert pathlib.Path.cwd() == os.path.join(base, testfn)
        assert pathlib.Path.cwd() == base
...
class TestNetUtils(PsutilTestCase):
    ...
    @pytest.mark.skipif(
        NETBSD or FREEBSD,
        reason="/var/run/log UNIX socket opened by default",
    )
