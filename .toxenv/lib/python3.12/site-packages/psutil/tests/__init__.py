...
HERE = os.path.realpath(pathlib.Path(__file__).parent)
...
def spawn_testproc(cmd=None, **kwds):
    ...
    kwds.setdefault("cwd", pathlib.Path.cwd())
    ...
    if cmd is None:
        testfn = get_testfn(dir=pathlib.Path.cwd())
        try:
            safe_rmpath(testfn)
            pyline = (
                "import time;"
...
def spawn_children_pair():
    ...
    try:
        s = textwrap.dedent(
            ...
            s = "import os, time;"
            s += "f = open('{pathlib.Path(testfn).name}', 'w');"
...
def create_py_exe(path):
    ...
    if POSIX:
        st = os.stat(path)
        pathlib.Path(path).chmod(st.st_mode | stat.S_IEXEC)
    return path
...
def get_testfn(suffix="", dir=None):
    ...
    while True:
        ...
        if not pathlib.Path(name).exists():  # also include dirs
...
def print_sysinfo():
    ...
    info["home"] = pathlib.Path("~").expanduser()
    info["cwd"] = pathlib.Path.cwd()
    ...
    info["memory"] = (
        f"{int(mem.percent)}%%, used={bytes2human(mem.used)}, total={bytes2human(mem.total)}"
    )
    swap = psutil.swap_memory()
    info["swap"] = (
        f"{int(swap.percent)}%%, used={bytes2human(swap.used)}, total={bytes2human(swap.total)}"
    )
...
def import_module_by_path(path):
    name = os.path.splitext(pathlib.Path(path).name)[0]
...
if POSIX:
    ...
    def copyload_shared_lib(suffix=""):
        ...
        libs = [
            ...
            and "python" in pathlib.Path(x.path).name.lower()
        ...
        if PYPY and not libs:
            libs = [
                x.path
                for x in psutil.Process().memory_maps()
                if "pypy" in pathlib.Path(x.path).name.lower()
            ]
        src = random.choice(libs)
        shutil.copyfile(src, dst)
        cfile = None
