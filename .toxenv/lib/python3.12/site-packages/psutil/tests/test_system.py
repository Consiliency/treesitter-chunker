...
class TestMiscAPIs(PsutilTestCase):
    ...
    @pytest.mark.skipif(
        CI_TESTING and not psutil.users(),
...
class TestCpuAPIs(PsutilTestCase):
    ...
    def test_cpu_count_none(self):
        ...
        for val in (-1, 0, None):
            with mock.patch(
                "psutil._psplatform.cpu_count_logical",
                return_value=val,
            ) as m:
                assert psutil.cpu_count() is None
                assert m.called
            with mock.patch(
                "psutil._psplatform.cpu_count_cores",
                return_value=val,
            ) as m:
    ...
    @pytest.mark.skipif(
        MACOS and platform.machine() == "arm64",
        reason="skipped due to #1892",
    )
...
class TestDiskAPIs(PsutilTestCase):
    @pytest.mark.skipif(
        PYPY and not IS_64BIT,
        reason="unreliable on PYPY32 + 32BIT",
    )
    def test_disk_usage(self):
        usage = psutil.disk_usage(pathlib.Path.cwd())
        ...
        if hasattr(shutil, "disk_usage"):
            ...
            shutil_usage = shutil.disk_usage(pathlib.Path.cwd())
    ...
    def test_disk_partitions(self):
        ...
        for disk in ls:
            ...
            if WINDOWS and "cdrom" in disk.opts:
                continue
            if not POSIX:
                assert pathlib.Path(disk.device).exists(), disk
            else:
                # we cannot make any assumption about this, see:
                # http://goo.gl/p9c43
                disk.device  # noqa: B018
            # on modern systems mount points can also be files
            assert pathlib.Path(disk.mountpoint).exists(), disk
    ...
    @pytest.mark.skipif(
        LINUX and not pathlib.Path("/proc/diskstats").exists(),
        reason="/proc/diskstats not available on this linux version",
    )
    @pytest.mark.skipif(
        CI_TESTING and not psutil.disk_io_counters(),
        reason="unreliable on CI",
    )  # no visible disks
    ...
    def test_disk_io_counters_no_disks(self):
        ...
        with mock.patch(
            "psutil._psplatform.disk_io_counters",
            return_value={},
        ) as m:
...
class TestNetAPIs(PsutilTestCase):
    ...
    def test_net_io_counters_no_nics(self):
        ...
        # https://github.com/giampaolo/psutil/issues/1062
        with mock.patch(
            "psutil._psplatform.net_io_counters",
            return_value={},
        ) as m:
    ...
    def test_net_if_addrs_mac_null_bytes(self):
        ...
        with mock.patch(
            "psutil._psplatform.net_if_addrs",
            return_value=ret,
        ) as m:
    ...
    @pytest.mark.skipif(
        not (LINUX or BSD or MACOS),
        reason="LINUX or BSD or MACOS specific",
    )
