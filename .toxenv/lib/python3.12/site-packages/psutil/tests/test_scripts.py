...
@pytest.mark.skipif(
    CI_TESTING and not pathlib.Path(SCRIPTS_DIR).exists(),
...
@pytest.mark.skipif(
    CI_TESTING and not pathlib.Path(INTERNAL_SCRIPTS_DIR).exists(),
...
@pytest.mark.skipif(
    CI_TESTING and not pathlib.Path(SETUP_PY).exists(),
    reason="can't find setup.py",
)
class TestSetupScript(PsutilTestCase):
    ...
    @pytest.mark.skipif(
        ...
        reason="python2.7 not installed",
    )
