...
def pid_exists(pid):
    ...
    return pathlib.Path(os.path.join(get_procfs_path(), str(pid), "psinfo")).exists()
...
class Process:
    ...
    def exe(self):
        ...
        if os.path.sep in exe:
            ...
            if not pathlib.Path(exe).is_absolute():
                # if cwd has changed, we're out of luck - this may be wrong!
                exe = pathlib.Path(os.path.join(self.cwd(), exe)).resolve()
            if (
                pathlib.Path(exe).is_absolute()
                and pathlib.Path(exe).is_file()
                and os.access(exe, os.X_OK)
            ):
                return exe
            # not found, move to search in PATH using basename only
            exe = pathlib.Path(exe).name
        # search for exe name PATH
        for path in os.environ["PATH"].split(":"):
            possible_exe = pathlib.Path(os.path.join(path, exe)).resolve()
            if pathlib.Path(possible_exe).is_file() and os.access(
                possible_exe,
                os.X_OK,
            ):
                return possible_exe
    ...
    def cwd(self):
        ...
        try:
            result = pathlib.Path(f"{procfs_path}/{self.pid}/cwd").readlink()
