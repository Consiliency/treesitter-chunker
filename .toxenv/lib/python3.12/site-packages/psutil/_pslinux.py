...
HAS_PROC_SMAPS = pathlib.Path(f"/proc/{os.getpid()}/smaps").exists()
HAS_PROC_SMAPS_ROLLUP = pathlib.Path(f"/proc/{os.getpid()}/smaps_rollup").exists()
...
def readlink(path):
    ...
    path = pathlib.Path(path).readlink()
...
if (
    pathlib.Path("/sys/devices/system/cpu/cpufreq/policy0").exists()
    or pathlib.Path("/sys/devices/system/cpu/cpu0/cpufreq").exists()
):
...
class NetConnections:
    ...
    def process_inet(file, family, type_, inodes, filter_pid=None):
        ...
        if file.endswith("6") and not pathlib.Path(file).exists():
            # IPv6 not supported
            return
...
def disk_io_counters(perdisk=False):
    ...
    if pathlib.Path(f"{get_procfs_path()}/diskstats").exists():
        gen = read_procfs()
    elif pathlib.Path("/sys/block").exists():
        gen = read_sysfs()
    else:
        msg = (
        ...
        for file in files:
            try:
                ...
                with f:
                    data = f.read().strip()
                    if data == needle:
                        name = pathlib.Path(pathlib.Path(file).parent).name
                        return f"/dev/{name}"
...
def disk_partitions(all=False):
    ...
    if procfs_path == "/proc" and pathlib.Path("/etc/mtab").is_file():
...
def sensors_temperatures():
    ...
    for base in basenames:
        try:
            ...
            path = os.path.join(pathlib.Path(base).parent, "name")
    ...
    if not basenames:
        ...
        for base in basenames:
            ...
            trip_points = {
                "_".join(pathlib.Path(p).name.split("_")[0:3]) for p in trip_paths
            }
            critical = None
            high = None
            for trip_point in trip_points:
                ...
                if trip_type == "critical":
                    critical = bcat(
                        os.path.join(base, trip_point + "_temp"),
                        fallback=None,
                    )
                elif trip_type == "high":
                    high = bcat(
...
def sensors_fans():
    ...
    for base in basenames:
        try:
            ...
            debug(err)
            continue
        unit_name = cat(os.path.join(pathlib.Path(base).parent, "name")).strip()
        label = cat(base + "_label", fallback="").strip()
...
def wrap_exceptions(fun):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        try:
            ...
            if not pathlib.Path(f"{self._procfs_path}/{pid}/stat").exists():
                raise NoSuchProcess(pid, name) from err
            raise
...
class Process:
    ...
    if pathlib.Path(f"/proc/{os.getpid()}/io").exists():
    ...
    def num_ctx_switches(
        self,
        _ctxsw_re=re.compile(rb"ctxt_switches:\t(\d+)"),
    ):
    ...
    if HAS_CPU_AFFINITY:
        ...
        def _get_eligible_cpus(
            self,
            _re=re.compile(rb"Cpus_allowed_list:\t(\d+)-(\d+)"),
        ):
