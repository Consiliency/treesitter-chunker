...
class Process:
    ...
    def _proc_basic_info(self):
        if (
            self.pid == 0
            and not pathlib.Path(f"{self._procfs_path}/{self.pid}/psinfo").exists()
        ):
    ...
    def exe(self):
        try:
            return pathlib.Path(f"{self._procfs_path}/{self.pid}/path/a.out").readlink()
        except OSError:
    ...
    def terminal(self):
        ...
        if tty != cext.PRNODEV:
            for x in (0, 1, 2, 255):
                try:
                    return pathlib.Path(f"{procfs_path}/{self.pid}/path/{x}").readlink()
    ...
    def cwd(self):
        ...
        procfs_path = self._procfs_path
        try:
            return pathlib.Path(f"{procfs_path}/{self.pid}/path/cwd").readlink()
        except FileNotFoundError:
    ...
    def open_files(self):
        ...
        for fd in os.listdir(f"{procfs_path}/{self.pid}/fd"):
            ...
            if pathlib.Path(path).is_symlink():
                try:
                    file = pathlib.Path(path).readlink()
                except FileNotFoundError:
                    hit_enoent = True
                    continue
                else:
