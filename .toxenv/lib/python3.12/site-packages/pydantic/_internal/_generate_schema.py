...
GetCoreSchemaFunction: TypeAlias = Callable[
    [Any, ModifyCoreSchemaWrapHandler],
...
def check_decorator_fields_exist(
    decorators: Iterable[AnyFieldDecorator],
    fields: Iterable[str],
) -> None:
...
def _add_custom_serialization_from_json_encoders(
    json_encoders: JsonEncoders | None,
    tp: Any,
    schema: CoreSchema,
) -> CoreSchema:
...
class GenerateSchema:
    ...
    def _dict_schema(self, keys_type: Any, values_type: Any) -> CoreSchema:
        return core_schema.dict_schema(
            self.generate_schema(keys_type),
            self.generate_schema(values_type),
        )
    ...
    def _add_js_function(
        self,
        metadata_schema: CoreSchema,
        js_function: Callable[..., Any],
    ) -> None:
    ...
    def generate_schema(
        ...
        schema = _add_custom_serialization_from_json_encoders(
            self._config_wrapper.json_encoders,
            obj,
            schema,
        )
        ...
        with self.defs.get_schema_or_ref(cls) as (model_ref, maybe_schema):
            ...
            with (
                ...
                schema = self._apply_model_serializers(
                    model_schema,
                    decorators.model_serializers.values(),
                )
    ...
    def _resolve_self_type(self, obj: Any) -> Any:
        ...
        if obj is None:
            raise PydanticUserError(
                "`typing.Self` is invalid in this context",
                code="invalid-self-type",
            )
        return obj
            ...
            with self._config_wrapper_stack.push(config):
                ...
                for field_name, annotation in annotations.items():
                    field_info = FieldInfo.from_annotation(
                        annotation,
                        _source=AnnotationSource.TYPED_DICT,
                    )
    ...
    def finalize_schema(self, schema: CoreSchema) -> CoreSchema:
        ...
        if remaining_defs:
            schema = core_schema.definitions_schema(
                schema=schema,
                definitions=[*remaining_defs.values()],
            )
        return schema
