...
from chunker.hierarchy import ChunkHierarchyBuilder, HierarchyNavigator, ChunkHierarchy
...
''')
    ...
    def test_parent_child_token_relationships(self, sample_python_file):
        ...
        def verify_token_relationship(node: ChunkHierarchy):
            ...
            if node.children:
                ...
                child_token_sum = sum(
                    child.chunk.metadata.get('tokens', 0)
                    for child in node.children
                )
    ...
    def test_incremental_hierarchy_with_tokens(self, test_repo_path):
        ...
        expected_files = {
            str(f.relative_to(test_repo_path)) for f in python_files
        }
    ...
    def test_token_based_hierarchy_filtering(self, test_repo_path):
        ...
        assert all(
            chunk.metadata.get('tokens', 0) >= min_tokens
