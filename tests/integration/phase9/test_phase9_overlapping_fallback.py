...
''')
    ...
    def test_overlapping_with_hierarchy(self, complex_file):
        ...
        method_chunks = [c for c in chunks if "def " in c.content and "class" not in c.content]
    ...
    def test_overlapping_with_metadata(self, complex_file):
        ...
        for i in range(len(chunks) - 1):
            ...
            if "def load_data" in current.content and "def load_data" in next_chunk.content:
                assert current.metadata.get("signatures") or next_chunk.metadata.get("signatures")
    ...
    def test_fallback_strategies(self, complex_file):
        ...
        assert len(line_chunks) != len(semantic_chunks) or line_chunks[0].content != semantic_chunks[0].content
        
        # Semantic should try to keep functions together
        semantic_contents = [c.content for c in semantic_chunks]
        complete_functions = sum(
            1 for content in semantic_contents
