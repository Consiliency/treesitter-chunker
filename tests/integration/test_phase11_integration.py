...
try:
    from chunker.sliding_window import DefaultSlidingWindowEngine, WindowConfig, WindowUnit, OverlapStrategy
    HAS_SLIDING_WINDOW = True
except ImportError:
    HAS_SLIDING_WINDOW = False

try:
    from chunker.text_processing import SentenceBoundaryDetector, ParagraphDetector
...
class TestPhase11Integration:
    ...
    @pytest.mark.skipif(not HAS_MARKDOWN_PROCESSOR, reason="MarkdownProcessor not available")
...
""")
        ...
        assert any("Column 1" in chunk.content and "Column 2" in chunk.content 
                  for chunk in chunks)
    ...
    def test_log_processor(self):
        ...
        log_file.write_text("""2024-01-22 10:00:00 INFO Starting application
...
""")
    ...
    @pytest.mark.skipif(not HAS_CONFIG_PROCESSOR, reason="ConfigProcessor not available")
...
""")
        ...
        log_file.write_text("""2024-01-22 10:00:00 INFO Application started
