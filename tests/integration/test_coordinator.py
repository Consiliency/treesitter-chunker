...
from tests.integration.coordinator import IntegrationCoordinator, TestScenario, TestResult
...
class TestIntegrationCoordinator:
    ...
    def test_check_dependencies(self, coordinator):
        ...
        coordinator.results.append(TestResult(
            scenario=scenario1,
            status="failed",
            duration=1.0,
            output=""
    ...
    def test_topological_sort(self, coordinator):
        ...
        s2 = TestScenario(name="s2", description="", worktree="", test_file="", 
                         dependencies=["s1"])
        s3 = TestScenario(name="s3", description="", worktree="", test_file="", 
                         dependencies=["s1", "s2"])
    ...
    def test_generate_report(self, coordinator):
        ...
        coordinator.results = [
            ...
            TestResult(scenario=scenario2, status="failed", duration=2.0, output="", 
                      error="Test failed"),
    ...
    def test_save_report(self, coordinator, tmp_path):
        ...
        scenario = TestScenario(name="test", description="", worktree="wt", test_file="")
        coordinator.results = [
