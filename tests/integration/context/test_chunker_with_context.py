...
'''.strip()
    ...
    def test_chunk_with_context_preservation(self, python_code_with_dependencies):
        ...
        parent_context = extractor.extract_parent_context(perimeter_node, tree.root_node, source)
        dependencies = extractor.extract_dependencies(perimeter_node, tree.root_node, source)
        
        # Filter relevant context
        all_context = imports + type_defs + parent_context + dependencies
        relevant_context = [
            item for item in all_context
            if filter.is_relevant(item, perimeter_node)
        ]
    ...
    def test_context_for_function_using_classes(self, python_code_with_dependencies):
        ...
        dependencies = extractor.extract_dependencies(create_square_node, tree.root_node, source)
        type_defs = extractor.extract_type_definitions(tree.root_node, source)
        ...
        relevant_context = [
            item for item in all_context
            if filter.is_relevant(item, create_square_node)
        ]
...
'''.strip()
        ...
        # Extract context
        imports = extractor.extract_imports(tree.root_node, source)
        parent_context = extractor.extract_parent_context(handle_submit, tree.root_node, source)
    ...
    def test_context_size_limitation(self, python_code_with_dependencies):
        ...
        limited = extractor.build_context_prefix(all_context, max_size=50)  # Use smaller limit
