...
from cli.main import app, load_config, get_files_from_patterns, should_include_file, process_file
...
class TestFilePatterns:
    ...
    def test_should_include_file(self):
        ...
        assert not should_include_file(Path("test_file.py"), exclude_patterns=["test_*"])
...
""")
            ...
            result = runner.invoke(app, ["batch", str(tmppath), "--include", "*.py", "--quiet"])
            assert result.exit_code == 0
            assert "2 total chunks" in result.output
            assert "from 2" in result.output and "files)" in result.output
...
""")
            ...
            input_data = f"{file1}\n{file2}\n"
            result = runner.invoke(app, ["batch", "--stdin", "--quiet"], input=input_data)
