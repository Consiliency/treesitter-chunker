...
from chunker.fallback.base import FallbackWarning, FallbackConfig
from chunker.interfaces.fallback import FallbackReason, ChunkingMethod
...
Contributions are welcome! Please read the guidelines."""
    ...
    def test_fixed_overlap_by_lines(self, chunker, log_content):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            fallback_warnings = [warn for warn in w if issubclass(warn.category, FallbackWarning)]
        ...
        for i in range(1, len(chunks)):
            ...
            assert prev_lines[-1] == curr_lines[0], f"Chunk {i-1} last line doesn't match chunk {i} first line"
    ...
    def test_fixed_overlap_by_characters(self, chunker, markdown_content):
        ...
        for i in range(1, len(chunks)):
            ...
            if i < len(chunks) - 1:
                ...
                assert len(curr_chunk) > 50, f"Chunk {i} is too small: {len(curr_chunk)} chars"
...
Third paragraph is shorter."""
        ...
        content2 = "This is the first sentence. This is the second sentence. And the third."
    ...
    def test_warning_logging(self, chunker, log_content, caplog):
        ...
        # Check log output
        assert any("overlapping fallback" in record.message.lower() 
                  for record in caplog.records)
        assert any("app.log" in record.message 
                  for record in caplog.records)
