...
""")
        ...
        method_names = [c.content.split('(')[0].split()[-1] for c in chunks if '(' in c.content]
...
""")
        ...
        assert len(function_chunks) >= 5  # globalFunction, log, swap, help, connect, disconnect
...
""")
        ...
        virtual_methods = [c for c in chunks if "virtual" in c.content or "override" in c.content]
...
""")
        ...
        function_chunks = [c for c in chunks if c.node_type == "function_definition"]
        assert len(function_chunks) >= 6  # addData, processAll, getSorted, filter, makeProcessor, demonstrateSTL
...
""")
        ...
        constexpr_chunks = [c for c in chunks if "constexpr" in c.content and "factorial" in c.content]
        assert len(constexpr_chunks) >= 1
        
        # Check for variadic template function
        variadic_chunks = [c for c in chunks if "print" in c.content and "..." in c.content]
        assert len(variadic_chunks) >= 1
        
        # Check for template with concept
        concept_chunks = [c for c in chunks if "add" in c.content and c.node_type == "function_definition"]
