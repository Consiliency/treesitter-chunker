...
"""
        ...
        assert len(chunks) == 2
        assert all(c.metadata.get('chunking_decision') == ChunkingDecision.TREE_SITTER.value 
                  for c in chunks)
...
'''
        ...
        assert any(c.metadata.get('chunking_decision') == ChunkingDecision.TREE_SITTER_WITH_SPLIT.value 
                  for c in chunks)
    ...
    def test_sliding_window_decision_no_language_support(self):
        ...
        assert len(chunks) >= 1
        assert all(c.metadata.get('chunking_decision') == ChunkingDecision.SLIDING_WINDOW.value 
                  for c in chunks)
...
"""
        ...
        assert ChunkingDecision.SLIDING_WINDOW.value in decisions or \
               ChunkingDecision.SPECIALIZED_PROCESSOR.value in decisions
    ...
    def test_parse_failure_fallback(self):
        ...
        assert chunks2[0].metadata.get('chunking_decision') == ChunkingDecision.SLIDING_WINDOW.value
