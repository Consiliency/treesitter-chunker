...
class TestConfigLoading:
    ...
    def test_load_yaml_config(self, temp_config_dir):
        ...
        assert config.plugin_configs["python"].chunk_types == {"function_definition", "class_definition"}
        assert config.plugin_configs["python"].custom_options["include_docstrings"] is True
...
class TestConfigSaving:
    ...
    def test_save_yaml_config(self, temp_config_dir):
        ...
        config.default_plugin_config = PluginConfig(min_chunk_size=3, max_chunk_size=500)
        ...
        assert loaded_data["chunker"]["plugin_dirs"] == ["plugins", str(Path("~/.chunker/plugins"))]
    ...
    def test_roundtrip_yaml(self, temp_config_dir):
        ...
        config1.default_plugin_config = PluginConfig(min_chunk_size=2, max_chunk_size=100)
        ...
        assert config3.plugin_configs["python"].chunk_types == config4.plugin_configs["python"].chunk_types
        assert config3.plugin_configs["python"].custom_options == config4.plugin_configs["python"].custom_options
...
class TestPathResolution:
    ...
    def test_resolve_relative_path(self, temp_config_dir):
        ...
        assert config.plugin_dirs[1] == (temp_config_dir / "../shared_plugins").resolve()
    ...
    def test_large_config_file(self, temp_config_dir):
        ...
        for i in range(100):
            languages[f"lang_{i}"] = {
                ...
                "min_chunk_size": i + 1,
                "custom_options": {
                    f"option_{k}": f"value_{k}" for k in range(20)
                }
            }
