...
from chunker.streaming import get_file_metadata, FileMetadata
...
class TestCacheBasics:
    ...
    def test_cache_initialization(self, temp_cache_dir):
        ...
        with sqlite3.connect(cache.db_path) as conn:
            cursor = conn.execute("SELECT sql FROM sqlite_master WHERE type='table' AND name='file_cache'")
    ...
    def test_cache_and_retrieve_chunks(self, cache, temp_python_file):
        ...
        assert all(c1.chunk_id == c2.chunk_id for c1, c2 in zip(chunks, cached_chunks))
        assert all(c1.content == c2.content for c1, c2 in zip(chunks, cached_chunks))
    ...
    def test_cache_multiple_languages(self, cache, temp_python_file):
        ...
        py_chunks = [
            CodeChunk(
                language="python", file_path=str(temp_python_file),
                node_type="function", start_line=1, end_line=3,
                byte_start=0, byte_end=50, parent_context="",
                content="def test(): pass", chunk_id="py1"
            )
        ]
        ...
        js_chunks = [
            CodeChunk(
                language="javascript", file_path=str(temp_python_file),
                node_type="function", start_line=1, end_line=3,
                byte_start=0, byte_end=50, parent_context="",
                content="function test() {}", chunk_id="js1"
            )
        ]
...
class TestCacheInvalidation:
    ...
    def test_invalidate_all_cache(self, cache, temp_python_file, temp_js_file):
        ...
        cache.cache_chunks(temp_python_file, "python", chunk_file(temp_python_file, "python"))
        cache.cache_chunks(temp_js_file, "javascript", chunk_file(temp_js_file, "javascript"))
