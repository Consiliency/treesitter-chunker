...
class TestCrossModuleErrors(ErrorPropagationMixin):
    ...
    def test_parser_error_to_cli(self, error_tracking_context, temp_workspace):
        ...
        parser_error = SyntaxError("Unexpected end of input while parsing function definition")
        ...
        chunker_error = RuntimeError(f"Failed to parse {test_file.name}: {parser_error}")
        ...
        cli_error_message = self._format_user_friendly_error(chunker_context, parser_context)
    ...
    def test_plugin_error_to_export(self, error_tracking_context, temp_workspace):
        ...
        error_chain = error_tracking_context.get_error_chain()
        plugin_errors = [e for e in error_chain if e["source_module"] == "plugin_manager.custom_analyzer"]
    ...
    def test_config_error_to_parallel(self, error_tracking_context):
        ...
        # Add config details
        config_context["context_data"]["invalid_fields"] = ["num_workers", "chunk_types", "timeout"]
        config_context["context_data"]["config_source"] = "runtime_update"
        
        # Simulate parallel module receiving config error
        parallel_error = RuntimeError("Cannot initialize parallel processing: Invalid configuration")
        ...
        config_errors = [e for e in error_chain if "ConfigValidator" in e["source_module"]]
        ...
        assert final_error["context_data"]["cleanup_status"]["resources_released"] == True
    ...
    def test_cascading_failure_scenario(self, error_tracking_context):
        ...
        cache_error = IOError("Cache database corrupted: Invalid header")
        ...
        chunker_error = RuntimeError("Cannot create chunker: Parser initialization failed")
    ...
    def test_error_context_preservation(self, error_tracking_context):
        ...
        expected_keys = {
            "file_path", "watcher_id", "cache_entries_affected",
            "parser_state", "chunks_pending", "queue_size", 
            "export_format", "traceback", "call_stack"
        }
    ...
    def test_user_friendly_formatting(self, error_tracking_context):
        ...
        error_scenarios = [
            ...
            {
                "error": ValueError("Invalid configuration: chunk_size must be positive"),
    ...
    def test_stack_trace_filtering(self, error_tracking_context):
        ...
        try:
            ...
            full_trace_with_internals = full_trace[:2] + internal_frames + full_trace[2:]
            
            # Filter for user mode
            user_trace = self._filter_stack_trace(full_trace_with_internals, debug=False)
            ...
            debug_trace = self._filter_stack_trace(full_trace_with_internals, debug=True)
    ...
    def _format_user_friendly_error(self, error_context: Dict[str, Any], 
                                   original_context: Dict[str, Any] = None) -> str:
        ...
        if error_type == "SyntaxError":
            lines.append("Error parsing file: " + 
                        (original_context or error_context)["context_data"].get("filename", "unknown"))
            ...
            elif "ValueError" in error_type and ("configuration" in message.lower() or "config" in message.lower()):
        ...
        elif "ValueError" in error_type and ("configuration" in error_message.lower() or "config" in error_message.lower()):
    ...
    def _filter_stack_trace(self, traceback_lines: List[str], debug: bool = False) -> List[str]:
    ...
    def _generate_recovery_suggestions(self, error_context: Dict[str, Any]) -> List[str]:
        ...
        if "parser" in error_type.lower() or "syntax" in error_type.lower():
            ...
            language_display = language.capitalize() if language.lower() in ["python", "javascript", "rust", "c", "cpp"] else language
