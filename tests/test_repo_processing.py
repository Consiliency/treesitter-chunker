...
''')
            ...
            (repo_path / "src" / "__pycache__" / "main.cpython-39.pyc").write_bytes(b"fake pyc")
    ...
    def test_traversal_strategies(self, temp_repo):
        ...
        # Depth-first
        df_processor = RepoProcessor(show_progress=False, traversal_strategy="depth-first")
        df_files = df_processor.get_processable_files(str(temp_repo))
        
        # Breadth-first
        bf_processor = RepoProcessor(show_progress=False, traversal_strategy="breadth-first")
    ...
    def test_parallel_processing(self, temp_repo):
        ...
        single_result = single_processor.process_repository(str(temp_repo), incremental=False)
        
        # Multiple workers
        multi_processor = RepoProcessor(show_progress=False, max_workers=4)
        multi_result = multi_processor.process_repository(str(temp_repo), incremental=False)
...
''')
    ...
    def test_should_process_file_gitignore(self, git_processor, git_repo):
        ...
        assert git_processor.should_process_file(str(repo_path / "main.py"), str(repo_path))
        ...
        assert not git_processor.should_process_file(
            str(repo_path / "build" / "output.py"), str(repo_path)
        )
        ...
        assert git_processor.should_process_file(
            str(repo_path / "new_file.py"), str(repo_path)
        )
    ...
    def test_get_file_history(self, git_processor, git_repo):
        ...
        for i in range(3):
            (repo_path / "main.py").write_text(f'''
def main():
    print("Version {i + 2}")
''')
        ...
        history = git_processor.get_file_history(
            str(repo_path / "main.py"), str(repo_path), limit=5
        )
