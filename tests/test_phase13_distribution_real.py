...
class TestDistributionIntegrationReal:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("shutil.which", return_value="/usr/bin/twine"):
                with patch("subprocess.run") as mock_run:
                    mock_run.return_value = Mock(returncode=0, stdout="Checking: PASSED", stderr="")
                    success, info = distributor.publish_to_pypi(package_dir,
                        repository="testpypi", dry_run=True)
                    assert success
    ...
    def test_docker_image_works_cross_platform(cls, distributor):
        ...
        with patch("shutil.which", return_value="/usr/bin/docker"):
            ...
            with patch("subprocess.run") as mock_run:
                mock_run.side_effect = [Mock(returncode=0), Mock(returncode=0, stdout="", stderr=""), Mock(returncode=0, stdout="sha256:test123", stderr="")]
    ...
    def test_release_process_updates_all_locations(cls, distributor):
        ...
        with patch.object(distributor.release_manager, "_run_tests",
            return_value=True), patch("subprocess.run") as mock_run:
            ...
            if success:
                ...
                expected_files = ["pyproject.toml",
                    "chunker/__init__.py", "CHANGELOG.md"]
                for file_path in expected_files:
                    assert any(file_path in str(f) for f in info[
                        "updated_files"])
