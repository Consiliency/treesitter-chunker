...
from tests.integration.fixtures import (
    resource_monitor, temp_workspace, test_file_generator,
    performance_monitor, thread_safe_counter, sample_code_files
)
        ...
        with self.lock:
            ...
            if self.cache_type == "memory":
                ...
                conn.execute(
                    ...
                    (file_path, content, timestamp, content_hash, json.dumps(metadata or {}))
                )
                conn.commit()
                conn.close()
...
class TestCacheFileMonitoring:
    ...
    def test_file_modification_detection(self, temp_workspace, resource_monitor,
                                       test_file_generator, performance_monitor):
    ...
    def test_file_deletion_handling(self, temp_workspace, resource_monitor,
                                  test_file_generator, thread_safe_counter):
    ...
    def test_file_rename_tracking(self, temp_workspace, resource_monitor,
                                 test_file_generator):
    ...
    def test_concurrent_file_modifications(self, temp_workspace, resource_monitor,
                                         test_file_generator, performance_monitor):
        ...
        with performance_monitor.measure("concurrent_modifications"):
            with ThreadPoolExecutor(max_workers=num_workers) as executor:
                ...
                for worker_id in range(num_workers):
                    future = executor.submit(modify_file_worker, worker_id, iterations_per_worker)
    ...
    def test_cache_consistency_across_workers(self, temp_workspace, resource_monitor,
                                            test_file_generator, performance_monitor):
        ...
        with performance_monitor.measure("parallel_caching"):
            with ProcessPoolExecutor(max_workers=4) as executor:
                ...
                for worker_id in range(4):
                    ...
                    future = executor.submit(cache_worker, worker_id, worker_files, cache_dir)
                    futures.append(future)
        ...
        with performance_monitor.measure("concurrent_modifications"):
            with ProcessPoolExecutor(max_workers=3) as executor:
                ...
                for worker_id in range(3):
                    ...
                    future = executor.submit(modifying_worker, worker_id, worker_files, cache_dir)
                    futures.append(future)
    ...
    def test_content_vs_timestamp_invalidation(self, temp_workspace, resource_monitor,
                                             test_file_generator, performance_monitor):
        ...
        assert timestamp_cache.get(str(test_file))["metadata"]["mtime"] < test_file.stat().st_mtime
        ...
        with performance_monitor.measure("timestamp_validation"):
            for file in test_files:
                ...
                if cached and cached["metadata"].get("mtime", 0) < file.stat().st_mtime:
                    # Need to recache
                    timestamp_cache.put(str(file), file.read_text(), {"mtime": file.stat().st_mtime})
        ...
        for i in range(10):
            ...
            if not cached_ts or cached_ts["metadata"].get("mtime", 0) < accuracy_file.stat().st_mtime:
                changes_detected_ts += 1
                timestamp_cache.put(str(accuracy_file), accuracy_file.read_text(), 
    ...
    def test_directory_level_changes(self, temp_workspace, resource_monitor,
                                   test_file_generator, performance_monitor,
                                   thread_safe_counter):
    ...
    def test_cache_corruption_recovery(self, temp_workspace, resource_monitor,
                                     test_file_generator, performance_monitor):
        ...
        with performance_monitor.measure("automatic_recovery"):
            try:
                ...
                recovery_cache = MockCache(temp_workspace / "cache", cache_type="sqlite")
        ...
        prevention_cache = MockCache(temp_workspace / "prevention_cache", cache_type="sqlite")
