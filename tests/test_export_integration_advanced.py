...
from chunker import chunk_file, chunk_files_parallel, CodeChunk
from chunker.export import JSONExporter, JSONLExporter, SchemaType
from chunker.export.formatters import FlatFormatter, FullFormatter, NestedFormatter, MinimalFormatter
...
""")
        ...
        # Compressed versions
        for compress_file in [export_dir / "compressed.json", export_dir / "compressed.jsonl"]:
            exporter = JSONExporter() if compress_file.suffix == ".json" else JSONLExporter()
...
""")
        ...
        full_chunks = formats_data['full']['chunks'] if 'chunks' in formats_data['full'] else formats_data['full']
        minimal_chunks = formats_data['minimal']
        jsonl_chunks = formats_data['jsonl']
        
        # All should have same number of chunks
        chunk_counts = [len(flat_chunks), len(full_chunks), len(minimal_chunks), len(jsonl_chunks)]
        ...
        for i in range(len(flat_chunks)):
            ...
            minimal_content = minimal_chunks[i].get('content', minimal_chunks[i].get('code', ''))
        ...
        for i in range(500):
            chunks.append(
                CodeChunk(
                    ...
                    content=f"def func_{i}(): pass" if i % 2 == 0 else f"class Class_{i}: pass"
                )
            )
...
""")
        ...
        large_chunks = [
            ...
            if chunk.end_line - chunk.start_line > 5
        ]
        ...
        assert len(data) >= 1
        assert all('large' in item['content'] or 
