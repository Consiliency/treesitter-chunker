...
from chunker.parallel import ParallelChunker, chunk_files_parallel, chunk_directory_parallel
...
class TestWorkerPoolSizing:
    ...
    def test_single_worker(self, temp_directory_with_files):
        ...
        results = chunker.chunk_files_parallel(list(temp_directory_with_files.glob("*.py")))
    ...
    def test_io_bound_sizing(self, temp_directory_with_files):
        ...
        chunker = ParallelChunker("python", num_workers=mp.cpu_count() * 2, use_cache=True)
        
        # First run - populate cache
        results1 = chunker.chunk_files_parallel(list(temp_directory_with_files.glob("*.py")))
        
        # Second run - should be faster due to cache
        start_time = time.time()
        results2 = chunker.chunk_files_parallel(list(temp_directory_with_files.glob("*.py")))
...
class TestFailureHandling:
    ...
    def test_worker_crash_handling(self):
        ...
        with patch('chunker.parallel.ParallelChunker._process_single_file') as mock_process:
        ...
        try:
            ...
            for i in range(10):
                ...
                if i % 3 == 0:
                    ...
                    file_path.write_text("This is not valid Python code at all! @#$%^&*()")
                else:
                    file_path.write_text(PYTHON_FUNCTION_TEMPLATE.format(idx=i, complexity=10))
    ...
    def test_memory_pressure(self):
        ...
        try:
            ...
            for i in range(5):
                ...
                content = []
                for j in range(1000):
                    content.append(PYTHON_FUNCTION_TEMPLATE.format(idx=j, complexity=10))
    ...
    def test_concurrent_cache_access(self):
        ...
        try:
            ...
            for path in files:
                ...
                assert all(c1.chunk_id == c2.chunk_id 
    ...
    def test_completion_order_tracking(self):
        ...
        try:
            ...
            for i in range(10):
                ...
                # Vary content size
                content = PYTHON_FUNCTION_TEMPLATE.format(idx=i, complexity=10) * (i + 1)
...
class TestMemoryUsage:
    ...
    def test_memory_efficient_streaming(self):
        ...
        try:
            ...
            chunker_streaming = ParallelChunker("python", num_workers=1, use_streaming=True)
            results_streaming = chunker_streaming.chunk_files_parallel([large_file])
            
            # Process without streaming
            chunker_normal = ParallelChunker("python", num_workers=1, use_streaming=False)
    ...
    def test_extension_filtering(self):
        ...
        try:
            ...
            results_custom = chunk_directory_parallel(
                temp_dir, "python", extensions=[".py", ".txt", ".md"]
            )
