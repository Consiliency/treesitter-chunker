...
"""
        chunks = chunk_text_with_token_limit(large_function, "python",
            max_tokens=200)
        assert len(chunks) > 1
        for chunk in chunks:
...
class Calculator:
    ...
    def divide(self, a, b):
        ...
        return result
"""
        chunks = chunk_text_with_token_limit(class_code, "python",
            max_tokens=150)
        assert len(chunks) >= 2
        split_chunks = [c for c in chunks if c.metadata.get("is_split", False)]
        if split_chunks:
            for chunk in split_chunks:
                assert "class Calculator" in chunk.content or chunk.node_type.startswith(
                    "method_")

    @classmethod
    def test_count_chunk_tokens(cls):
        """Test token counting for individual chunks."""
        chunk = CodeChunk(language="python", file_path="test.py", node_type="function_definition", start_line=1, end_line=3, byte_start=0,
            byte_end=50, parent_context="", content="""def hello():
    print('Hello, World!')""")
