...
class MockLanguageConfig(LanguageConfig):
    ...
    def __init__(self, language_id: str, chunk_types: Set[str], ignore_types: Set[str] = None):
...
class TestChunkRuleAdvancedFeatures:
    ...
    def test_parent_type_in_should_chunk_node(self):
        ...
        class ParentAwareConfig(LanguageConfig):
            ...
            def should_chunk_node(self, node_type: str, parent_type: Optional[str] = None) -> bool:
...
class TestLanguageConfigAdditionalFeatures:
    ...
    def test_multiple_rules_same_node_type(self):
        ...
        config.add_chunk_rule(ChunkRule(
        ...
        def read_configs(iterations: int):
            ...
            try:
                for _ in range(iterations):
                    ...
                    for lang_id in configs_registered[:]:  # Copy to avoid modification during iteration
                        config = registry.get(lang_id)
                        if config and config.language_id != lang_id:
                            errors.append(f"Mismatch: expected {lang_id}, got {config.language_id}")
                    # List all languages
...
class TestCompositeConfigAdvanced:
    ...
    def test_diamond_inheritance(self):
        ...
        right.add_chunk_rule(ChunkRule(
    ...
    def test_multiple_inheritance_order(self):
        ...
        config1.add_chunk_rule(ChunkRule(
        ...
        config2.add_chunk_rule(ChunkRule(
            node_types={"shared_rule"},
