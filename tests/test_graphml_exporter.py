...
class TestGraphMLExporter:
    ...
    def test_xml_namespace(self, exporter, sample_chunk):
        ...
        assert "http://graphml.graphdrawing.org/xmlns" in root.attrib.get("{http://www.w3.org/2001/XMLSchema-instance}schemaLocation", "")
    ...
    def test_edge_creation(self, exporter):
        ...
        chunk1 = CodeChunk(
            ...
            byte_start=0, byte_end=50, content="def a(): pass",
            node_type="function", language="python", parent_context="module",
            metadata={"chunk_type": "function"}
        )
        chunk2 = CodeChunk(
            file_path="b.py", start_line=1, end_line=5,
            byte_start=0, byte_end=50, content="def b(): pass",
            node_type="function", language="python", parent_context="module",
            metadata={"chunk_type": "function"}
        )
    ...
    def test_special_character_escaping(self, exporter):
        ...
        assert "&gt;" in xml_str  # >
        assert "&amp;" in xml_str  # &
        assert "&quot;" in xml_str or '"' in xml_str  # quotes might not be escaped in text content
    ...
    def test_visualization_hints_integration(self, exporter):
        ...
        chunks = [
            CodeChunk(
                ...
                byte_start=0, byte_end=50, content="def f(): pass",
                node_type="function", language="python", parent_context="module",
                metadata={"chunk_type": "function"}
            ),
            CodeChunk(
                file_path="test.py", start_line=10, end_line=20,
                byte_start=100, byte_end=200, content="class C: pass",
                node_type="class", language="python", parent_context="module",
