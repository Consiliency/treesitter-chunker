...
''')
    ...
    def test_complete_phase10_workflow(self):
        ...
        assert metadata.relationship_type in ['semantic', 'dependency', 'usage', 'structural']
...
''')
        ...
        api_chunk = next(
            (c for c in all_chunks if '@app.route' in c.content and 'process' in c.content),
            None
        )
        ...
        if api_chunk:
            ...
            js_usage = [
                (chunk, meta) for chunk, meta in usage_context 
        ...
        print(f"✓ Processed {len(all_chunks)} chunks across {len(project_languages)} languages")
        print(f"✓ Query index contains {stats['total_chunks']} chunks")
        print(f"✓ Found {len(diff.added)} new chunks after modification")
        print(f"✓ All Phase 10 features working together successfully!")
