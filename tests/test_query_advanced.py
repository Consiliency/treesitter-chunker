...
class TestNaturalLanguageQueryEngine:
    ...
    def sample_chunks(cls):
        ...
        return [CodeChunk(language="python", file_path="/src/auth.py",
            node_type="function_definition", start_line=10, end_line=20,
            byte_start=100, byte_end=300, parent_context="class:AuthManager", content="""def authenticate_user(username, password):
    try:
        ...
        raise AuthenticationError("Invalid credentials")"""
            , chunk_id="auth_1"), CodeChunk(language="python", file_path="/src/auth.py", node_type="function_definition", start_line=25,
            end_line=30, byte_start=350, byte_end=450, parent_context="class:AuthManager", content="""def logout_user(token):
    session = Session.objects.get(token=token)
    session.delete()
    return {"status": "logged out"}"""
            , chunk_id="auth_2"), CodeChunk(language="python", file_path="/src/errors.py", node_type="class_definition", start_line=5,
            end_line=15, byte_start=50, byte_end=250, parent_context="",
    ...
    def handle_exception(self, e):
        ...
        return {"error": "Internal server error"}"""
            , chunk_id="error_1"), CodeChunk(language="javascript",
            file_path="/src/api.js", node_type="function_definition",
            start_line=20, end_line=35, byte_start=400, byte_end=700,
            parent_context="", content="""async function fetchUserData(userId) {
    try {
        ...
        throw error;
            ...
            , chunk_id="api_1"), CodeChunk(language="python", file_path="/tests/test_auth.py", node_type="function_definition",
            start_line=10, end_line=20, byte_start=200, byte_end=400,
            parent_context="class:TestAuthentication", content="""def test_login_success(self):
    ...
    def test_natural_language_search(query_engine, sample_chunks):
        ...
        results = query_engine.search("find authentication functions",
            sample_chunks, QueryType.NATURAL_LANGUAGE)
        assert len(results) >= 2
        assert results[0].chunk.chunk_id in {"auth_1", "auth_2"}
        assert results[0].score > 0.3
        auth_intents_found = any("authentication" in r.metadata.get(
            "matched_intents", []) for r in results[:2])
        assert auth_intents_found or any("auth" in r.chunk.content.lower() for
            r in results[:2])

    @staticmethod
    def test_error_handling_search(query_engine, sample_chunks):
        """Test searching for error handling code."""
        results = query_engine.search("show me error handling code",
            sample_chunks, QueryType.NATURAL_LANGUAGE)
        assert len(results) >= 2
    ...
    def test_structured_search(query_engine, sample_chunks):
        ...
        results = query_engine.search(
            "type:function_definition language:python", sample_chunks,
    ...
    def test_regex_search(query_engine, sample_chunks):
        ...
        results = query_engine.search("async\\s+function", sample_chunks,
            QueryType.REGEX)
    ...
    def test_ast_pattern_search(query_engine, sample_chunks):
        ...
        results = query_engine.search(
            "function_definition parent=class:AuthManager", sample_chunks,
            QueryType.AST_PATTERN)
        assert len(results) == 2
        assert all(r.chunk.parent_context == "class:AuthManager" for r in
            results)
    ...
    def test_filter_functionality(query_engine, sample_chunks):
        ...
        filtered = query_engine.filter(sample_chunks, node_types=[
            "class_definition"])
        assert len(filtered) == 1
        assert filtered[0].chunk_id == "error_1"
        filtered = query_engine.filter(sample_chunks, min_lines=10,
            max_lines=15)
    ...
    def test_find_similar_chunks(query_engine, sample_chunks):
        ...
        results = query_engine.find_similar(auth_chunk, sample_chunks,
            threshold=0.3)
        assert len(results) >= 1
    ...
    def test_no_matches(query_engine, sample_chunks):
        ...
        results = query_engine.search("find quantum computing algorithms",
            sample_chunks)
    ...
    def test_case_insensitive_search(query_engine, sample_chunks):
        ...
        assert [r.chunk.chunk_id for r in results1] == [r.chunk.chunk_id for
            r in results2]

    @staticmethod
    def test_highlight_generation(query_engine, sample_chunks):
        """Test highlight position generation."""
        results = query_engine.search("user password", sample_chunks,
...
class TestAdvancedQueryIndex:
    ...
    def sample_chunks(cls):
        ...
        return [CodeChunk(language="python", file_path="/src/utils.py",
            node_type="function_definition", start_line=1, end_line=5,
            byte_start=0, byte_end=100, parent_context="", content="""def calculate_sum(a, b):
    return a + b""", chunk_id="chunk_1"), CodeChunk(language="python", file_path="/src/utils.py", node_type="function_definition", start_line=7,
            end_line=12, byte_start=120, byte_end=250, parent_context="",
            content="""def calculate_product(a, b):
    return a * b""",
            chunk_id="chunk_2"), CodeChunk(language="javascript", file_path="/src/math.js", node_type="function_definition", start_line=1,
            end_line=3, byte_start=0, byte_end=50, parent_context="",
            content="function add(x, y) { return x + y; }", chunk_id="chunk_3"),
            ]
    ...
    def test_update_chunk(cls, index, sample_chunks):
        ...
        index.build_index(sample_chunks)
        updated_chunk = CodeChunk(language="python", file_path="/src/utils.py", node_type="function_definition", start_line=1,
...
class TestSmartQueryOptimizer:
    ...
    def test_typo_correction(optimizer):
        ...
        optimized = optimizer.optimize_query(
            "find fucntion with retrun statement", QueryType.NATURAL_LANGUAGE)
    ...
    def test_synonym_expansion(optimizer):
        ...
        optimized = optimizer.optimize_query("find authentication methods",
            QueryType.NATURAL_LANGUAGE)
    ...
    def test_stop_word_removal(optimizer):
        ...
        optimized = optimizer.optimize_query(
            "find the function with a return statement", QueryType.
            NATURAL_LANGUAGE)
    ...
    def test_query_suggestions(cls, optimizer):
        ...
        chunks = [CodeChunk(language="python", file_path="/test.py",
            node_type="function_definition", start_line=1, end_line=5,
            byte_start=0, byte_end=100, parent_context="", content="""def test_authentication():
    pass""", chunk_id="1"),
            CodeChunk(language="python", file_path="/test.py", node_type="class_definition", start_line=10, end_line=20, byte_start=200,
            byte_end=400, parent_context="", content="""class TestUser:
    pass""", chunk_id="2")]
    ...
    def test_structured_key_normalization(optimizer):
        ...
        optimized = optimizer.optimize_query("lang:python typ:function",
            QueryType.STRUCTURED)
...
class TestIntegration:
    ...
    def test_end_to_end_query(cls, query_system):
        ...
        chunks = [CodeChunk(language="python", file_path="/src/auth.py",
            node_type="function_definition", start_line=1, end_line=10,
            byte_start=0, byte_end=200, parent_context="", content="""def login_user(username, password):
    if not username or not password:
        raise ValueError("Missing credentials")
    user = authenticate(username, password)
    return create_session(user)"""
            , chunk_id="auth_func"), CodeChunk(language="python", file_path="/tests/test_auth.py", node_type="function_definition",
            start_line=1, end_line=8, byte_start=0, byte_end=150,
            parent_context="", content="""def test_login():
