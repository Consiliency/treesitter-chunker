...
'''
        ...
        for lang, result in results.items():
            assert result['success'], f"{lang} failed: {result.get('error', 'Unknown error')}"
...
class TestConcurrentParsing:
    ...
    def test_stress_concurrent_parsing(self):
        ...
        errors = []
        parse_counts = {lang: 0 for lang in languages}
        lock = threading.Lock()
...
class TestMemoryEfficiency:
    ...
    def test_parser_reuse(self):
        ...
        from chunker.parser import _initialize, _factory, _registry
