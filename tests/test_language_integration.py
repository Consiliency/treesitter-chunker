...
""")
        ...
        language_config_registry.clear()
        if original_config:
            language_config_registry.register(original_config, aliases=["py", "python3"])
        else:
...
""")
        ...
        for chunk in chunks:
            ...
            elif chunk.node_type == "function_definition" and "inner_function" in chunk.content and "deeply_nested" not in chunk.content:
                # Function inside outer_method
                assert chunk.parent_context == "function_definition"
            elif chunk.node_type == "function_definition" and "deeply_nested" in chunk.content:
                # Function inside inner_function
                assert chunk.parent_context == "function_definition"
            elif chunk.node_type == "class_definition" and "InnerClass" in chunk.content:
...
""")
        ...
        assert any("@decorator" in c and "decorated_function" in c for c in decorated_contents)
        assert any("@decorator1" in c and "@decorator2" in c for c in decorated_contents)
        assert any("@dataclass" in c and "DecoratedClass" in c for c in decorated_contents)