...
class TestPerformanceImpactOfConfigLookups:
    ...
    def test_config_lookup_overhead_during_parsing(self):
        """Test the overhead of frequent config lookups during parsing."""
        # Create a mock parser that performs config lookups
        class MockParser:
            ...
            def parse_with_config_lookups(self, text: str, lookup_frequency: int) -> List[Dict]:
        ...
        # Test different lookup frequencies
        ...
        for freq in frequencies:
            ...
            results[freq] = {
                ...
                "lookups_per_second": parser.lookup_count / parser.parse_time if parser.parse_time > 0 else 0
            }
        
        # Analyze performance impact
        baseline = results[500]  # Least frequent lookups
        
        for freq in frequencies:
            if freq != 500:
                overhead = (results[freq]["parse_time"] - baseline["parse_time"]) / baseline["parse_time"] * 100
                print(f"Lookup every {freq} tokens: {overhead:.1f}% overhead, "
                      f"{results[freq]['lookups_per_second']:.0f} lookups/sec")
    ...
    def test_config_caching_effectiveness(self):
        ...
        class CachedConfigParser:
            ...
            def lookup_with_cache(self, key: str, default: Any = None) -> Any:
                ...
                time.sleep(0.0001)  # 0.1ms lookup time
        ...
        # Simulate parsing with repeated config lookups
        common_keys = [
    ...
    def test_parallel_parsing_config_contention(self):
        ...
        class ThreadSafeConfigStore:
            ...
            def get(self, key: str, default: Any = None) -> Any:
                ...
                acquired = self.lock.acquire(timeout=0.001)  # 1ms timeout
                
                if not acquired:
                    self.contention_events += 1
                    self.lock.acquire()  # Wait indefinitely
        ...
        for num_threads in thread_counts[1:]:
            speedup = single_thread_time / performance_results[num_threads]["elapsed_time"]
            efficiency = speedup / num_threads * 100
            
            print(f"{num_threads} threads: {speedup:.2f}x speedup, "
                  f"{efficiency:.0f}% efficiency, "
                  f"{performance_results[num_threads]['contention_events']} contentions")
...
class TestConfigHotReloadingDuringChunking:
    ...
    def test_hot_reload_during_chunking(self, tmp_path):
        ...
        # Modify config while chunking
        modified_config = initial_config.copy()
        modified_config["chunk_size"] = 50  # Smaller chunks
        ...
        modified_config["chunk_size"] = 150  # Larger chunks
    ...
    def test_config_consistency_during_reload(self):
        ...
        def reader_thread(thread_id: int, iterations: int):
            ...
            for i in range(iterations):
                ...
                # All settings values should be version-appropriate
                version = config.get("version", 0)
                expected_sum = version * 6  # sum(1,2,3) * version
                actual_sum = sum(config.get("settings", {}).values())
                
                if actual_sum != expected_sum:
                    inconsistencies.append(
                        f"Thread {thread_id}: Version {version} has sum {actual_sum}, "
...
class TestMemoryUsageWithLargeConfigHierarchies:
    ...
    def test_large_config_hierarchy_memory(self):
        ...
        # Create large hierarchical config
        # Depth 5, breadth 10 = 10^5 = 100,000 leaf nodes
        ...
        print(f"Total nodes: {total_nodes}, Bytes per node: {bytes_per_node:.2f}")
        ...
        # Test config traversal performance
        ...
        print(f"Traversal time: {traversal_time:.3f}s for {len(all_paths)} paths")
    ...
    def test_config_inheritance_memory_efficiency(self):
        ...
        class InheritanceConfig:
            ...
            def add_inherited_config(self, name: str, parent: str, overrides: Dict[str, Any]):
        ...
        # Create many inherited configs with small overrides
        for i in range(100):
        ...
        for i in range(100):
            ...
            assert config["common"]["retry_count"] == 3  # From base
            assert config["common"]["timeout"] == 30 + i  # Override
        
        # Measure memory after resolution
        stats_after = config_manager.get_memory_stats()
        
        # Calculate memory efficiency
        memory_per_config = stats_after["estimated_memory"] / (stats_after["inherited_configs"] + 1)
        
        print(f"Memory before resolution: {stats_before['estimated_memory'] / 1024:.2f} KB")
        print(f"Memory after resolution: {stats_after['estimated_memory'] / 1024:.2f} KB")
    ...
    def test_weak_reference_config_cleanup(self):
        ...
        class WeakConfigManager:
            ...
            def add_config(self, name: str, config: ConfigObject, critical: bool = False):
                ...
                if critical:
                    ...
                    self.weak_refs[name] = weakref.ref(config, 
            ...
            def get_config(self, name: str) -> Optional[ConfigObject]:
                ...
                elif name in self.weak_refs:
                    config = self.weak_refs[name]()
                    if config is not None:
                        return config
                    else:
                        # Config was garbage collected
            ...
            def get_stats(self) -> Dict[str, Any]:
                ...
                alive_weak = sum(1 for ref in self.weak_refs.values() if ref() is not None)
                return {
        ...
        # Delete references to some cached configs
        del cached_configs[10:]  # Delete last 10
        gc.collect()
        ...
        assert config is None  # Should be collected
        ...
        # Check final state
        stats = manager.get_stats()
        assert stats["strong_refs"] == 2  # Critical configs remain
        
        # Verify critical configs still accessible
...
class TestCircularDependencyDetectionEdgeCases:
    ...
    def test_simple_circular_dependency(self):
        ...
        class ConfigResolver:
            ...
            def resolve(self, name: str, path: List[str] = None) -> Dict[str, Any]:
                ...
                if name in self.resolving:
                    ...
                    raise ConfigurationError(f"Circular dependency detected: {' -> '.join(cycle)}")
        ...
        # Test 3: Long chain circular dependency
        resolver.configs.clear()
    ...
    def test_complex_inheritance_cycles(self):
                ...
                try:
                    ...
                    if "_mixins" in config:
                        ...
                        for mixin in mixins:
                            ...
                            # Mixin configs don't override existing values
        ...
        # Test 1: Diamond inheritance (no cycle)
        ...
        resolver.add_config("diamond", {"_extends": ["left", "right"], "diamond_value": 3})
        ...
        # Test 2: Mixin with circular dependency
        resolver.configs.clear()
        ...
        # Test 3: Complex multi-level inheritance with cycle
        resolver.configs.clear()
        resolver.resolved_cache.clear()
        ...
        resolver.add_config("level5", {"_extends": "level1", "l5": 5})  # Cycle!
    ...
    def test_dynamic_circular_dependencies(self):
        ...
        class DynamicConfigSystem:
            def __init__(self):
                self.configs = {}
                self.reference_graph = {}  # Track dependencies
        ...
        # Create cycle through value reference
        with pytest.raises(ConfigurationError):
            system.update_config("config3", {"value": "${config1.value}"})
        
        # Test conditional cycles
        system.configs.clear()
        system.reference_graph.clear()
        ...
        # Conditionally create cycle based on environment
        try:
        ...
        # Verify original state preserved after failed update
