...
"""
    ...
    def test_chunk_with_token_limit(self):
        ...
        lines = ["def large_function():", "    # This is a very large function that will exceed token limits"]
        ...
        chunks = self.chunker.chunk_with_token_limit(
            str(file_path), "python", max_tokens=50
        )
        
        # Debug: print chunk info
        for i, chunk in enumerate(chunks):
            print(f"Chunk {i}: tokens={chunk.metadata.get('token_count', 0)}, lines={chunk.end_line - chunk.start_line + 1}")
...
'''
        ...
        chunks = self.chunker.chunk_with_token_limit(
            str(file_path), "python", max_tokens=80
        )
        ...
        for chunk in chunks:
            if "is_split" in chunk.metadata and chunk.metadata["is_split"]:
...
"""
        ...
        chunks = self.chunker.chunk(tree.root_node, content.encode(), str(file_path), "python")
...
"""
        ...
        gpt4_chunks = self.chunker.chunk_with_token_limit(
            str(file_path), "python", max_tokens=1000, model="gpt-4"
        )
        
        claude_chunks = self.chunker.chunk_with_token_limit(
            str(file_path), "python", max_tokens=1000, model="claude"
        )
    ...
    def test_edge_case_empty_file(self):
        ...
        chunks = self.chunker.chunk_with_token_limit(
            str(file_path), "python", max_tokens=100
        )
...
"""
        ...
        chunks = self.chunker.chunk_with_token_limit(
            str(file_path), "python", max_tokens=30
        )
        ...
        if split_chunks:
            ...
            original_ids = set(c.metadata.get("original_chunk_id") for c in split_chunks)
...
"""
        ...
        js_chunks = self.chunker.chunk_with_token_limit(
            str(js_file), "javascript", max_tokens=100
        )
...
"""
        ...
        chunks = self.chunker.chunk_with_token_limit(
            str(file_path), "python", max_tokens=10000
        )
