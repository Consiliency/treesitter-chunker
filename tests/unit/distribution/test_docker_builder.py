...
class TestDockerBuilder:
    ...
    def test_single_platform_build(cls, mock_which, mock_run):
        ...
        mock_run.side_effect = [Mock(returncode=1), Mock(returncode=0),
            Mock(returncode=0, stdout="Successfully built abcd1234", stderr=""), Mock(returncode=0, stdout="sha256:abcd1234567890", stderr=""),
            ]
        builder = DockerBuilder()
        with tempfile.TemporaryDirectory() as tmpdir:
            dockerfile = Path(tmpdir) / "Dockerfile"
            ...
            success, image_id = builder.build_image("test:latest",
                platforms=["linux/amd64"], dockerfile_path=dockerfile)
            assert success
    ...
    def test_multi_platform_build(cls, mock_which, mock_run):
        ...
        mock_run.side_effect = [Mock(returncode=0), Mock(returncode=0),
            ...
            Mock(returncode=0, stdout="writing image sha256:multi123 done",
            stderr="")]
        builder = DockerBuilder()
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            dockerfile.write_text("FROM python:3.9")
            success, image_id = builder.build_image("test:latest",
                platforms=["linux/amd64", "linux/arm64"], dockerfile_path=dockerfile)
            assert success
    ...
    def test_verify_image(cls, mock_which, mock_run):
        ...
        mock_which.return_value = "/usr/bin/docker"
        image_data = {"Id": "sha256:abcd1234", "Created":
            "2023-01-01T00:00:00Z", "Size": 1234567, "Architecture":
            "amd64", "Os": "linux", "RootFS": {"Layers": ["layer1", "layer2"]}}
