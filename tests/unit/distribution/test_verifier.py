...
class TestInstallationVerifier:
    ...
    def test_verify_pip_installation_success(cls, mock_run, mock_venv):
        ...
        verifier = InstallationVerifier()
        mock_run.side_effect = [Mock(returncode=0, stdout="Successfully installed", stderr=""), Mock(returncode=0, stdout="1.0.0", stderr=""), Mock(returncode=0, stdout="chunker version 1.0.0", stderr=""), Mock(returncode=0, stdout="Functionality test passed", stderr="")]
    ...
    def test_verify_conda_installation(cls, mock_run, mock_which):
        ...
        verifier = InstallationVerifier()
        mock_run.side_effect = [Mock(returncode=0, stdout="Environment created", stderr=""), Mock(returncode=0, stdout="Package installed", stderr=""), Mock(returncode=0, stdout="1.0.0", stderr=""), Mock(returncode=0)]
        success, details = verifier.verify_installation("conda", "linux")
        assert success
    ...
    def test_verify_docker_installation(cls, mock_which, mock_run):
        ...
        verifier = InstallationVerifier()
        mock_run.side_effect = [Mock(returncode=1, stdout="", stderr="Unable to find image")]
        success, details = verifier.verify_installation("docker", "linux")
    ...
    def test_verify_homebrew_installation(cls, mock_which, mock_run):
        ...
        verifier = InstallationVerifier()
        mock_run.side_effect = [Mock(returncode=1, stdout="", stderr="Formula not found")]
        success, details = verifier.verify_installation("homebrew", "darwin")
        assert isinstance(success, bool)
        assert isinstance(details, dict)
        assert "tests_passed" in details
    ...
    def test_verify_homebrew_local_formula(cls, mock_exists, mock_run,
        mock_which):
        """Test Homebrew verification with local formula fallback"""
        mock_which.return_value = "/usr/local/bin/brew"
        mock_exists.return_value = True
        verifier = InstallationVerifier()
        mock_run.side_effect = [Mock(returncode=1, stdout="", stderr="Formula not found"), Mock(returncode=0, stdout="Formula installed", stderr=""), Mock(returncode=0, stdout="1.0.0", stderr=""), Mock(returncode=0, stdout="Test passed",
            stderr="")]
        success, details = verifier.verify_installation("homebrew", "darwin")
        assert success
