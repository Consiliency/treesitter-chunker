...
from chunker.interfaces.context import ContextType, ContextItem
...
class TestBaseContextExtractor:
    ...
    def test_build_context_prefix_basic(self):
        ...
        # Create mock nodes (in real tests, these would be actual AST nodes)
    ...
    def test_build_context_prefix_with_types(self):
    ...
    def test_build_context_prefix_with_max_size(self):
        ...
        # The truncation message adds to the length, so we check the actual truncation happened
        assert "truncated" in result
        # The original content should be truncated
        assert len(result.split('\n')[0]) <= 20
    
    def test_build_context_prefix_sorted_by_importance(self):
...
'''.strip()
    ...
    def test_extract_imports(self, python_code):
    ...
    def test_extract_type_definitions(self, python_code):
        """Test extracting type definitions from Python code."""
        parser = get_parser('python')
        tree = parser.parse(python_code.encode())
        
        extractor = ContextFactory.create_context_extractor('python')
        type_defs = extractor.extract_type_definitions(tree.root_node, python_code.encode())
        
        assert len(type_defs) == 2
        assert type_defs[0].type == ContextType.TYPE_DEF
        assert "class User:" in type_defs[0].content
        assert "..." in type_defs[0].content  # Should be truncated
        assert "class UserManager:" in type_defs[1].content
    
    def test_extract_decorators(self, python_code):
    ...
    def test_extract_parent_context(self, python_code):
        ...
        parent_context = extractor.extract_parent_context(
            add_user_method, tree.root_node, python_code.encode()
        )
...
'''.strip()
    ...
    def test_extract_imports(self, javascript_code):
    ...
    def test_extract_type_definitions(self, javascript_code):
        """Test extracting class definitions from JavaScript code."""
        parser = get_parser('javascript')
        tree = parser.parse(javascript_code.encode())
        
        extractor = ContextFactory.create_context_extractor('javascript')
        type_defs = extractor.extract_type_definitions(tree.root_node, javascript_code.encode())
    ...
    def test_extract_parent_context(self, javascript_code):
        ...
        parent_context = extractor.extract_parent_context(
            add_user_method, tree.root_node, javascript_code.encode()
        )
