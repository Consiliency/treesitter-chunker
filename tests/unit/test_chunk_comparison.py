...
"""
    ...
    def test_compare_multiple_strategies(comparison, test_file):
        ...
        result = comparison.compare_strategies(test_file, "python", [
    ...
    def test_compare_nonexistent_file(comparison):
        ...
        with pytest.raises(FileNotFoundError):
            comparison.compare_strategies("nonexistent.py", "python", [
                "default"])
    ...
    def test_empty_file_handling(cls, comparison):
        ...
        with tempfile.NamedTemporaryFile(encoding="utf-8", mode="w", suffix=".py", delete=False) as f:
            ...
            f.flush()
        try:
            result = comparison.compare_strategies(f.name, "python", [
                "default"])
            assert "strategies" in result
            assert "default" in result["strategies"]
        finally:
