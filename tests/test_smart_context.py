...
class TestTreeSitterSmartContextProvider:
    ...
    def test_semantic_context(self, provider, sample_chunks):
        """Test semantic context extraction."""
        # Test getting semantic context for the calculate_total function
    ...
    def test_semantic_similarity_calculation(self, provider):
        ...
        # Should have some similarity due to same node type and similar structure
        assert 0 < similarity < 1
        assert similarity > 0.3  # Same node type gives 0.3
    
    def test_dependency_context(self, provider, sample_chunks):
        """Test dependency context extraction."""
        # The ShoppingCart class depends on calculate_total
        dependencies = provider.get_dependency_context(sample_chunks[2], sample_chunks)
        
        assert isinstance(dependencies, list)
        assert len(dependencies) > 0
        
        # Check that calculate_total is identified as a dependency
    ...
    def test_usage_context(self, provider, sample_chunks):
        """Test usage context extraction."""
        # Find usages of calculate_total
        usages = provider.get_usage_context(sample_chunks[0], sample_chunks)
        
        assert isinstance(usages, list)
        assert len(usages) > 0
        
        # ShoppingCart should be identified as using calculate_total
        usage_chunks = [usage[0] for usage in usages]
        usage_ids = [chunk.chunk_id for chunk in usage_chunks]
        assert "chunk3" in usage_ids  # ShoppingCart class
    
    def test_structural_context(self, provider, sample_chunks):
        """Test structural context extraction."""
        # Test structural relationships between chunks in the same file
    ...
    def test_extract_imports(self, provider):
...
import numpy as np""",
    ...
    def test_extract_function_calls(self, provider):
    ...
    def test_extract_exports(self, provider):
...
async def async_function():
    ...
    def test_structural_relationships(self, provider):
...
class TestContextStrategies:
    ...
    def test_relevance_strategy(self):
        ...
        # Test selection with token limit
        selected = strategy.select_context(main_chunk, candidates, max_tokens=120)
        
        # Should select helper1 and helper2 (50 + 60 = 110 tokens)
        assert len(selected) == 2
        assert selected[0].chunk_id == "helper1"  # Highest relevance
        ...
        assert len(ranked) == 3
        assert ranked[0][0].chunk_id == "helper1"  # Highest score
        assert ranked[0][1] > ranked[1][1]  # Scores in descending order
    
    def test_hybrid_strategy(self):
        """Test hybrid context selection strategy."""
        strategy = HybridContextStrategy(weights={
            'dependency': 0.4,
            'semantic': 0.3,
            'usage': 0.2,
            'structural': 0.1
        ...
        # Should include chunks from different types based on weights
        ...
        assert len(ranked) == 3
        # dep1 should rank highest (0.9 * 0.4 = 0.36)
...
class TestContextCache:
    ...
    def test_cache_expiration(self):
        ...
        cache = InMemoryContextCache(ttl=0.1)  # 0.1 second TTL
...
class TestIntegration:
    ...
    def test_full_context_workflow(self):
        ...
        # Create a set of interconnected chunks
        chunks = [
            CodeChunk(
                language="python",
