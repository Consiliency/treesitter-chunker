...
class TestParquetCLIIntegration:
    ...
    def test_parquet_with_include_exclude_filters(
        self,
        temp_workspace,
        test_file_generator,
        sample_code_files,
    ...
    def test_parquet_with_chunk_type_filtering(
        self,
        temp_workspace,
        test_file_generator,
    ):
...
"""
        ...
        function_names = df[df["chunk_type"].str.contains("function", case=False)][
            "name"
        ].tolist()
...
"""
    ...
    def test_parquet_with_parallel_processing(
        self,
        temp_workspace,
        test_file_generator,
        performance_monitor,
...
"""
        ...
        duplicates = df.duplicated(
            subset=["file_path", "chunk_type", "name", "start_line"],
        )
        ...
        assert (
            perf_stats[4] < perf_stats[1] * 0.8
        ), "Parallel processing not improving performance"

    def test_large_file_streaming_to_parquet(
        self,
        test_file_generator,
        temp_workspace,
        performance_monitor,
...
""",
        ...
        for batch_size in batch_sizes:
            ...
            assert len(df_batch) == len(
                df,
            ), f"Different results with batch size {batch_size}"

    def test_schema_evolution_across_languages(
        self,
        temp_workspace,
        test_file_generator,
        sample_code_files,
    ...
    def test_compression_options(
        ...
        for compression in compression_types:
            output_file = (
                temp_workspace
                / "output"
                / f"compressed_{compression or 'none'}.parquet"
            )
            ...
            results[compression or "none"] = {
                "size": file_size,
                "time": performance_monitor.get_stats(
                    f"compress_{compression or 'none'}",
                )["average"],
        ...
        for compression in results:
            ...
            decomp_time = performance_monitor.get_stats(f"decompress_{compression}")[
                "average"
            ]
...
"""
        ...
        if percentages:
            ...
            assert (
                percentages[-1] > 80
            ), f"Progress didn't reach completion: {percentages[-1]}%"
            ...
            assert (
                large_jumps < len(percentages) * 0.2
