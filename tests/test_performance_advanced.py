    ...
    def test_streaming_memory_efficiency(self, tmp_path):
        ...
        for i in range(500):
            content_lines.extend([
                ...
                f"    result = x + y * z",
                f"    data = [j for j in range(10)]",
                f"    return result + sum(data)",
                ""
            ])
...
class TestRealWorldScenarios:
    ...
    def test_mixed_file_sizes_performance(self, tmp_path):
        ...
        for i in range(5):
            ...
            for j in range(100):
                content_lines.extend([
                    ...
                    f"    ",
                    f"    def method_a(self):",
                    f"        return len(self.data)",
                    f"    ",
                    f"    def method_b(self, value):",
                    f"        self.data.append(value)",
                    f"        return self.data",
                    ""
                ])
            large_file.write_text('\n'.join(content_lines))
        ...
        small_chunks = sum(
            len(chunks) for path, chunks in results.items()
            if "small_" in path.name
        )
        medium_chunks = sum(
            len(chunks) for path, chunks in results.items()
            if "medium_" in path.name
        )
        large_chunks = sum(
            len(chunks) for path, chunks in results.items()
            if "large_" in path.name
        )
