...
""",
    ...
    def test_track_relationship_basic(cls, tracker):
        ...
        chunk1 = CodeChunk(language="python", file_path="test.py",
            ...
            byte_start=0, byte_end=100, parent_context="", content="def foo(): pass")
        chunk2 = CodeChunk(language="python", file_path="test.py",
            node_type="function_definition", start_line=7, end_line=10,
            byte_start=101, byte_end=200, parent_context="", content="def bar(): foo()")
        tracker.track_relationship(chunk2, chunk1, RelationshipType.CALLS,
            {"function": "foo"})
    ...
    def test_get_relationships_filtered(cls, tracker):
        ...
        for i in range(3):
            chunk = CodeChunk(language="python", file_path=f"test{i}.py",
                node_type="function_definition", start_line=1, end_line=5,
                byte_start=0, byte_end=100, parent_context="", content=f"def func{i}(): pass")
            chunks.append(chunk)
        tracker.track_relationship(chunks[0], chunks[1], RelationshipType.CALLS,
            )
        tracker.track_relationship(chunks[1], chunks[2], RelationshipType.CALLS,
            )
        tracker.track_relationship(chunks[0], chunks[2], RelationshipType.
            DEPENDS_ON)
        ...
        depends_rels = tracker.get_relationships(relationship_type=RelationshipType.DEPENDS_ON)
    ...
    def test_infer_python_inheritance(tracker, python_chunks):
        ...
        dog_inherits = any(r.metadata.get("base_class") == "Animal" for r in
            inheritance_rels)
        assert dog_inherits
    ...
    def test_infer_python_calls(tracker, python_chunks):
        ...
        relationships = tracker.infer_relationships(python_chunks)
        call_rels = [r for r in relationships if r.relationship_type ==
    ...
    def test_infer_python_imports(tracker, python_chunks):
        ...
        relationships = tracker.infer_relationships(python_chunks)
        dep_rels = [r for r in relationships if r.relationship_type ==
    ...
    def test_infer_javascript_inheritance(tracker, javascript_chunks):
        ...
        inheritance_rels = [r for r in relationships if r.relationship_type ==
            RelationshipType.INHERITS]
        assert len(inheritance_rels) >= 1
        circle_extends = any(r.metadata.get("base_class") == "Shape" for r in
            inheritance_rels)
    ...
    def test_infer_javascript_calls(tracker, javascript_chunks):
        ...
        create_circle_called = any(r.metadata.get("function") ==
            "createCircle" for r in call_rels)
    ...
    def test_clear_relationships(cls, tracker):
        ...
        chunk2 = CodeChunk(language="python", file_path="test.py",
            node_type="function_definition", start_line=7, end_line=10,
            byte_start=101, byte_end=200, parent_context="", content="def bar(): pass")
...
""",
        ...
        inheritance_count = sum(1 for r in relationships if r.
            relationship_type == RelationshipType.INHERITS)
        assert inheritance_count >= 1
        call_count = sum(1 for r in relationships if r.relationship_type ==
            RelationshipType.CALLS)
        assert call_count >= 2
