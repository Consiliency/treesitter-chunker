...
from chunker.interfaces.metadata import SignatureInfo, ComplexityMetrics
...
'''
        ...
        assert signature.name == '<anonymous>'  # Arrow functions are typically anonymous
        assert len(signature.parameters) == 2
...
'''
        ...
        method_nodes = [child for child in interface_node.children 
...
'''
        ...
        factorial_chunk = next(c for c in chunks if c.node_type == 'function_definition')
        ...
        assert 'factorial' in deps or len(deps) == 0  # May not detect self-reference in all cases
        
        # Check Calculator.add method
        add_chunk = next((c for c in chunks if c.node_type == 'method_definition'), None)
        if add_chunk:
