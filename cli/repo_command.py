...
def process(
    ...
    file_pattern: Optional[str] = typer.Option(None, help="Glob pattern for files to include"),
    exclude: Optional[List[str]] = typer.Option(None, help="Patterns to exclude"),
    output: Optional[Path] = typer.Option(None, help="Output file for results (JSON)"),
    ...
    traversal: str = typer.Option("depth-first", help="Traversal strategy: depth-first or breadth-first"),
):
...
def changed(
    repo_path: str = typer.Argument(help="Path to repository"),
    since: Optional[str] = typer.Option(None, help="Show changes since this commit (e.g., HEAD~1)"),
    branch: Optional[str] = typer.Option(None, help="Compare with this branch"),
):
