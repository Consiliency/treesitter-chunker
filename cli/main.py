...
def get_files_from_patterns(patterns: List[str], base_path: Path = Path.cwd()) -> Iterator[Path]:
...
def chunk(
    ...
    language: Optional[str] = typer.Option(None, "--lang", "-l", help="Language name (auto-detect if not specified)"),
    json_out: bool = typer.Option(False, "--json", help="Output JSON instead of Rich table"),
    chunk_types: Optional[str] = typer.Option(None, "--types", "-t", help="Comma-separated list of chunk types to include"),
    min_size: Optional[int] = typer.Option(None, "--min-size", help="Minimum chunk size in lines"),
    max_size: Optional[int] = typer.Option(None, "--max-size", help="Maximum chunk size in lines"),
...
@app.command()
def batch(
    paths: Optional[List[Path]] = typer.Argument(None, help="Files or directories to process"),
    pattern: Optional[str] = typer.Option(None, "--pattern", "-p", help="Glob pattern for files"),
    language: Optional[str] = typer.Option(None, "--lang", "-l", help="Language name (auto-detect if not specified)"),
    json_out: bool = typer.Option(False, "--json", help="Output JSON/JSONL"),
    jsonl: bool = typer.Option(False, "--jsonl", help="Output as JSONL (one JSON per line)"),
    chunk_types: Optional[str] = typer.Option(None, "--types", "-t", help="Comma-separated list of chunk types"),
    min_size: Optional[int] = typer.Option(None, "--min-size", help="Minimum chunk size in lines"),
    max_size: Optional[int] = typer.Option(None, "--max-size", help="Maximum chunk size in lines"),
    include: Optional[str] = typer.Option(None, "--include", "-i", help="Include file patterns (comma-separated)"),
    exclude: Optional[str] = typer.Option(None, "--exclude", "-e", help="Exclude file patterns (comma-separated)"),
    recursive: bool = typer.Option(True, "--recursive/--no-recursive", "-r/-R", help="Recursively process directories"),
    parallel: Optional[int] = typer.Option(None, "--parallel", "-j", help="Number of parallel workers"),
    config: Optional[Path] = typer.Option(None, "--config", "-c", help="Path to config file"),
    quiet: bool = typer.Option(False, "--quiet", "-q", help="Suppress progress output"),
    from_stdin: bool = typer.Option(False, "--stdin", help="Read file paths from stdin"),
):
    ...
    if from_stdin:
        ...
        if not paths and pattern:
                ...
                if path.is_file():
                    ...
                    if recursive:
                        for file_path in path.rglob('*'):
                            if file_path.is_file() and should_include_file(file_path, include_patterns, exclude_patterns):
                                files_to_process.append(file_path)
                    else:
            ...
            console.print("[red]Error: No files specified. Use paths, --pattern, or --stdin[/red]")
            raise typer.Exit(1)
    ...
    if quiet:
        ...
        with ThreadPoolExecutor(max_workers=parallel) as executor:
            futures = {executor.submit(process_with_progress, f): f for f in files_to_process}
    ...
    if jsonl:
        ...
        tbl = Table(title=f"Chunk Summary ({total_chunks} total chunks from {len(files_to_process)} files)")
